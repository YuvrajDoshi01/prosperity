...
r2_v5: r1_v17 EXACTLY + bid()=MAF_BID for Round 2 MAF.

Empirical rationale for choosing v17 over r2_v2 (r1_v4 base) for R2 website:
  - r2_v2 submission 274128 website score: 8,412 (IPR 7,386 + ACO 1,026)
  - ACO position ended at -76 (near limit short), FV=10000 off from actual ~10004
  - Local BT on website day-1 data shows:
      r2_v2 (v4 base, FV=10000):   7,496  (IPR 7,403 + ACO 93)
      r2_v4 (median-of-10):        7,298  (IPR 7,403 + ACO -105)
      r1_v17 (tick-0 wall-mid):    7,680  (IPR 7,354 + ACO 326)
  - v17 beats v2 by +184 on the exact data website used. Tick-0 wall-mid
    bootstrap (+cubic skew + crash_mode) handles the FV shift.
  - CSV-based ranking is misleading (different realization from website).

MAF bid = 0 (conservative). Rationale:
  - R1 cumulative = 177k already clears the 200k threshold when combined with
    any reasonable R2 score. No need to chase extra edge via MAF.
  - Structural argument: injected quote levels land BETWEEN existing L1/L2
    ticks, tightening the effective spread and competing with our maker
    quotes. For a making-heavy strategy (LU take-clear-make + cubic skew),
    MAF likely reduces fill rate and narrows captured spread => net negative.
  - Community consensus (Prosperity Discord) aligns: "past 180k DO NOT BID".
  - Local BT --extra-flow=scale shows only +0.3% uplift, suggesting the
    effect is small even in the optimistic scaling model.
  - Zero bid guarantees no MAF cost and no R2 PnL erosion.
"""

MAF_BID = 0  # Trap hypothesis: injected levels compete with our maker quotes
# (tighter effective spread, split taker flow). Community consensus + structural
# analysis both suggest MAF is net-negative for making-heavy strategies like ours.
# R1 at 177k already clears the 200k threshold, so no upside to justify the risk.
