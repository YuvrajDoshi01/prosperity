...
"""
r2_v15 -- Optimal combination based on full BT benchmark (v9-v16, R1+R2).

ACO: v14 architecture + two targeted improvements.

  v14 wins across BOTH rounds combined:
    R1: 67,584 ACO (2nd, -1,226 vs v12)
    R2: 50,939 ACO (1st, +10,514 vs v12)
    Combined: 118,523 vs v12's 109,235 (+9,288)

  v14 core strengths (kept):
    - Composite FV: 0.90 * microprice + 0.10 * anchor. Adapts maker quotes
      to market level (better passive fills) while retaining mean-reversion
      anchor (take thresholds stay near 10000).
    - Spread-regime conditioning: tight spread (<= 14) -> wider takes,
      tighter makes. Adapts to market microstructure.
    - Crash mode + cubic skew + MAD-calibrated threshold + hysteresis.

  Improvements over v14:
    1. SOFT CRASH: v14 suppresses ALL takes during crash. v15 allows takes
       at wider width (3 instead of 1). This captures strongly mispriced
       orders even during volatile periods, without taking marginal ones.
    2. FASTER BOOTSTRAP: 10 samples (1 second) instead of 20 (2 seconds).
       Anchor locks faster; fallback is FV=10000 which is correct for OU.

IPR: v9's 228959 trend-following (no warmup gate).
  Beats v16's drift-boosted FV by 12,668 on R1 (254,300 vs 241,632).
"""

LIMIT = 80

# -- ACO Parameters (v14 base + soft crash + faster bootstrap) ----------------
...
# Spread-regime conditioning (from v14)
ACO_TIGHT_SPREAD_THRESHOLD = 14
ACO_TIGHT_TAKE_WIDTH = 2
ACO_TIGHT_JOIN_EDGE = 3
ACO_TIGHT_DEFAULT_EDGE = 3

# Composite FV: base_fv = cur_mid - MR_BETA * (cur_mid - anchor)
ACO_MR_BETA = 0.10
ACO_MAX_CONCESSION = 4.0         # cubic skew max
ACO_MID_WINDOW = 21

# Bootstrap + Crash
ACO_BOOTSTRAP_SAMPLES = 10       # FASTER: 10 instead of v14's 20
ACO_CRASH_FLOOR = 15
ACO_CRASH_K_MAD = 4.0
ACO_MAD_WARMUP = 50
...
# Crash-mode maker widening
ACO_CRASH_JOIN_EDGE_BONUS = 4
ACO_CRASH_DEFAULT_EDGE_BONUS = 8

# SOFT CRASH: take at wider width during crash instead of suppressing
ACO_CRASH_TAKE_WIDTH = 3

# -- IPR Parameters (228959 trend-following) ----------------------------------
...
class Trader:
    ...
    # === ACO: v14 composite + regime + crash (with soft-crash improvement) ====
    ...
    def _aco(self, state):
        ...
        # -- Spread regime + crash -> edge selection --------------------------
        tight = spread <= ACO_TIGHT_SPREAD_THRESHOLD and not crash

        if crash:
            take_width = ACO_CRASH_TAKE_WIDTH        # SOFT CRASH: 3 (not suppressed)
        ...
        # -- Crash: suppress accumulation beyond threshold --------------------
        if crash:
        ...
        # -- 1. TAKE: always active (soft crash uses wider width) -------------
        ...
        # -- 2. CLEAR: flatten inventory at fv_eff (skip during crash) --------
        if not crash:
