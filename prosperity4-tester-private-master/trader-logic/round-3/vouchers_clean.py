...
class CleanVoucherState:
    def __init__(self):
        self.ivh = {k: [] for k in [4000, 4500, 5000, 5100, 5200, 5300, 5400, 5500]}
        self.fvd = {k: [] for k in [4000, 4500, 5000, 5100, 5200, 5300, 5400, 5500]}
        self.vfe_prices = []

    def to_dict(self):
        return {
            "ivh": {str(k): [round(x, 5) for x in v[-50:]] for k, v in self.ivh.items()},
            "fvd": {str(k): [round(x, 2) for x in v[-50:]] for k, v in self.fvd.items()},
...
def run_vouchers_clean(state: TradingState, vstate: CleanVoucherState):
    ...
    for K in STRIKES:
        ...
        diff = mid - fv
        vstate.fvd[K].append(diff)
        if len(vstate.fvd[K]) > 50:
            vstate.fvd[K].pop(0)

        hist_fvd = vstate.fvd[K]
        roll_med_fvd = sorted(hist_fvd)[len(hist_fvd)//2] if hist_fvd else 0.0
