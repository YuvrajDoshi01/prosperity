...
def run_vouchers(state, vstate):
    ...
    # v13 Layer I: compute VFE combined OBI to skew Wall Mid fair value.
    # Also returned via vstate for Layer J (voucher BS skew).
    obi_l1_ve = 0.0
    obi_deep_ve = 0.0
    obi_combined_ve = 0.0
    if od_ve and od_ve.buy_orders and od_ve.sell_orders:
        bids_sorted = sorted(od_ve.buy_orders.keys(), reverse=True)
        asks_sorted = sorted(od_ve.sell_orders.keys())
        bv1 = od_ve.buy_orders[bids_sorted[0]] if len(bids_sorted) >= 1 else 0
        av1 = -od_ve.sell_orders[asks_sorted[0]] if len(asks_sorted) >= 1 else 0
        bv2 = od_ve.buy_orders[bids_sorted[1]] if len(bids_sorted) >= 2 else 0
        bv3 = od_ve.buy_orders[bids_sorted[2]] if len(bids_sorted) >= 3 else 0
        av2 = -od_ve.sell_orders[asks_sorted[1]] if len(asks_sorted) >= 2 else 0
        av3 = -od_ve.sell_orders[asks_sorted[2]] if len(asks_sorted) >= 3 else 0
        denom_l1 = bv1 + av1
        if denom_l1 > 0:
            obi_l1_ve = (bv1 - av1) / denom_l1
        bd = bv2 + bv3; ad = av2 + av3
        denom_deep = bd + ad
        if denom_deep > 0:
            obi_deep_ve = (bd - ad) / denom_deep
        obi_combined_ve = obi_l1_ve - 0.5 * obi_deep_ve
    ...
    if od_ve and od_ve.buy_orders and od_ve.sell_orders:
        ...
        if wm is not None:
            ...
            # v13 Layer I: REVERTED — skew dropped −$252 day-2 1k (too noisy
            # at this short timescale; round() flipped FV ±1 on OBI sign).
            fv = round(wm)
    ...
    for K in V_BS_STRIKES:
        ...
        # v13 Layer J: tilt BS FV by VFE OBI × delta (β=0.5)
        fv_bs = v_bs_call(spot, K, T, sigma)
        delta_v = v_bs_delta(spot, K, T, sigma)
        fv_bs = fv_bs + 0.5 * obi_combined_ve * delta_v
    ...
    # ── v12 Layer B + v13 Layer F + Layer K: VEV_4000 MM ────────────────────
    # v13 Layer F: SIZE 5→15 (engine caps fills at market_trade qty).
    # v13 Layer K: inventory dampener — halve same-side size when |pos|>75
    # to mitigate full-day adverse accumulation.
    K4000 = 4000
    ...
    if od_4000 is not None and od_4000.buy_orders and od_4000.sell_orders:
        ...
        if vba - vbb >= 2:  # need at least 2 spread for inside-spread room
            ...
            existing_sell = sum(-o.quantity for o in existing if o.quantity < 0)
            MM_4000_SIZE = 15
            MM_4000_POS_CAP = 50  # back to v12 cap
            DAMPER_THR = 35
            # v13 Layer K: dampen the SAME-SIDE size when leaning that direction
            buy_size = MM_4000_SIZE // 2 if pos_v > DAMPER_THR else MM_4000_SIZE
            sell_size = MM_4000_SIZE // 2 if pos_v < -DAMPER_THR else MM_4000_SIZE
            ...
            if ts_v > 0:
                q = min(ts_v, sell_size)
                if q > 0:
                    orders.setdefault(sym_4000, []).append(Order(sym_4000, vba - 1, -q))

    # ── v13 Layer G: best±1 MM on VEV_4500/5000/5100/5200 (SIZE=15) ─────────
    # Currently $0 on all four (no fills v12 1k). Post inside-spread when
    # spread >= 2; cap ±100 per strike. Modest expected uplift; main goal
    # is full-day robustness.
    LAYER_G_STRIKES = [4500, 5000, 5100, 5200]
    LAYER_G_SIZE = 15
    LAYER_G_POS_CAP = 100
    for K in LAYER_G_STRIKES:
        sym = VOUCHER_SYM[K]
        od_v = state.order_depths.get(sym)
        if od_v is None or not od_v.buy_orders or not od_v.sell_orders: continue
        vbb = max(od_v.buy_orders); vba = min(od_v.sell_orders)
        if vba - vbb < 2: continue  # need inside-spread room
        pos_v = state.position.get(sym, 0)
        existing = orders.get(sym, [])
        existing_buy = sum(o.quantity for o in existing if o.quantity > 0)
        existing_sell = sum(-o.quantity for o in existing if o.quantity < 0)
        tb_v = LAYER_G_POS_CAP - pos_v - existing_buy
        ts_v = LAYER_G_POS_CAP + pos_v - existing_sell
        if tb_v > 0:
            q = min(tb_v, LAYER_G_SIZE)
            orders.setdefault(sym, []).append(Order(sym, vbb + 1, q))
        if ts_v > 0:
            q = min(ts_v, LAYER_G_SIZE)
            orders.setdefault(sym, []).append(Order(sym, vba - 1, -q))

    # ── v12 Layer C + v13 Layer H: Passive OTM bid on VEV_5300/5400/5500 ────
    # v13 Layer H: VEV_5300 → SIZE=15, drop the BS_fair-2 cap (use bb+1).
    #   v12 BS cap pushed bid_px below bb+1 most ticks (deep-bid rarely fills).
    # 5400/5500 stay at SIZE=5 with original logic (no flow in 1k).
    OTM_BID_POS_CAP = 50
    OTM_BID_EDGE = 2  # bid must be at least 2 below BS fair (5400/5500 only)
    for K in VOUCHER_STRIKES_OTM_PASSIVE:
        ...
        if K == 5300:
            # v13 Layer H: unconditional bb+1, SIZE=15
            otm_size = 15
            bid_px = bb + 1
        else:
            otm_size = 5
