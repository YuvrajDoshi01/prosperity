...
# IV mean-reversion (V7.2)
IV_WINDOW = 150               # CMU Physics window
IV_Z_ENTRY = 1.0              # Martin Oravec ±1 bands
IV_Z_EXIT = 0.3               # exit when z back near 0
IV_MIN_HIST = 30              # warm-up ticks
IV_POS_CAP = 100              # max position from IV signals (smaller than 300 limit)
IV_TRADE_SIZE = 30            # per-trigger order size
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # V7.2: IV smile DISABLED — small net loss in BT (-$500 3-day total).
        # Vega-weighted z-score didn't beat spread cost on R3 voucher data.
        # Code retained for future tuning. Enable by uncommenting:
        # for K in STRIKES_IV:
        #     r = self._iv_signal_trade(state, K, spot, T, orders)
        #     if r:
        #         orders.setdefault(SYM[K], []).extend(r)
