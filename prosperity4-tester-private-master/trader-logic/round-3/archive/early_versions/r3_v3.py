"""r3_v3.py — v1 + structural arbitrage + vol-scaled underlying MM.

Changes from v1 (based on 4-agent analysis 2026-04-25):
  F1. Call-spread arbitrage scanner across all 45 strike pairs (bounded by K_hi - K_lo).
  F2. Expanded intrinsic arb to all 10 strikes (v1 had only 4000/4500).
  F3. Vol-scaled HYDROGEL_PACK posting width (rolling 20-tick std, cap ±3) — fixes day-2 weakness.
  F4. IV z-score trading REMOVED (underperformed v1 by $7.7k on 3-day BT).

Spot freshness guard: arb scanners skip if ticks_since_spot_update > 3.

Expected 3-day BT: +30–35k (v1 baseline: +28,013).
"""
import itertools
import json
...
class Trader:
    ...
    def _intrinsic_arb(self, state, strike, spot, existing_orders, deep_itm):
        """Intrinsic-value arbitrage.

        For deep ITM (4000, 4500): both sides use `|mid - intrinsic| > edge`
        (v1 behavior — TV is small enough that this is ~arb in practice).
        For all other strikes: use strict no-arb bounds:
          - Buy if ask < max(spot-K, 0) - edge (lower bound violation)
          - Sell if bid > spot + edge (upper bound violation, extremely rare)
        """
        product = SYM[strike]
        od = state.order_depths.get(product)
        if od is None:
        ...
        result = []
        # BUY side: both paths use lower bound — ask < intrinsic - edge
        for p, v in sorted(od.sell_orders.items()):
            if tb > 0 and p < intrinsic - INTRINSIC_EDGE:
                q = min(tb, -v, INTRINSIC_QTY)
                result.append(Order(product, p, q))
                tb -= q
        # SELL side depends on strike class
        sell_threshold = intrinsic + INTRINSIC_EDGE if deep_itm else spot + INTRINSIC_EDGE
        for p, v in sorted(od.buy_orders.items(), reverse=True):
            if ts > 0 and p > sell_threshold:
    ...
    def run(self, state: TradingState):
        ...
        # F2: Intrinsic arb — deep ITM (4000/4500) uses loose TV-assumption rule,
        # other strikes use strict no-arb bounds (rarely triggers but safe).
