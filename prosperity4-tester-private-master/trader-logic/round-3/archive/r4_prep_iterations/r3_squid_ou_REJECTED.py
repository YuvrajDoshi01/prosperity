"""r3_v19_squid_ou.py — v17 + Squid-Ink-style OU positioning lean on HP.

Adds continuous OU lean to HYDROGEL_PACK passive MM. Calibrated from
P3 2025 SQUID_INK (half-life 307.4) and HP (half-life 293.8) — same OU profile.

Mechanism:
  - Compute zscore z = (mid - HP_OU_MEAN) / HP_OU_STD
  - Target position lean_target = -clip(z, -2, 2) * SQUID_LEAN_SCALE
  - When mid > μ + 1σ: lean_target ≈ -100 (mild short bias)
  - When mid < μ - 1σ: lean_target ≈ +100 (mild long bias)
  - Bias propagates via existing v11 INV_ADJ_COEFF mechanism

Difference from v11/v17: v17 had `lean_target = 0.0` (never set). v19 makes
this dynamic based on OU zscore. The discrete triggers (spread=17 SHORT,
MR LONG at 9950) are PRESERVED — this just adds a continuous lean BETWEEN
those triggers.
...
class HydrogelParams:
    ...
    # ── Squid-Ink-style OU positioning lean (v19) ─────────────────────────
    # Calibrated empirically: HP OU half-life 293.8, std=31.62, mean=9989.40.
    # When mid deviates from mean, lean position toward reversion.
    SQUID_OU_MEAN     = 9989.40
    SQUID_OU_STD      = 31.62
    SQUID_LEAN_SCALE  = 25.0     # at z=±1, lean_target = ∓25; clamped to ±50
    SQUID_LEAN_CAP    = 50.0     # maximum |lean_target|
    SQUID_LEAN_ENABLE = True     # set False to disable and fall back to v17 behavior
...
def run_hydrogel(state, hstate):
    ...
    # ─────────────────────────────────────────────────────────────────────
    # v19 Squid OU lean: set hstate.lean_target dynamically from zscore
    if p.SQUID_LEAN_ENABLE:
        z = (mid - p.SQUID_OU_MEAN) / p.SQUID_OU_STD
        z_clipped = _clip(z, -2.0, 2.0)
        # When mid > mean (z>0), lean SHORT (negative target)
        target = _clip(-z_clipped * p.SQUID_LEAN_SCALE,
                       -p.SQUID_LEAN_CAP, p.SQUID_LEAN_CAP)
        hstate.lean_target = target
