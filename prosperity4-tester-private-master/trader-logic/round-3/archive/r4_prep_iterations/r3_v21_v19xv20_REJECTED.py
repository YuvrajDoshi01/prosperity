"""r3_v21.py — v19 HP (S7+FLIP) + v20 voucher additions. SLIGHT REGRESSION vs v19.

Attempted to combine two independent improvements:
  1. v19's HP block: teammate 406026's S7-bottom cover + flip-long-180 mechanic
     (gives v19 BT $86,194 10k 3-day vs v17 $56,328)
  2. v20's voucher block: BS-anchored ATM MM (5100-5400), Phase 4.1 deep-ITM
     theta carry, smile R² gate, empirical δ gate, portfolio delta hedge

Verified BT (R3 historical):
  1k-tick day 2: $15,717   (vs v19 $15,760 — REGRESSION −$43)
  10k 3-day:    $86,162   (vs v19 $86,194 — REGRESSION −$32)

Per-day 10k vs v19 baseline:
  Day 0: 31,392 vs 31,964 = −$572 (v20 ATM MM hurts)
  Day 1: 18,054 vs 17,952 = +$102
  Day 2: 36,715 vs 36,278 = +$437 (v20 ATM MM helps)

DIAGNOSIS: v20's BS-anchored ATM MM (edge=1.5) replaces v19's spread-anchored
MM (post at vbb+1/vba-1). On day 2 (rising VFE, persistent ATM drift), the
BS-anchored approach is better. On day 0 (different IV regime), it's worse.
The v19 spread-anchored MM is regime-agnostic; v20's BS-anchored requires
correctly-calibrated sigma which we hardcoded.

VERDICT: Don't ship v21. v19 is the better all-rounder. v21 is preserved
as a documentation of the combination attempt and a starting point if
day-type-conditional voucher MM (use BS on trending days, spread on flat) is
explored later.

Inherits r3_v19 (HP) + r3_v20 (vouchers) documentation below.
=====================================================================
...
class HydrogelParams:
    ...
    # v19 Exit: spread==7 AND mid <= bottom-8% of last 500-tick window
    #          (NEW from teammate 406026); fallback at TIMEOUT if S7 never fires
    ENTRY_MID_MIN          = 10010   # only short if elevated
    COVER_TARGET           = 0       # disabled (v14j)
    TIMEOUT                = 1000    # v19 fallback: force-cover after 1000 ticks if S7 never fires
    S7_WINDOW              = 500     # rolling window for percentile threshold
    S7_PERCENTILE          = 0.08    # bottom 8% of window
    S7_COVER_SPREAD        = 7       # require this spread for cover signal
    S7_FLIP_TARGET         = 180     # post-cover long position size
    S7_FLIP_EXIT_BAND      = 8       # MR exit when |mid − GLOBAL_MEAN| <= this
...
class HydrogelState:
    def __init__(self):
        ...
        # v19 cover+flip tracking: True while buying back to +S7_FLIP_TARGET
...
def run_hydrogel(state, hstate):
    ...
    # ─────────────────────────────────────────────────────────────────────
    # LAYER 0A: SPREAD=17 GIGA SHORT (v19: teammate 406026's S7+FLIP exit)
    # Entry: spread==17 AND mid > ENTRY_MID_MIN  →  short to -200
    # Exit:  spread==7  AND mid <= 8th-pctl of last 500 mids  →  cover, then
    #        flip long to +S7_FLIP_TARGET (180), hand off to MR exit at mean.
    # Fallback: force-cover at TIMEOUT=1000 ticks if S7 never fires.
    # ─────────────────────────────────────────────────────────────────────

    # Exit check: S7 bottom or fallback timeout
    if hstate.s17_entry_row is not None and position < 0:
        ...
        if window_ready:
            ...
            bot_thresh = sorted_window[int(len(sorted_window) * p.S7_PERCENTILE)]
            s7_bottom = (spread == p.S7_COVER_SPREAD and mid <= bot_thresh)
        else:
            bot_thresh = None
            s7_bottom = False
        timeout_hit = rows_held >= p.TIMEOUT

        if s7_bottom:
            # S7 bottom — start cover+flip phase, switch to lifting asks
            hstate.s7_covering = True
            hstate.s17_entry_row = None
            hstate.s17_entry_mid = None
            logger.print(
                f"S17 COVER START [S7_8PCT]: mid={mid:.1f} bot_thresh={bot_thresh:.1f} "
                f"pos={position} rows_held={rows_held}"
            )
        elif timeout_hit:
            # Fallback: force-cover via best_ask (no flip)
            cover_qty = -position
            orders.append(Order(P, best_ask, cover_qty))
            logger.print(
                f"S17 COVER [TIMEOUT]: qty={cover_qty} px={best_ask} mid={mid:.1f} "
                f"rows_held={rows_held}"
            )
            hstate.s17_entry_row = None
            hstate.s17_entry_mid = None
            return orders, hstate

        # While in S17 short (and not in flip yet): keep building to -200
        if not hstate.s7_covering:
            headroom = pos_lim + position
            if headroom > 0:
                orders.append(Order(P, best_bid, -headroom))
            return orders, hstate

    # Cover+flip phase: lift ask each tick until pos >= S7_FLIP_TARGET
    if hstate.s7_covering:
        remaining = p.S7_FLIP_TARGET - position
        if remaining > 0:
            orders.append(Order(P, best_ask, remaining))
            logger.print(f"S7 FLIP BUILD: buy={remaining} px={best_ask} mid={mid:.1f} pos={position}")
        if position >= p.S7_FLIP_TARGET:
            hstate.s7_covering = False
            hstate.mr_side = 1   # hand off to MR exit at mean
            logger.print(f"S7 FLIP COMPLETE: pos={position}")
        return orders, hstate
...
HEDGE_ENABLE     = False   # disabled in v21 — interferes with v19's HP positioning
                           # (which sometimes goes long VFE-correlated via FLIP)
