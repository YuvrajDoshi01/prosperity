"""r3_v16.py — v15 + empirical-vs-BS delta divergence gate (Phase 4.3, alpha A9).

Tracks rolling 20-tick regression of voucher Δmid on VFE Δmid (empirical delta β).
Compares to BS delta. When |β - bs_delta| > DELTA_DIVERGENCE_THRESH, the BS
price model is unreliable for that strike — SKIP BS taking on it this tick.

Mechanism:
  - Each tick: append (spot_mid_chg, voucher_mid_chg) to per-strike deque
  - Maintain rolling window of 20 changes
  - β = sum(s*v) / sum(s²) (covariance / variance of spot changes)
  - bs_delta_pred = N(d1) where d1 from BS formula
  - If |β - bs_delta_pred| > 0.15: skip BS take this tick

Baseline (v15): 1k-tick day 2 = $12,340, 10k 3-day = $56,328.
Expected v16: net positive on 10k 3-day (~$200-1000) via avoiding bad BS fills.
