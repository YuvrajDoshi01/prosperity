"""r3_v14.py — v12 baseline + tuned HP exit (sweep winner v14h: COVER=0, TIMEOUT=1000).

Sweep on day-2 1k HP exit logic — v12 baseline used COVER_TARGET=9998, TIMEOUT=200.
v14h disables price cover (COVER_TARGET=0) and extends TIMEOUT to 1000, holding the
short across the entire 1k-tick window so we capture more of the downtrend.

Day-2 1k BT: $12,826 (vs v12 $12,410, +$416)
3-day 10k BT: $57,362 (vs v12 $56,654, +$708)
Expected website (BT × 0.99): ~$12,698

Note: v14e (COVER=9998, TIMEOUT=500) scored 3-day $62,308 but day-2 1k identical to v12
(cover hit before 500-tick timeout); see notes/alpha_hunt_2026-04-25/v14_sweep_results.md.
...
class HydrogelParams:
    ...
    COVER_TARGET   = 0       # v14h: disabled, no early cover
    TIMEOUT        = 300     # v14i: hold to ~end of 1k window
