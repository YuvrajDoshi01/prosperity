...
class VoucherState:
    def __init__(self):
        self.iv_history = {k: [] for k in VOUCHER_STRIKES_ALL}
        self.last_spot = None
        self.spot_age = 0
        ...
        self.vfe_prices: List[float] = [] # NEW

    def to_dict(self):
        return {
            "ivh": {str(k): [round(x, 5) for x in v[-50:]] for k, v in self.iv_history.items()},
            "ls": self.last_spot,
            "sa": self.spot_age,
...
def run_vouchers(state, vstate):
    ...
    # ── v22 Vol Arb Layer ───────────────────────────────────────────────────────
    # Trade IV mean reversion and RV vs IV spread
    ANNUAL_TICKS = 2500000
    VOL_ARB_Z_THRESH = 1.5
    VOL_ARB_RV_THRESH = 0.03
    VOL_ARB_SIZE = 15
    VOL_ARB_POS_CAP = 100
    ...
    if len(vstate.vfe_prices) > 10:
        ...
        rv = _std(rets) * math.sqrt(ANNUAL_TICKS) if _std(rets) else 0.0
    ...
    for K in [4500, 5000, 5100, 5200, 5300, 5400, 5500]:
        ...
        current_iv = hist[-1]
        roll_mean = _mean(hist)
        roll_std = _std(hist)
        
        if roll_std is None or roll_std == 0: continue
            
        zscore = (current_iv - roll_mean) / roll_std
        vol_spread = rv - current_iv
        
        vol_alpha = 0
        if zscore < -VOL_ARB_Z_THRESH: vol_alpha += 1
        elif zscore > VOL_ARB_Z_THRESH: vol_alpha -= 1
            
        if vol_spread > VOL_ARB_RV_THRESH: vol_alpha += 1
        elif vol_spread < -VOL_ARB_RV_THRESH: vol_alpha -= 1
        ...
        vba = min(od_v.sell_orders)
        vbb = max(od_v.buy_orders)
        
        new_orders = []
        if vol_alpha > 0:
            room = min(300 - pos_v - already_buy, VOL_ARB_POS_CAP - pos_v - already_buy)
            q = min(VOL_ARB_SIZE, room)
            if q > 0:
                fv_cheap = v_bs_call(spot, K, T, current_iv + 0.05)
                if vba <= fv_cheap:
                    new_orders.append(Order(sym, vba, q))
                    
        elif vol_alpha < 0:
            room = min(300 + pos_v - already_sell, VOL_ARB_POS_CAP + pos_v - already_sell)
            q = min(VOL_ARB_SIZE, room)
            if q > 0:
                fv_exp = v_bs_call(spot, K, T, max(0.01, current_iv - 0.05))
                if vbb >= fv_exp:
                    new_orders.append(Order(sym, vbb, -q))
