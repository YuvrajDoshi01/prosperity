...
class HydrogelParams:
    ...
    # Entry: spread==17 AND mid > ENTRY_MID_MIN -> GIGA short
    # Exit:  spread==7  AND mid < S7_COVER_MAX  -> cover everything
    #        OR timeout as safety net
    ENTRY_MID_MIN  = 10010   # only short if price elevated

    # ── Spread=7 COVER signal ─────────────────────────────────────────────
    # Cover when spread=7 AND mid < bottom 20% of last S7_WINDOW prices.
    # Day 0: 72% correct at h20, 29 signals, avg +8t. No hardcoded threshold.
    S7_WINDOW      = 500     # rolling window for bottom 20% calculation
...
class HydrogelState:
    def __init__(self):
        ...
        # v8 cover+flip tracking: True while buying back to +60
...
def run_hydrogel(state, hstate):
    ...
    # Check exit: cover on spread=7 AND mid < bottom 20% of last 500 prices
    if hstate.s17_entry_row is not None and position < 0:
        # Require full 500-tick window before firing — early window is
        # unreliable since all prices are from the opening peak.
        window_ready = len(hstate.mid_buf_500) >= p.S7_WINDOW
        if window_ready:
            sorted_window = sorted(hstate.mid_buf_500[-p.S7_WINDOW:])
            bot10_thresh  = sorted_window[int(len(sorted_window) * 0.08)]
            s7_bottom     = (spread == 7 and mid <= bot10_thresh)
        else:
            bot10_thresh  = None
            s7_bottom     = False
        should_cover = s7_bottom
        if should_cover:
            # S7 bottom fired — start cover+flip phase
            hstate.s7_covering  = True
            hstate.s17_entry_row = None
            hstate.s17_entry_mid = None
            logger.print(f"S17 COVER START [S7_8PCT]: mid={mid:.1f} bot8={bot10_thresh:.1f} pos={position}")
    ...
    # Cover+flip phase: keep lifting ask every tick until position reaches +60
    FLIP_TARGET = 180
    if hstate.s7_covering:
        ...
        if position >= FLIP_TARGET:
            ...
            hstate.mr_side     = 1  # hand off to MR exit to close when near mean
