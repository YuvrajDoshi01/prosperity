"""hp_inv_aware_mm.py - v22 HP with inventory-aware passive MM.

Modification: when position > INV_SUPPRESS_THRESH during passive MM,
suppress bid posting (only post ask to reduce inventory). When position
< -INV_SUPPRESS_THRESH, suppress ask. This should reduce the holding
MTM bleed without eliminating spread capture.

Sweep INV_SUPPRESS_THRESH across {50, 80, 100, 120, 150, 200}.

Usage:
  PYTHONPATH=prosperity4bt python -m prosperity4bt trader-logic/round-3/notes/hp_inv_aware_mm.py 3-2 --ticks 10000 --no-progress --no-out
"""
...
# TUNABLE: suppress same-side quotes when |position| > threshold
INV_SUPPRESS_THRESH = 80  # default; can be overridden
...
def run_hydrogel(state, hstate):
    """v22 HP with inventory-aware passive MM suppression."""
    ...
    # Passive MM with inventory-aware suppression
    ...
    # When position is heavily long, suppress bids (stop adding longs).
    # When position is heavily short, suppress asks (stop adding shorts).
    if position > INV_SUPPRESS_THRESH:
        bid_qty = 0  # Don't add more longs
    elif position < -INV_SUPPRESS_THRESH:
        ask_qty = 0  # Don't add more shorts
...
# ---- Voucher/VFE (exact copy, abbreviated for clarity) ----
# Full copy from v22 below...
...
class VoucherState:
    ...
    def to_dict(self):
        return {"ivh": {str(k): v[-V_IV_ADAPT_WINDOW:] for k, v in self.iv_history.items()}, "ls": self.last_spot, "sa": self.spot_age, "p_ap": self.prev_ve_ap1, "p_bp": self.prev_ve_bp1}
    @staticmethod
    def from_dict(d):
        s = VoucherState()
...
def v_get_adaptive_sigma(state, vstate):
    ...
    for k in V_BS_STRIKES:
        ...
        if len(hist) >= V_IV_ADAPT_MIN_HIST: all_recent.extend(hist[-V_IV_ADAPT_WINDOW:])
    if len(all_recent) < V_IV_ADAPT_MIN_HIST: return V_BS_SIGMA_DEFAULT
...
def run_vouchers(state, vstate):
    ...
    if od_ve and od_ve.buy_orders and od_ve.sell_orders:
        ...
        if vstate.prev_ve_ap1 is not None and vstate.prev_ve_bp1 is not None:
            if spread_ve == 2 and ba_ve < vstate.prev_ve_ap1:
                ...
                if q > 0:
                    px = int(ba_ve - 1)
                    if px > bb_ve and px < ba_ve: ve_layer_e.append(Order(VEVE_SYM, px, q)); layer_e_buy = q
            elif spread_ve == 3 and bb_ve > vstate.prev_ve_bp1:
                room_sell = E_POS_CAP + pos_ve; q = min(E_SIZE, room_sell)
                if q > 0:
                    px = int(bb_ve + 1)
                    if px > bb_ve and px < ba_ve: ve_layer_e.append(Order(VEVE_SYM, px, -q)); layer_e_sell = q
        vstate.prev_ve_ap1 = ba_ve; vstate.prev_ve_bp1 = bb_ve
    if od_ve and od_ve.buy_orders and od_ve.sell_orders:
        wm = v_wall_mid(od_ve)
        if wm is not None:
            bb = max(od_ve.buy_orders); ba = min(od_ve.sell_orders); pos = state.position.get(VEVE_SYM, 0); fv = round(wm)
            tb = 200 - pos - layer_e_buy; ts = 200 + pos - layer_e_sell; half = 100
            mbp = fv - 1 if pos > half else fv; msp = fv + 1 if pos < -half else fv
            ve_orders = list(ve_layer_e)
            for p, v in sorted(od_ve.sell_orders.items()):
                if tb > 0 and p <= mbp: q = min(tb, -v); ve_orders.append(Order(VEVE_SYM, p, q)); tb -= q
            for p, v in sorted(od_ve.buy_orders.items(), reverse=True):
                if ts > 0 and p >= msp: q = min(ts, v); ve_orders.append(Order(VEVE_SYM, p, -q)); ts -= q
            if tb > 0: ve_orders.append(Order(VEVE_SYM, min(fv - V_POST_SLACK_VE, bb + 1), tb))
            if ts > 0: ve_orders.append(Order(VEVE_SYM, max(fv + V_POST_SLACK_VE, ba - 1), -ts))
            orders[VEVE_SYM] = ve_orders
        elif ve_layer_e: orders[VEVE_SYM] = list(ve_layer_e)
    spot = v_plain_mid(state.order_depths.get(VEVE_SYM))
    if spot is None:
        if vstate.last_spot is not None and vstate.spot_age < 20: spot = vstate.last_spot; vstate.spot_age += 1
        else: return orders
    else: vstate.last_spot = spot; vstate.spot_age = 0
    if vstate.spot_age > 3: return orders
    T = max(V_TTE_DAYS_AT_START - timestamp / 1_000_000.0, 0.01) / V_TTE_YEAR
    for K in V_BS_STRIKES:
        sym = VOUCHER_SYM[K]; od_v = state.order_depths.get(sym)
        ...
        if iv is not None:
            hist = vstate.iv_history.setdefault(K, []); hist.append(iv)
    ...
    for K in VOUCHER_STRIKES_ALL:
        ...
        existing = orders.get(sym, []); already_buy = sum(o.quantity for o in existing if o.quantity > 0); already_sell = sum(-o.quantity for o in existing if o.quantity < 0)
        pos_v = state.position.get(sym, 0); tb = 300 - pos_v - already_buy; ts = 300 + pos_v - already_sell
        intrinsic = max(spot - K, 0.0); deep_itm = K in VOUCHER_STRIKES_DEEP_ITM; new_orders = []
        for p, v in sorted(od_v.sell_orders.items()):
            if tb > 0 and p < intrinsic - V_INTRINSIC_EDGE: q = min(tb, -v, V_INTRINSIC_QTY); new_orders.append(Order(sym, p, q)); tb -= q
        ...
        for p, v in sorted(od_v.buy_orders.items(), reverse=True):
            if ts > 0 and p > sell_thr: q = min(ts, v, V_INTRINSIC_QTY); new_orders.append(Order(sym, p, -q)); ts -= q
        if new_orders: orders.setdefault(sym, []).extend(new_orders)
    for K in V_BS_STRIKES:
        sym = VOUCHER_SYM[K]; od_v = state.order_depths.get(sym)
        if od_v is None or not od_v.buy_orders or not od_v.sell_orders: continue
        pos_v = state.position.get(sym, 0); existing = orders.get(sym, [])
        already_buy = sum(o.quantity for o in existing if o.quantity > 0); already_sell = sum(-o.quantity for o in existing if o.quantity < 0)
        cur_pos_buy = pos_v + already_buy; cur_pos_sell = pos_v - already_sell
        fv_bs = v_bs_call(spot, K, T, sigma); buy_thr = fv_bs - V_BS_EDGE; sell_thr_bs = fv_bs + V_BS_EDGE; new_orders = []
        for px in sorted(od_v.sell_orders.keys()):
            if px > buy_thr: break
            avail = -od_v.sell_orders[px]; room = min(300 - cur_pos_buy, V_BS_POS_CAP - cur_pos_buy); size = min(V_BS_TRADE_SIZE, avail, room)
            if size > 0: new_orders.append(Order(sym, px, +size)); cur_pos_buy += size
        for px in sorted(od_v.buy_orders.keys(), reverse=True):
            if px < sell_thr_bs: break
            avail = od_v.buy_orders[px]; room = min(300 + cur_pos_sell, V_BS_POS_CAP + cur_pos_sell); size = min(V_BS_TRADE_SIZE, avail, room)
            if size > 0: new_orders.append(Order(sym, px, -size)); cur_pos_sell -= size
        if new_orders: orders.setdefault(sym, []).extend(new_orders)
    committed_buy = {sym: 0 for sym in VOUCHER_SYM.values()}; committed_sell = {sym: 0 for sym in VOUCHER_SYM.values()}
    for sym, ord_list in orders.items():
        if sym in committed_buy: committed_buy[sym] = sum(o.quantity for o in ord_list if o.quantity > 0); committed_sell[sym] = sum(-o.quantity for o in ord_list if o.quantity < 0)
    for K_lo, K_hi in itertools.combinations(VOUCHER_STRIKES_ALL, 2):
        ...
        if od_lo.sell_orders and od_hi.buy_orders:
            ...
            if ask_lo - bid_hi < 0:
                ...
                room_buy_lo = 300 - pos_lo - committed_buy[sym_lo]; room_sell_hi = 300 + pos_hi - committed_sell[sym_hi]
                size = min(V_ARB_SIZE, ask_lo_vol, bid_hi_vol, room_buy_lo, room_sell_hi)
                if size > 0:
                    orders.setdefault(sym_lo, []).append(Order(sym_lo, ask_lo, +size)); orders.setdefault(sym_hi, []).append(Order(sym_hi, bid_hi, -size))
    ...
    for K in VOUCHER_STRIKES_TRADEABLE:
        ...
        pos_v = state.position.get(sym, 0); existing = orders.get(sym, [])
        existing_buy = sum(o.quantity for o in existing if o.quantity > 0); existing_sell = sum(-o.quantity for o in existing if o.quantity < 0)
        tb_v = 300 - pos_v - existing_buy; ts_v = 300 + pos_v - existing_sell
        if tb_v > 0: orders.setdefault(sym, []).append(Order(sym, vbb + 1, min(tb_v, V_VOUCHER_MM_SIZE)))
        if ts_v > 0: orders.setdefault(sym, []).append(Order(sym, vba - 1, -min(ts_v, V_VOUCHER_MM_SIZE)))
    DEEP_ITM_MM_SIZE = 30; DEEP_ITM_POS_CAP = 100; DEEP_ITM_BID_OFFSET = 1; DEEP_ITM_ASK_OFFSET = 1
    for K in VOUCHER_STRIKES_DEEP_ITM:
        sym = VOUCHER_SYM[K]; od_v = state.order_depths.get(sym)
        if od_v is None or not od_v.buy_orders or not od_v.sell_orders: continue
        intrinsic = max(spot - K, 0.0)
        if intrinsic <= 0: continue
        vbb = max(od_v.buy_orders); vba = min(od_v.sell_orders); pos_v = state.position.get(sym, 0)
        existing = orders.get(sym, []); existing_buy = sum(o.quantity for o in existing if o.quantity > 0); existing_sell = sum(-o.quantity for o in existing if o.quantity < 0)
        room_buy = DEEP_ITM_POS_CAP - pos_v - existing_buy
        if room_buy > 0:
            target_bid = int(round(intrinsic - DEEP_ITM_BID_OFFSET)); bid_px = min(target_bid, vbb + 1); bid_px = max(1, bid_px)
            if bid_px <= vba - 1: qty = min(DEEP_ITM_MM_SIZE, room_buy); orders.setdefault(sym, []).append(Order(sym, bid_px, qty))
        room_sell = DEEP_ITM_POS_CAP + pos_v - existing_sell
        if room_sell > 0:
            target_ask = int(round(intrinsic + DEEP_ITM_ASK_OFFSET)); ask_px = max(target_ask, vba - 1)
            if ask_px >= vbb + 1: qty = min(DEEP_ITM_MM_SIZE, room_sell); orders.setdefault(sym, []).append(Order(sym, ask_px, -qty))
    ...
    for K in VOUCHER_STRIKES_OTM_PASSIVE:
        ...
        fv_bs = v_bs_call(spot, K, T, sigma); bs_cap = math.floor(fv_bs - OTM_BID_EDGE); bid_px = min(bb + 1, bs_cap)
        if bid_px <= 0: continue
        if bid_px >= ba: continue
        pos_v = state.position.get(sym, 0); existing = orders.get(sym, []); existing_buy = sum(o.quantity for o in existing if o.quantity > 0)
        tb_v = OTM_BID_POS_CAP - pos_v - existing_buy
        if tb_v > 0: q = min(tb_v, OTM_BID_SIZE); orders.setdefault(sym, []).append(Order(sym, bid_px, q))
    return orders


class Trader:
    def run(self, state: TradingState) -> Tuple[Dict[Symbol, List[Order]], int, str]:
        orders = {p: [] for p in state.order_depths}; conversions = 0; trader_data = state.traderData or ""
        try: raw = json.loads(trader_data) if trader_data else {}
        except Exception: raw = {}
        hstate = HydrogelState.from_dict(raw.get("hg", {}))
        hydrogel_orders, hstate = run_hydrogel(state, hstate)
        orders[Product.HYDROGEL_PACK] = hydrogel_orders; raw["hg"] = hstate.to_dict()
