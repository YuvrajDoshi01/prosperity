"""r3_v24.py — v23 + OTM Layer C tuning (SIZE 5->15, POS_CAP 50->100, EDGE 2->1).

Built on r3_v23.py ($40,268 day-2 10k default / $42,389 imc).

v24 change: OTM passive bids on VEV_5300/5400/5500 tuned for more cascade-dump
capture. Larger bid size catches more of the bot's sell-dump fills; tighter edge
(BS_fair-1 instead of BS_fair-2) positions bids closer to fair, increasing fill
probability; higher position cap allows more accumulation.

Sweep results (vs v23 baseline):
  SIZE=15 POS_CAP=100 EDGE=2: +$248 day-2 10k, +$287 3-day (VEV_5400 only)
  SIZE=15 POS_CAP=100 EDGE=1: +$440 day-2 10k, +$456 3-day, +$647 imc day-2
  SIZE=15 POS_CAP=100 EDGE=1 + VEV_5200 added: $0 (conflicts with existing MM)
  SIZE=15 POS_CAP=100 EDGE=1 + OTM 2-way (ask exits): $0 (no buyer flow)

Additional hypotheses REJECTED in this session:
  - VEV_4500 ladder MM (tighter offsets [7.5,7,6,5]): -$266 imc day-2 (adverse sel)
  - HP wall_mid FV for passive MM: $0 (cycle dominates, passive MM rarely fires)
  - HP FLIP_TIMEOUT 3000 (longer hold): -$424 3-day (day 0 regresses)
  - HP FLIP_TIMEOUT 1000 (shorter hold): +$292 3-day (within noise, inconsistent)
  - VFE V_POST_SLACK_VE=0 (tighter passive): -$139 imc (adverse selection)
  - VFE half=60 (earlier skew): -$954 day-2 (fewer fills)
  - VFE msp=fv+1 always (buy bias): -$2,789 day-2 (miss profitable sells)
  - BS_EDGE=7 (tighter BS take): -$4,399 3-day (catastrophic adverse selection)
  - BS_EDGE=15 (wider BS take): -$415 3-day (miss profitable takes)
  - VEV_4000 5-rung ladder [10.5,10,9,8,7] size=7: -$66 imc 3-day (smaller per-rung)
  - Voucher MM size 30 (from 20): $0 (taker sizes already <20)

BT results:
  Default day-2 10k:  $40,708  (+$440 vs v23, predicted live $39,934 via ×0.981)
  Default 3-day 10k:  $105,021 (+$456 vs v23)
  imc day-2 10k:      $43,036  (+$647 vs v23, predicted live $43,251 via ×1.005)
  imc 3-day 10k:      $96,826  (+$862 vs v23)

v23 docstring (preserved for context):
======================================================================
v23: v22 + VEV_4000 ladder MM. v22: v20 (SD's S17+S7+FLIP) + HOLD_FLIP@10020.
HP layers: S17 GIGA SHORT -> S7-bottom cover -> FLIP-to-+200 -> HOLD_FLIP -> passive MM.
VFE/voucher: Wall-Mid MM, BS taking (edge=10), intrinsic arb, deep-ITM theta MM,
OTM passive bids, Layer E spread-state lift.
"""
