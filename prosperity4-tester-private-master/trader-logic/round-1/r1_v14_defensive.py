...
"""
r1_v14_defensive — r1_v12_defensive + accumulation-side suppression under stress.

v13 tried forced-dump (spread-cross when trapped) and lost −21,715 synthetic vs
v12 because dumps flip pos from +60 to −15, standard make logic re-posts both
sides, toxic sell flow hits the bid again, cycle repeats. The root cause of
"trap at +80" is not the inability to exit — it's continuous accumulation from
adverse taker flow on the passive bid.

v14 addresses this directly: when `crash_mode` is active AND `|pos| >= 60`,
suppress the quote on the ACCUMULATION side. Specifically:
  - pos >= +60: skip bid posting (prevent more toxic buys), keep ask (exit path)
  - pos <= -60: skip ask posting (prevent more toxic sells), keep bid

This breaks the accumulation cycle without forcing the cycle-loss that killed
v13. We neither buy nor sell aggressively — we freeze our bid on the side where
we're already long and let the market move around our resting ask.

Trade-offs:
  + On sustained crashes, we stop accumulating past the DUMP_THRESHOLD.
  + On mean-reverting crashes, our ask catches the rebound on the way back.
  + Zero overhead in non-crash regimes (crash_mode gate).
  − On CRASH_DEEP (no recovery), we stay stuck at pos=+60 with unrealized loss,
    but v12 would have accumulated further to +80 so v14 stops the bleeding.

Kept from v12: blind-eye reset fix, sweep-optimal (MAX_CONCESSION=4,
CRASH_THRESHOLD=15), cur_mid crash trigger, IPR one-sided penny-improve drop,
cubic skew shape, all correctness fixes from earlier versions.
"""
