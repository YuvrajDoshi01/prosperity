...
"""
r1_v6b — Round 1 Strategy (r1_v4 + IPR Wall-Mid FV)

Replaces IPR's simple mid with wall-mid (deepest-bid + deepest-ask) / 2.
Nancy's 10.7k submission uses wall-mid on IPR — worth testing in isolation.

IPR: wall-mid + drift_bias=5 (instead of simple mid + drift_bias).
ACO: unchanged from r1_v4 (fixed FV=10000).
"""

LIMIT = 80
SOFT_LIMIT = 40

# IPR (r1_v4 + wall-mid FV)
...
class Trader:
    ...
    def _ipr(self, state):
        ...
        if has_bids and has_asks:
            # WALL-MID instead of simple mid
            fv_base = _wall_mid(book, (best_bid + best_ask) * 0.5)
            fv_int = round(fv_base + IPR_DRIFT_BIAS)
