...
"""
r1_chess_fader — "Forced Sequence" Strategy
...
IPR_LIMIT = 80
IPR_DRIFT_BIAS = 5.0          # FV shift: deterministic +100/1k uptrend
IPR_BUY_SLACK = 2             # Take asks up to FV+2
IPR_SELL_SLACK = 3            # Only take bids at FV+3 or above (long bias)
...
class Trader:
    def __init__(self):
        self.ipr_mids = []       # Rolling mid history for AC(1) detection
        self.aco_liq = []        # ACO liquidation window
    ...
    def run(self, state: TradingState):
        ...
        if IPR in state.order_depths:
            ...
            if has_bids or has_asks:
                ...
                if has_bids and has_asks:
                    ...
                    fv = mp + obi * IPR_OBI_SHIFT + IPR_DRIFT_BIAS
                    fv_int = round(fv)
        ...
        return result, conversions, json.dumps(
            {"m": self.ipr_mids, "l": self.aco_liq},
            separators=(",", ":")
        )
