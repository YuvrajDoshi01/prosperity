...
"""
probe4_state_logger — Round 1 Strategy (website: 10,444)
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        result = {}
        conversions = 0
        # LOG: capture all available state data
        import sys
        if state.timestamp <= 500:
            obs_str = ''
            if hasattr(state, 'observations') and state.observations:
                obs = state.observations
                obs_str += f'plain={obs.plainValueObservations} '
                if obs.conversionObservations:
                    for prod, co in obs.conversionObservations.items():
                        obs_str += f'conv[{prod}]=(bid={co.bidPrice},ask={co.askPrice},fees={co.transportFees},exp={co.exportTariff},imp={co.importTariff},sugar={co.sugarPrice},sun={co.sunlightIndex}) '
                else:
                    obs_str += 'conv=EMPTY '
            else:
                obs_str = 'NO_OBS '
            
            listings_str = str({k: (v.symbol, v.product, v.denomination) for k,v in state.listings.items()}) if state.listings else 'NO_LISTINGS'
            
            print(f'STATE[{state.timestamp}]: obs={obs_str} listings={listings_str}')
