...
Phase A BT (r4_v4_voucher_obi precursor):
  1k day 3 probe: $5,586 (+$226 vs v3)
  10k 3-day default: $133,730 (+$3,160 vs v3)
  10k 3-day imc: $124,284 (+$3,282 vs v3)
  Day 1/2/3 individual gains: +$840 / +$1,653 / +$667 (default 10k)

STRICTLY PARETO-DOMINATES v3 and v4 on every test window.
...
class VoucherState:
    def __init__(self):
        ...
        # v6 Mark 22S → Mark 55S sequence
        self.m22_sell_vfe_ts: List[int] = []  # last Mark 22 VFE sell timestamps
        self.seq_short_entry: Optional[float] = None
        self.seq_short_entry_ts: Optional[int] = None
    ...
    def to_dict(self):
        return {
            ...
            "m22ts": getattr(self, "m22_sell_vfe_ts", []),
            "seq_e": getattr(self, "seq_short_entry", None),
            "seq_ets": getattr(self, "seq_short_entry_ts", None),
        }
    ...
    def from_dict(d):
        ...
        s.m22_sell_vfe_ts = d.get("m22ts", [])
        s.seq_short_entry = d.get("seq_e")
        s.seq_short_entry_ts = d.get("seq_ets")
        return s


# ── v6: Mark 22S → Mark 55S sequence on VFE → SHORT ──────────────────────────
# n=181 events (3-day), t=3.17, fwd=+2.07 with sgn=-1 → mid drops -2.07/100ticks.
M22_SEQ_WINDOW_TS = 10000   # 100 ticks lookback for Mark 22 sell
M22_SEQ_HOLD_TS = 10000     # 100 ticks max hold
M22_SEQ_SIZE = 50           # modest short size
M22_SEQ_TP_PER_SHARE = 1.5  # cover at +$1.5/share profit
M22_SEQ_SL_PER_SHARE = 3.0  # cover at -$3/share loss


def run_mark_seq_short(state, vstate):
    """Returns (orders, sell_qty) for Mark 22S→Mark 55S short layer."""
    P = VEVE_SYM
    ...
    # Detect Mark 22 / Mark 55 sells in market_trades
    new_m22_sell = False
    new_m55_sell = False
    for t in state.market_trades.get(P, []):
        if t.seller == "Mark 22":
            new_m22_sell = True
        if t.seller == "Mark 55":
            new_m55_sell = True

    # Update Mark 22 sell timestamp queue
    if new_m22_sell:
        vstate.m22_sell_vfe_ts.append(state.timestamp)
    # Purge old timestamps
    vstate.m22_sell_vfe_ts = [ts for ts in vstate.m22_sell_vfe_ts
                              if state.timestamp - ts <= M22_SEQ_WINDOW_TS]

    # Don't overlap with VFE momentum short (already at -200 if firing)
    momo_active = vstate.vfe_momo_short_entry is not None

    # Trigger: Mark 55 sells AND Mark 22 sold within last 100 ticks AND no active short
    if (new_m55_sell and len(vstate.m22_sell_vfe_ts) > 0
        and vstate.seq_short_entry is None
        and not momo_active):
        # SHORT VFE
        room = 200 + pos
        qty = min(M22_SEQ_SIZE, room)
        if qty > 0:
            orders.append(Order(P, bb, -qty))
            sell_qty = qty
            vstate.seq_short_entry = mid
            vstate.seq_short_entry_ts = state.timestamp

    # Exit conditions for active short
    if vstate.seq_short_entry is not None:
        per_share_pnl = vstate.seq_short_entry - mid
        held_ts = state.timestamp - (vstate.seq_short_entry_ts or state.timestamp)
        cover = (per_share_pnl >= M22_SEQ_TP_PER_SHARE
                 or per_share_pnl <= -M22_SEQ_SL_PER_SHARE
                 or held_ts >= M22_SEQ_HOLD_TS)
        if cover:
            qty = min(M22_SEQ_SIZE, 200 - pos)
            if qty > 0:
                orders.append(Order(P, ba, qty))
            vstate.seq_short_entry = None
            vstate.seq_short_entry_ts = None

    return orders, sell_qty
...
def run_vouchers(state, vstate):
    ...
    # v6: Mark 22S → Mark 55S sequence short layer
    seq_orders, seq_sell = run_mark_seq_short(state, vstate)
    ...
    if od_ve and od_ve.buy_orders and od_ve.sell_orders:
        ...
        if wm is not None:
            ...
            fv = round(wm)
            # Account for Layer E + VFE-momentum + sequence commitments
            tb = 200 - pos - layer_e_buy - momo_buy
            ts = 200 + pos - layer_e_sell - momo_sell - seq_sell
            half = 100
            mbp = fv - 1 if pos > half else fv
            msp = fv + 1 if pos < -half else fv
            momentum_active = vstate.vfe_momo_short_entry is not None
            seq_active = vstate.seq_short_entry is not None
            if momentum_active or seq_active:
                tb = 0
            ve_orders = list(momo_orders) + list(seq_orders) + list(ve_layer_e)
        ...
        elif momo_orders or seq_orders:
            orders[VEVE_SYM] = list(momo_orders) + list(seq_orders)
        elif ve_layer_e:
