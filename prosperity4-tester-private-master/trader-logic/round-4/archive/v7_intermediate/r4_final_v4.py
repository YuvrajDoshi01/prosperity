"""r4_final_v4.py — v3 + DEFENSIVE STACK (zero-cost insurance).

DEFENSIVE LAYERS added vs v3 (both BT byte-identical to v3 — no PnL change but tail-risk insurance):
  1. S17 CIRCUIT BREAKER (Agent 21): freeze S17 entries after 2 FLIP_HOLD timeouts.
     Bounds worst-case loss in unforeseen day-4 regimes.
  2. POSITION-LIMIT CLAMP (Agent 32): final-pass safety net that truncates orders
     if any layer-stack double-counts. Prevents IMC's all-or-nothing rejection.

These layers don't fire in our 10k 3-day BT but provide robustness for day 4.
Confirmed BT byte-identical to v3: $5,360 / $130,570 default / $121,002 imc.

Rejected v4 candidates (all BT-tested, none beat v3):
  - microprice tilt (-$972 default, -$171 imc)
  - early DEFENSE flag (-$2,702 default, -$2,810 imc)
  - VFE inventory skew (+$140 default, -$1,186 imc — wash)
  - P3 rolling-IV mean MM (regression)
  - voucher portfolio scaling (byte-identical, no fills)
