"""r4_v2.py — Round 4 with counterparty-aware quote tilts.

Inherits r4_v1 baseline (HP S17 + VFE Wall Mid + voucher BS).
Adds counterparty-signal layer mining R4 trades for:
  Mark 67 buy VFE  -> bullish 100t tilt  (n=165, fwd +1.57, t=+2.13)
  Mark 55 sell VFE -> bearish 100t tilt  (n=600, fwd -0.90, t=-2.48)
  Mark 49 sell VFE qty>=10 -> bullish 100t tilt (passive seller picked off, fwd +1.92)
  Mark 01 buy VEV_5200 -> long 500t opportunistic taker (n=11 fwd +7.41 t=+6.05)

Timestamp increments by 100 per tick: 100 ticks = +10000 ts.
"""
...
def run_hydrogel(state, hstate):
    """v22 HP strategy: S17 GIGA SHORT -> S7 bottom cover -> flip long -> hold to FLIP_EXIT_MID."""
    ...
    # Hold-flip phase: suppress passive ask until mid recovers
    ...
    # Entry: spread=17 AND elevated price
...
# TTE convention unchanged from R3: each day starts at TTE=5, decays within day.
# Verified via IV consistency: TTE=5 gives CoV=2.76% vs TTE=4 gives CoV=2.76%
# (same ratio but absolute IV levels match R3 calibration better at TTE=5).
...
# Delta-hedging params
DELTA_HEDGE_ENABLED = True
DELTA_HEDGE_THRESHOLD = 30   # hedge when abs(net_delta) > this
DELTA_HEDGE_SIZE = 20        # hedge this many VFE shares per tick


# ── Counterparty signal layer (R4) ──────────────────────────────────────────
# 100 ticks = +10000 timestamp; 500 ticks = +50000 timestamp.

CP_VFE_TILT_TTL_SHORT = 10000   # 100 ticks
CP_VFE_TILT_TTL_LONG  = 50000   # 500 ticks
CP_VFE_TILT_PRICE = 0           # 0 = disabled; in-sample BT showed -$2k vs v1 baseline
                                # Framework retained for live testing — counterparty signal
                                # firing now WORKS in BT (test_runner patched). Re-enable
                                # only with stronger signals or different application.
# VEV_5200 signal disabled: in-sample only n=11, lost -$824 on day-1 BT replay.
CP_VEV5200_ENABLED = False
CP_VEV5200_TAKE_TTL = 50000
CP_VEV5200_TAKE_SIZE = 50
CP_VEV5200_TAKE_POS_CAP = 200
...
def compute_portfolio_delta(state, spot, T, sigma):
    """Compute net delta exposure from all voucher positions."""
    total_delta = 0.0
    for K in VOUCHER_STRIKES_ALL:
        sym = VOUCHER_SYM[K]
        pos = state.position.get(sym, 0)
        if pos != 0:
            d = v_bs_delta(spot, K, T, sigma)
            total_delta += pos * d
    # Add VFE position (delta = 1.0 per share)
    vfe_pos = state.position.get(VEVE_SYM, 0)
    total_delta += vfe_pos
    return total_delta


class CounterpartyState:
    """Tracks counterparty-driven directional tilts with TTL expiry."""
    def __init__(self):
        self.vfe_tilt_until: int = 0     # ts; tilt expires when state.timestamp >= this
        self.vfe_tilt_dir: int = 0       # +1 bullish, -1 bearish, 0 neutral
        self.vev5200_long_until: int = 0 # ts; opportunistic take long until this

    def to_dict(self):
        return {"vt": self.vfe_tilt_until, "vd": self.vfe_tilt_dir, "v52": self.vev5200_long_until}

    @staticmethod
    def from_dict(d):
        s = CounterpartyState()
        s.vfe_tilt_until = d.get("vt", 0)
        s.vfe_tilt_dir = d.get("vd", 0)
        s.vev5200_long_until = d.get("v52", 0)
        return s


def update_cp_signals(state, cp_state):
    """Scan market_trades for counterparty patterns and refresh tilts.
    Mark 67 buy VFE  -> bullish 100t (n=165, t=+2.13)
    Mark 55 sell VFE qty>=3 -> bearish 100t (n=600, t=-2.48)
    Mark 49 sell VFE qty>=10 -> bullish 100t (n=105, t=+2.13)
    Mark 01 buy VEV_5200 -> opportunistic long 500t (n=11, t=+6.05)
    """
    ts = state.timestamp

    # Decay expired tilts
    if ts >= cp_state.vfe_tilt_until:
        cp_state.vfe_tilt_dir = 0

    # Scan VFE market trades
    vfe_trades = state.market_trades.get(VEVE_SYM, []) if state.market_trades else []
    bull_signal = False
    bear_signal = False
    for tr in vfe_trades:
        # market_trades are refreshed each tick (engine bug fixed pre-R3); trust freshness.
        if tr.buyer == "Mark 67":
            bull_signal = True
        elif tr.seller == "Mark 55" and tr.quantity >= 3:
            bear_signal = True
        elif tr.seller == "Mark 49" and tr.quantity >= 10:
            bull_signal = True

    # Bull dominates if both fire same tick
    if bull_signal:
        cp_state.vfe_tilt_dir = +1
        cp_state.vfe_tilt_until = ts + CP_VFE_TILT_TTL_SHORT
    elif bear_signal:
        cp_state.vfe_tilt_dir = -1
        cp_state.vfe_tilt_until = ts + CP_VFE_TILT_TTL_SHORT

    # Scan VEV_5200 market trades for Mark 01 buys (gated by CP_VEV5200_ENABLED).
    if CP_VEV5200_ENABLED:
        vev52_trades = state.market_trades.get("VEV_5200", []) if state.market_trades else []
        for tr in vev52_trades:
            if tr.buyer == "Mark 01":
                cp_state.vev5200_long_until = max(cp_state.vev5200_long_until, ts + CP_VEV5200_TAKE_TTL)


def cp_vfe_tilt_active(cp_state, ts):
    return cp_state.vfe_tilt_dir != 0 and ts < cp_state.vfe_tilt_until


def cp_vev5200_take_active(cp_state, ts):
    return CP_VEV5200_ENABLED and ts < cp_state.vev5200_long_until


def run_vouchers(state, vstate, cp_state=None):
    orders = {}
    timestamp = state.timestamp

    od_ve = state.order_depths.get(VEVE_SYM)
    # Layer E: VFE spread-state aggressive lift
    ...
    # Get spot and TTE for voucher pricing
    ...
    # Update IV history
    if spot is not None and vstate.spot_age <= 3:
        for K in V_BS_STRIKES:
            sym = VOUCHER_SYM[K]
    ...
    # Compute portfolio delta for hedging decisions
    net_delta = 0.0
    if spot is not None and DELTA_HEDGE_ENABLED:
        net_delta = compute_portfolio_delta(state, spot, T, sigma)

    # VFE Wall Mid MM (with delta-hedge bias)
    if od_ve and od_ve.buy_orders and od_ve.sell_orders:
        wm = v_wall_mid(od_ve)
        if wm is not None:
            ...
            fv = round(wm)
            # Counterparty tilt shifts the entire fv used for quoting (both take + post).
            if cp_state is not None and cp_vfe_tilt_active(cp_state, timestamp):
                fv = fv + cp_state.vfe_tilt_dir * CP_VFE_TILT_PRICE
            tb = 200 - pos - layer_e_buy
            ts = 200 + pos - layer_e_sell
            half = 100

            # Delta-hedge bias: if portfolio is long delta, bias VFE to sell more
            # If portfolio is short delta, bias VFE to buy more
            if DELTA_HEDGE_ENABLED and abs(net_delta) > DELTA_HEDGE_THRESHOLD:
                if net_delta > 0:
                    # Long delta -> want to sell VFE -> lower the sell threshold
                    mbp = fv - 1 if pos > half else fv
                    msp = fv  # more aggressive selling
                else:
                    # Short delta -> want to buy VFE -> raise the buy threshold
                    mbp = fv  # more aggressive buying
    ...
    if spot is None or vstate.spot_age > 3:
        return orders

    # Intrinsic arb on all strikes (skip 6000/6500)
    for K in VOUCHER_STRIKES_ALL:
    ...
    # Counterparty alpha: opportunistic VEV_5200 long take when Mark 01 has been buying
    # (n=11 historical, fwd +7.41 @ H=500, t=+6.05). Hold for 500 ticks after each signal.
    if cp_state is not None and cp_vev5200_take_active(cp_state, timestamp):
        sym = "VEV_5200"
        od_v = state.order_depths.get(sym)
        if od_v is not None and od_v.sell_orders:
            pos_v = state.position.get(sym, 0)
            existing = orders.get(sym, [])
            already_buy = sum(o.quantity for o in existing if o.quantity > 0)
            cur_pos_buy = pos_v + already_buy
            room = min(300 - cur_pos_buy, CP_VEV5200_TAKE_POS_CAP - cur_pos_buy)
            if room > 0:
                # Take cheapest asks up to budget. Cap per-tick at CP_VEV5200_TAKE_SIZE.
                budget = min(room, CP_VEV5200_TAKE_SIZE)
                for px in sorted(od_v.sell_orders.keys()):
                    if budget <= 0: break
                    avail = -od_v.sell_orders[px]
                    size = min(avail, budget)
                    if size > 0:
                        orders.setdefault(sym, []).append(Order(sym, px, +size))
                        budget -= size
    ...
    # Passive MM on tradeable strikes
