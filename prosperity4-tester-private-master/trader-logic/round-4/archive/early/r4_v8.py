"""r4_v4.py — R4 submission: v22 HP S17 + z-score mean-reversion overlay.

HP: S17 GIGA SHORT (v22) + z-score mean-reversion (NEW).
  Priority: S17 entry/cover/flip > z-score > passive MM.
  z-score: 500-tick window, |z|>2.0 => directional entry, |z|<0.5 => flatten.
  Size scales with |z|: base_size * min(|z|/2.0, 3.0), capped at 120.

  10k imc results:
    Day 1: $11,558 (v22: $21,552)  — z-score too conservative on uptrend day
    Day 2: $30,929 (v22: $39,853)  — z trades against S17 cycle slightly
    Day 3: $43,330 (v22: -$12,684) — z-score captures mean-reversion perfectly
    Total: $85,818 (v22: $48,720)  — net +$37,098

  1k day 2 (website probe): $15,448 (identical to v22).

VFE: Wall Mid + Layer-E spread-state lift (v22 unchanged).
Vouchers: BS taking + intrinsic arb + call-spread arb + passive MM + deep-ITM
  theta carry + OTM passive bids (v22 unchanged).
"""
...
class HydrogelParams:
    ...
    Z_ENTRY = 2.5          # v8: tightened from 2.0 (reduce false pos on calm days)
