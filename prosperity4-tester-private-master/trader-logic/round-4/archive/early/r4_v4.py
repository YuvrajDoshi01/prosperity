"""r4_v1.py — Round 4 baseline: v22 HP + improved VFE/voucher strategy.

Based on r3_v22 (best live R3 strategy: $40k 10k-tick day 2).
Changes from v22:
  - TTE adjusted for R4 (one more day elapsed vs R3)
  - Tighter voucher MM on ATM strikes (narrower edge, larger size)
  - Delta-hedging: offset voucher delta exposure via VFE position
  - Voucher position capped tighter to limit day-3-style crash losses

HP: spread==17 & mid>10010 -> short to -200; cover when spread==7 & mid <= 8th-pctl
of last 500-tick window; flip long to +200; HOLD until mid >= 10020 or timeout.

VFE: Wall Mid (highest-volume bid/ask midpoint) + Layer-E aggressive lift.

Vouchers: BS taking (adaptive IV) + intrinsic arb + call-spread arb + passive MM
+ deep-ITM theta carry + OTM passive bids + delta-hedging.
"""
...
class HydrogelParams:
    ...
    S7_BOTTOM_Q = 0.08
    FLIP_TARGET = 200
    FLIP_EXIT_MID = 10010          # was 10020 — R4 day-3 mid only recovered to 10001
    FLIP_TIMEOUT_TICKS = 1500
    FLIP_STOP_LOSS_DROP = 30       # exit flip if mid drops this much below entry
    POS_LIMIT = 200
    QUOTE_SIZE = 25
    Z500_WINDOW = 500
    STD100_WINDOW = 100
