...
class Trader:
    def run(self, state: TradingState) -> Tuple[Dict[Symbol, List[Order]], int, str]:
        ...
        # === v6 NEW: Mark 67 Olivia-analog post-trade camp ===
        # Forensics (intel/mark67_olivia.md): Mark 67 buys VFE 165x/3 days,
        # h=1 mean +1.97 t=26, h=10 mean +2.26 t=10. Drift is REAL beyond
        # trade impact. Strategy: when Mark 67 buys VFE, camp aggressive
        # ask-1 bid for M67_CAMP_TICKS to ride the drift.
        # Rejected: pre-position (IAT≈Poisson, no clusters), daily-extrema
        # (only 23% in bottom25% ≈ random), joint M49 (marginal).
        M67_CAMP_TICKS = 10
        M67_CAMP_SIZE = 30
        m67_left = int(raw.get("m67_left", 0))
        for trd in state.market_trades.get(VEVE_SYM, []):
            if getattr(trd, "buyer", "") == "Mark 67":
                m67_left = M67_CAMP_TICKS
                break
        if m67_left > 0:
            od_ve = state.order_depths.get(VEVE_SYM)
            if od_ve and od_ve.buy_orders and od_ve.sell_orders:
                pos_ve = state.position.get(VEVE_SYM, 0)
                already_buy = sum(o.quantity for o in orders.get(VEVE_SYM, []) if o.quantity > 0)
                bb_ve = max(od_ve.buy_orders); ba_ve = min(od_ve.sell_orders)
                room = 200 - pos_ve - already_buy
                if room > 0 and ba_ve - bb_ve >= 2:
                    px = ba_ve - 1
                    q = min(M67_CAMP_SIZE, room)
                    orders.setdefault(VEVE_SYM, []).append(Order(VEVE_SYM, px, q))
            m67_left -= 1
        raw["m67_left"] = m67_left
