"""r4_final_v2.py — R4 RE-SUBMISSION after 494304 disaster.

CONTEXT: Submission 494304 (= prior r4_final, z=2.25 only) scored **-$9,997 LIVE**
on the 1k day-3 probe. BT predicted -$9,860 (calibration matches).
Killed by HP S17 GIGA SHORT firing at ts=14200 (mid=10018.5) — HP didn't revert
in 1k window, holding -200 short into elevated price = -$11k.

CHANGES in v2 (= r4_v12_vfe_momo_oneshot):
  1. **S17 VFE-CRASH GATE**: block S17 entry when VFE drifted < -$5 vs first-25
     ticks of 200-tick rolling buffer. Day 3 first-1k VFE drops $32 → S17 blocked
     → avoids -$11k trap. Days 1-2 VFE stable → S17 fires normally.
  2. **VFE MOMENTUM SHORT (one-shot per day)**: when (mid - mid_50_ago) <= -3,
     SHORT 200 VFE at best bid. Exit at TP=+$2k MTM or SL=-$3k MTM, else hold to EOD.
...
class HydrogelState:
    def __init__(self):
        ...
        self.vfe_buf: List[float] = []  # v10: VFE mid for crash gate (kept for compat)
        self.s17_failed_count: int = 0  # v15: circuit breaker — freeze S17 after 2 timeouts
    ...
    def from_dict(d):
        ...
        s.s17_failed_count  = d.get("s17fc", 0)
        return s
...
def run_hydrogel(state, hstate):
    ...
    # v15: Increment s17_failed_count on TIMEOUT (not mid_target) — circuit breaker.
    if hstate.flip_holding:
        ...
        if mid >= p.FLIP_EXIT_MID or held_for >= p.FLIP_TIMEOUT_TICKS:
            ...
            if held_for >= p.FLIP_TIMEOUT_TICKS:
                hstate.s17_failed_count += 1   # v15: circuit-breaker counter
            hstate.flip_holding = False
            hstate.flip_entry_row = None
        return orders, hstate

    # v15: HP-INTERNAL regime gate (replaces VFE-drift gate per Agent 9 finding).
    # VFE-drift gate cost ~$6k on day 1 by blocking the BEST S17 setups.
    # HP regime detector: count number of times HP crossed 500-tick mean recently.
    # If recent HP shows no cycling (revert_count < 2 AND range_200 < 25) → flat regime → block S17.
    hp_revert_count = 0
    hp_range_200 = 0
    if len(hstate.mid_buf_500) >= 200:
        recent_200 = hstate.mid_buf_500[-200:]
        hp_range_200 = max(recent_200) - min(recent_200)
        # Count zero-crossings of (mid - mean_500) in the last 200 ticks
        if len(hstate.mid_buf_500) >= 500:
            mean_500 = sum(hstate.mid_buf_500) / 500
        else:
            mean_500 = sum(hstate.mid_buf_500) / len(hstate.mid_buf_500)
        # Sign sequence
        signs = [1 if x > mean_500 else (-1 if x < mean_500 else 0) for x in recent_200]
        for i in range(1, len(signs)):
            if signs[i] != 0 and signs[i-1] != 0 and signs[i] != signs[i-1]:
                hp_revert_count += 1

    hp_flat_regime = (hp_revert_count < 2 and hp_range_200 < 25 and len(hstate.mid_buf_500) >= 200)

    # v15: Circuit breaker — freeze S17 after 2 FLIP_HOLD timeouts (Agent 21 rec).
    s17_frozen = hstate.s17_failed_count >= 2

    # Entry: spread=17 AND elevated price AND HP not in flat regime AND not frozen
    if (spread == p.REGIME2_SPREAD
            and mid > p.ENTRY_MID_MIN
            and hstate.s17_entry_row is None
            and not hp_flat_regime
            and not s17_frozen):
        headroom = pos_lim + position
