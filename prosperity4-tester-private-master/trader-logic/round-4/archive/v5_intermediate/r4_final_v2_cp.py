...
def run_hydrogel(state, hstate):
    ...
    # Entry: spread=17 AND elevated price AND not already in S17 short AND VFE not crashing
...
def run_vouchers(state, vstate):
    ...
    # ── COUNTERPARTY LAYER (refined intel): Mark 67 / 49 protective suppression ──
    # Mark 67 buys VFE → next-tick mid moves +1.97 (t=26.2, win 95.7%, n=165 over 3 days).
    # Mark 49 sells VFE → next-tick mid moves -1.81 (t=-19.0, win 96.7%, n=122).
    # Defense: when we see their trade in market_trades THIS tick, suppress our passive
    # SELL (vs Mark 67) or passive BUY (vs Mark 49) on VFE for this tick — avoid being
    # picked off before the rally/crash plays out.
    cp_suppress_buy = False
    cp_suppress_sell = False
    mt_vfe = state.market_trades.get(VEVE_SYM, []) if state.market_trades else []
    for t in mt_vfe:
        if t.timestamp == state.timestamp:
            if t.buyer == "Mark 67" and t.quantity >= 6:
                cp_suppress_sell = True   # don't sell to him; he'll mark us up +1.97
            if t.seller == "Mark 49" and t.quantity >= 6:
                cp_suppress_buy = True    # don't buy from him; mid drops -1.81
    ...
    if od_ve and od_ve.buy_orders and od_ve.sell_orders:
        ...
        if wm is not None:
            ...
            if cp_suppress_sell: ts = 0   # cancel ask: don't get picked off by Mark 67
            if cp_suppress_buy:  tb = 0   # cancel bid: don't catch Mark 49's falling knife
