...
def run_vouchers(state, vstate):
    ...
    # voucher_alpha upgrade: strike-specific MM size (5300 = winning strike, +50%)
    STRIKE_MM_SIZE = {5000: 20, 5100: 20, 5200: 20, 5300: 30, 5400: 20}
    for K in VOUCHER_STRIKES_TRADEABLE:
        ...
        mm_size = STRIKE_MM_SIZE.get(K, V_VOUCHER_MM_SIZE)
        if tb_v > 0:
            orders.setdefault(sym, []).append(Order(sym, vbb + 1, min(tb_v, mm_size)))
        if ts_v > 0:
            orders.setdefault(sym, []).append(Order(sym, vba - 1, -min(ts_v, mm_size)))
