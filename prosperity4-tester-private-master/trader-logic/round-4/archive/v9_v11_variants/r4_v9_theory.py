"""r4_v9_theory.py — v8 with theory-fail layers REMOVED.

THEORY-FIRST AUDIT (intel/theory_audit.md):
  - Layer 10 (VFE momentum one-shot): REMOVED. BT-FIT, day-3-specific,
    LOOKBACK/TP swept multiple times. -$718 BT cost (essentially wash).
  - Layer 12 (Mark 55 follow-flow): REMOVED. corr=+0.16 fails Bonferroni
    on 7-mark x >=10-horizon search. +$1,736 BT GAIN from removal.

KEPT (theory-defensible):
  - All structural arb (intrinsic, call-spread, BS taking, theta carry, deep OTM)
  - HP S17 cycle + z-gate + circuit breaker (reverse-engineered MM bot)
  - HP z-MR (canonical OU mean reversion)
  - VFE Wall-Mid MM (P3 winner, institutional anchor)
  - M49 fade (t=20, n=105, survives multiple-testing)
  - Conditional Voucher OBI (partial Kyle theory; -$6,401 to remove, kept)
  - YOLO regime gate (FLAGGED day-3 fit but +$90k headline)

Ablation BT (10k 3-day default):
  v8 baseline:        $261,854
  v8 no VFE momo:     $261,136 (-$718)
  v8 no M55:          $263,590 (+$1,736) -- removal HELPS
  v8 no OBI:          $255,453 (-$6,401)
  v8 no YOLO:         $171,510 (-$90,344)
  v9_theory target:   ~$263k (removes both wash-or-helpful layers)

Original v6 docstring below.
...
def _compute_m55_netflow(state, vstate):
    """v9_theory: Mark 55 follow-flow REMOVED (theory-fail, BT-FIT).
    Returns (0, 0) — caller logic short-circuits when signal == 0."""
    return 0, 0
    tick_net = 0
...
def run_vfe_momentum(state, vstate, current_vfe_orders, signal_buy_used, signal_sell_used):
    """v9_theory: VFE momentum ONE-SHOT REMOVED (BT-FIT, day-3-specific).
    LOOKBACK=25, TP=12 are sweep-tuned. one-shot-per-day has no theory.
    Returns ([], 0, 0) so the caller treats VFE as MM-only."""
    return [], 0, 0
    P = VEVE_SYM
    od = state.order_depths.get(P)
    if not od or not od.buy_orders or not od.sell_orders:
