"""r4_v9_riskmgmt.py — v8 with YOLO regime gate DISABLED (Frankfurt risk philosophy).

YOLO sanity-check verdict (intel/yolo_sanity.md):
  - Bayesian P(day-3 LIVE) = 0.55  ->  E[YOLO PnL] = +$15k, Sharpe = 0.14, CVaR_5 = -$253k
  - Days 1/2 counterfactual (if YOLO had fired): -$28.9k / -$47.6k respectively
  - Sharpe 0.14 fails Frankfurt floor (lock-in lead, accept lower EV for variance reduction)
  - CVaR_5 = -$253k > full v8 baseline 3-day BT ($261k) -> one bad path wipes round
  - Top teams getting $50k LIVE day-3 1k probe -> did NOT go max-size short

Decision:
  - Keep ALL v8 alpha layers (HP S17, VFE Wall-Mid MM + momo, voucher MM, OBI, M49 fade)
  - DISABLE YOLO regime gate by setting YOLO_DRIFT_THRESHOLD to -infinity (never fires)
  - Byte-identical to v8 on days 1/2 (gate already skipped there in v8)
  - On day 3: forfeit $90k YOLO uplift, keep $69,955 v7c base -> 3-day BT = $171,510

Risk-adjusted trade:
  Give up $15k Bayesian-EV uplift to remove $253k CVaR_5 tail. Sharpe-positive remove.

Identical to r4_final.py except YOLO_DRIFT_THRESHOLD = -1e9 (gate never fires).
All other layers - HP, VFE, voucher MM, OBI, M49 - unchanged.

= = =

Original r4_final.py docstring (PRESERVED):
...
YOLO_DRIFT_THRESHOLD = -1e9      # v9: disabled (was -1.5). Frankfurt risk-philosophy lock-in.
