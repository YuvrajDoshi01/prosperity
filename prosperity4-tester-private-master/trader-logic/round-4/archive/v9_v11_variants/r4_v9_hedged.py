"""r4_v9_hedged.py — v8 hybrid + max-long VFE delta hedge during yolo regime.

NEW vs v8 (r4_final.py):
  HEDGING DISCIPLINE re-audit (re-evaluation of Agent 18's rejected delta hedge).

  When yolo regime fires (VFE drift <= -1.5 at ts 3000), v8 holds
    8 × -300 voucher shorts + -200 VFE.
  Aggregate delta computed from BT log (10k day-3):
    voucher delta ≈ -1500 (BS, sigma=0.27, ATM 5300)
    + VFE delta -200 = total spot delta ≈ -1700
    MTM swing on +/-30 VFE move: +/-50,000 PnL.

  Variance-reduction analysis (synthetic Monte Carlo, N=2000 paths):
    Variant         | mean    | std    | CVaR-5%  | Sharpe
    v8 (VFE=-200)   | +63,160 | 92,696 | -144,354 | 0.68
    v9d (VFE=+200)  | +62,792 | 72,626 | -102,235 | 0.86
    Delta-mean: -369 (well within $5k criterion)
    Delta-CVaR: +42,119 (passes $20k criterion)
    Delta-std:  -20,070 (22% variance reduction)

  Mechanism: voucher delta unchanged (alpha capture intact), VFE swings from
  -200 short to +200 long (a 400-unit shift toward delta-positive). On
  symmetric prior, modestly under-hedges the -1500 voucher delta but is the
  maximum offset allowed by VFE position limit (200).

  Real day-3 cost (single realized path, bearish VFE drift):
    v8 = $160,299 / v9d = $135,654 (-$24,645). This is the insurance premium.

  Days 1+2: BYTE-IDENTICAL to v8 (yolo doesn't fire on those days).
  3-day BT: v8 $261,854 / v9d $237,209 (-$24,645, all on day 3).

  Acceptance: v9d Pareto-passes the user's hedging-audit criteria
  (CVaR-5 improvement >= $20k, mean drop < $5k) under symmetric and fat-tail
  distributions. Bullish-bias scenario (worst for shorts): v9d adds +$12k
  mean AND $46k CVaR — strict win.

  Trade-off: gives up $25k vs v8 in the realized day-3 scenario for ~$50k
  drawdown protection in the symmetric prior and substantially more in
  bullish-stress scenarios.

  See: intel/hedging_audit.md for full analysis.

== v8 (r4_final.py) ORIGINAL DOCSTRING BELOW ==
...
YOLO_VFE_TARGET = 200  # v9d: max long VFE hedge (400-unit swing toward delta-positive)
...
class Trader:
    def run(self, state: TradingState) -> Tuple[Dict[Symbol, List[Order]], int, str]:
        ...
        if yolo_regime is True:
            ...
            # VFE adjustment to YOLO_VFE_TARGET (bidirectional)
            if vfe_od:
                pos_vfe = state.position.get("VELVETFRUIT_EXTRACT", 0)
                desired = YOLO_VFE_TARGET - pos_vfe
                if desired < 0 and vfe_od.buy_orders:
                    qty = -desired
                ...
                elif desired > 0 and vfe_od.sell_orders:
                    qty = desired
                    ba = min(vfe_od.sell_orders)
                    avail = -vfe_od.sell_orders[ba]
                    q = min(qty, avail, 200)
                    if q > 0:
                        orders.setdefault("VELVETFRUIT_EXTRACT", []).append(Order("VELVETFRUIT_EXTRACT", ba, q))
