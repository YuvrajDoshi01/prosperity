"""r4_v7.py — R4 submission: v5 + COUNTERPARTY SKEW on VFE WM MM.

CHANGES from v5:
  - Layer F (skew-based): when Mark 67 buys VFE qty>=6 OR Mark 49 sells qty>=6,
    open a 100-tick bullish window. In that window, raise WM MM fv by +1
    (tighter bid, wider ask). This shifts MM aggression toward the alpha
    direction WITHOUT paying spread cost on a separate aggressive order.

  v6 (separate cross-spread orders) lost $3.5k/3-day to spread churn even
  with one-shot pulses, because Layer F orders SUBSTITUTED for passive MM
  buys that already captured Mark 49's flow. Skew approach avoids substitution.
"""
...
# ── R4 Layer F: Counterparty skew on VFE WM MM ────────────────────────────────
# Mark 67 (165 trades, t=+10.2 at h10) — bullish for VFE
# Mark 49 (122 trades, t=-7.8 at h10) — bullish for VFE (FADE = buy when sells)
# Both signals → shift wall_mid +1 for 100 ticks (bullish window).
# No separate orders — purely a skew on existing WM MM passive quotes.

CP_QTY_MIN = 6
CP_HORIZON_TS = 10000  # 100 ticks
CP_FV_BUMP = 1         # raise WM MM fv by +1 in bullish window


class CounterpartyState:
    def __init__(self):
        self.bull_window_end_ts: Optional[int] = None  # absolute ts when window expires

    def to_dict(self):
        return {"bw": self.bull_window_end_ts}

    @staticmethod
    def from_dict(d):
        s = CounterpartyState()
        s.bull_window_end_ts = d.get("bw")
        return s


def detect_bullish_window(state, cstate):
    """Update cstate.bull_window_end_ts based on this tick's market_trades.
    Returns: True if currently in a bullish window (any time post-update).
    """
    for t in state.market_trades.get(VEVE_SYM, []):
        if t.quantity < CP_QTY_MIN:
            continue
        # Mark 67 buy OR Mark 49 sell triggers bullish window
        if t.buyer == "Mark 67" or t.seller == "Mark 49":
            new_end = state.timestamp + CP_HORIZON_TS
            if cstate.bull_window_end_ts is None or new_end > cstate.bull_window_end_ts:
                cstate.bull_window_end_ts = new_end

    if cstate.bull_window_end_ts is None:
        return False
    return state.timestamp <= cstate.bull_window_end_ts
...
def run_vouchers(state, vstate, cstate=None):
    ...
    # Layer F: detect bullish window (Mark 67 buy / Mark 49 sell on VFE)
    in_bull_window = False
    if cstate is not None:
        in_bull_window = detect_bullish_window(state, cstate)
    ...
    if od_ve and od_ve.buy_orders and od_ve.sell_orders:
        ...
        if wm is not None:
            ...
            fv = round(wm)
            # Layer F: shift fv up by +1 in bullish window (tighter bid, wider ask)
            if in_bull_window:
                fv += CP_FV_BUMP
