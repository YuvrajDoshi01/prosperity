"""r4_v6.py — R4 submission: v5 + COUNTERPARTY ALPHA (Mark 67 COPY / Mark 49 FADE).

CHANGES from v5:
  - V_TTE_DAYS_AT_START = 4.0 (was 5.0 — R4 has TTE=4 on trading day)
  - NEW Layer F (VFE): COPY Mark 67 buys (qty>=6, t=+10.2 h10), FADE Mark 49
    sells (t=-7.8 h10). Crosses spread, 100-tick hold, capped at 60 inv.
  - Layer F orders run BEFORE WM MM and reserve position headroom.

Counterparty intel (intel/counterparty_mining.md):
  Mark 67 = pure VFE buyer (165 trades, t=+10.2 at h10, +$1,867 next-tick PnL)
  Mark 49 = pure VFE seller (122 trades, t=-7.8 at h10, -$1,404 PnL)
  Both survive Bonferroni at α=0.05/7 ⇒ |t|>2.69
  Expected combined alpha: $3-4k / 3-day on top of v5's $86k baseline.
"""
...
class HydrogelParams:
    ...
    # ===================== Z-SCORE MEAN REVERSION (v4 NEW) =============
    Z_WINDOW = 500         # rolling window for z-score (longer = more conservative)
    Z_ENTRY = 2.0          # |z| > this => directional entry (higher = fewer trades)
...
# ── R4 Layer F: Counterparty alpha on VFE ─────────────────────────────────────
# Mark 67 (165 trades, t=+10.2 at h10) — COPY by buying when he buys
# Mark 49 (122 trades, t=-7.8 at h10) — FADE by selling when he sells
# Filter: qty >= 6 to remove micro noise. Hold ~100 ticks (10000 ts units).

CP_QTY_MIN = 6
CP_HORIZON_TS = 10000        # 100 ticks
CP_SIG_QTY = 10              # cross-spread qty per active signal tick
CP_INV_CAP = 60              # cap signal-driven inventory tilt (keep VFE MM headroom)


class CounterpartyState:
    def __init__(self):
        self.m67_last_buy_ts: Optional[int] = None
        self.m49_last_sell_ts: Optional[int] = None
        self.signal_inv: int = 0   # tracker of signal-driven net inventory

    def to_dict(self):
        return {"m67": self.m67_last_buy_ts, "m49": self.m49_last_sell_ts, "si": self.signal_inv}

    @staticmethod
    def from_dict(d):
        s = CounterpartyState()
        s.m67_last_buy_ts = d.get("m67")
        s.m49_last_sell_ts = d.get("m49")
        s.signal_inv = d.get("si", 0)
        return s


def run_counterparty_vfe(state, cstate):
    """Layer F: ONE-SHOT pulse on detected Mark 67 buy / Mark 49 sell.

    Avoids per-tick churn — we only emit orders when we *detect* a new trade
    in market_trades this tick. signal_inv is just a cap-tracker; it decays
    via WM MM naturally rebalancing toward zero (no flatten orders here).
    """
    orders: List[Order] = []
    signal_buy = 0
    signal_sell = 0
    ...
    new_m67_buy = False
    new_m49_sell = False
    for t in state.market_trades.get(VEVE_SYM, []):
        if t.quantity < CP_QTY_MIN:
            continue
        if t.buyer == "Mark 67":
            new_m67_buy = True
            cstate.m67_last_buy_ts = state.timestamp
        if t.seller == "Mark 49":
            new_m49_sell = True
            cstate.m49_last_sell_ts = state.timestamp

    # Decay signal_inv if last trade was long ago (let WM MM rebalance us)
    HORIZON_DECAY = CP_HORIZON_TS * 2  # 200 ticks: signal_inv resets to 0
    last_trade_ts = max(cstate.m67_last_buy_ts or -1, cstate.m49_last_sell_ts or -1)
    if last_trade_ts > 0 and state.timestamp - last_trade_ts > HORIZON_DECAY:
        cstate.signal_inv = 0

    od = state.order_depths.get(VEVE_SYM)
    if not od or not od.buy_orders or not od.sell_orders:
        return orders, signal_buy, signal_sell

    pos = state.position.get(VEVE_SYM, 0)
    bb = max(od.buy_orders); ba = min(od.sell_orders)

    # Only emit orders on new trade detection (one-shot per signal)
    if new_m67_buy:
        room = min(CP_INV_CAP - cstate.signal_inv, 200 - pos)
        qty = min(CP_SIG_QTY, room)
        if qty > 0:
            orders.append(Order(VEVE_SYM, ba, qty))
            signal_buy = qty
            cstate.signal_inv += qty
    elif new_m49_sell:
        room = min(CP_INV_CAP + cstate.signal_inv, 200 + pos)
        qty = min(CP_SIG_QTY, room)
        if qty > 0:
            orders.append(Order(VEVE_SYM, bb, -qty))
            signal_sell = qty
            cstate.signal_inv -= qty
...
def run_vouchers(state, vstate, cstate=None):
    orders = {}
    timestamp = state.timestamp

    od_ve = state.order_depths.get(VEVE_SYM)

    # ── R4 Layer F: Counterparty alpha (Mark 67 COPY / Mark 49 FADE) ─────
    ve_layer_f: List[Order] = []
    layer_f_buy = 0
    layer_f_sell = 0
    if cstate is not None:
        ve_layer_f, layer_f_buy, layer_f_sell = run_counterparty_vfe(state, cstate)
    ...
    if od_ve and od_ve.buy_orders and od_ve.sell_orders:
        ...
        if (vstate.prev_ve_ap1 is not None and vstate.prev_ve_bp1 is not None):
            ...
            if spread_ve == 2 and ba_ve < vstate.prev_ve_ap1:
                room_buy = E_POS_CAP - pos_ve - layer_f_buy
            ...
            # Sell signal: spread==3 AND bid raised
                ...
                room_sell = E_POS_CAP + pos_ve - layer_f_sell
    ...
    if od_ve and od_ve.buy_orders and od_ve.sell_orders:
        ...
        if wm is not None:
            ...
            pos = state.position.get(VEVE_SYM, 0)
            fv = round(wm)
            # Account for Layer F + Layer E commitments in headroom
            tb = 200 - pos - layer_e_buy - layer_f_buy
            ts = 200 + pos - layer_e_sell - layer_f_sell
            half = 100
            mbp = fv - 1 if pos > half else fv
            msp = fv + 1 if pos < -half else fv
            ve_orders = list(ve_layer_f) + list(ve_layer_e)  # F first, then E
        ...
        elif ve_layer_f or ve_layer_e:
            orders[VEVE_SYM] = list(ve_layer_f) + list(ve_layer_e)
