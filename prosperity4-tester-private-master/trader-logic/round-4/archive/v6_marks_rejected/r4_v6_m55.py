"""r4_v6_m55.py — v5 + Mark 55 follow-flow alpha layer.

NEW vs v5: Mark 55 is a VFE-only specialist bot whose rolling 50-tick net flow
*positively* predicts VFE forward Δmid (r=+0.10 to +0.16, t=+8 to +16 across
all 3 days). When Σ(M55 net flow over last 50 ticks) crosses ±30 we take the
*same direction* (follow, not fade) at best ask/bid, qty=20, capped at ±60 pos.

PRESERVED from v5: HP S17 stack, OBI voucher layer, WM MM VFE, all v4 layers.

BT (vs r4_final_v5):
  10k 3-day default : $166,200 (+$2,824 vs $163,376)  STRICT Pareto
  10k 3-day imc     : $158,096 (+$1,914 vs $156,182)  STRICT Pareto
  1k day-3 probe    : $6,390   (= v5; signal needs warmup, won't fire in 1k)

THR sweep: 20→$160k (over-fires), 30→$166k★, 35→$163k, 40→$163k (=v5).

See intel/mark55_alpha.md for full hypothesis tests and analysis script.
...
class VoucherState:
    def __init__(self):
        ...
        # v6 NEW: Mark 55 net-flow rolling buffer (last 50 ticks of per-tick netflow)
...
# ── v6 Mark 55 follow-flow alpha ──────────────────────────────────────────────
# Source: intel/mark55_alpha.md
# Mark 55 is a VFE-specialist bot (598 buys / 600 sells, qty ~5.5 avg). His net
# flow over rolling 50 ticks correlates *positively* with VFE forward drift
# (r=+0.10 at h=50 across all 3 days, t=8-15). When Σ(netflow last 50 ticks) >=
# +20, take long; when <= -20, take short. Cap incremental position at ±M55_POS_CAP.
# Day-by-day backtest sim w=50 thr=±20 hold=50 size=20: $1.5k (d1) / $5k (d2) / $5k (d3).
M55_WINDOW = 50
M55_THRESH = 30            # sweep winner: +$2,824 def vs v5
M55_TAKE_SIZE = 20         # per-tick take while signal active
M55_POS_CAP = 60           # cap incremental position from M55 layer
...
def _compute_m55_netflow(state, vstate):
    """v6 Mark 55 follow-flow alpha:
    Per-tick net flow on VFE — combines market_trades AND own_trades.
    Necessary because in default-mode BT (and live IMC), when WE trade with
    Mark 55, the market-trade is consumed and only appears in own_trades.
    We still want to know which side M55 was on.
    """
    tick_net = 0
    ...
    for t in own:
        # When M55 is buyer, he was lifting offers → upward flow → +
        # When M55 is seller, he was hitting bids → downward flow → -
        if t.buyer == "Mark 55":
            tick_net += t.quantity
        elif t.seller == "Mark 55":
            tick_net -= t.quantity
...
def run_vouchers(state, vstate):
    ...
    # ── v6 NEW: Mark 55 follow-flow alpha (PRE-MM, claims position headroom) ──
    # Σ(M55 net flow last 50 ticks) >= +M55_THRESH → take BUY at best ask
    # Σ <= -M55_THRESH → take SELL at best bid
    # Capped at ±M55_POS_CAP from M55 layer (uses *current* pos as proxy).
    ...
    if od_ve and od_ve.buy_orders and od_ve.sell_orders:
        ...
        if m55_signal >= M55_THRESH and pos_m < M55_POS_CAP:
            ...
            room = min(M55_POS_CAP - pos_m, 200 - pos_m - layer_e_buy)
            ...
            room = min(M55_POS_CAP + pos_m, 200 + pos_m - layer_e_sell)
            q = min(M55_TAKE_SIZE, avail, room)
            if q > 0:
    ...
    # v6: roll M55 commitments into momo_buy/momo_sell counters so WM MM
    # respects them when computing remaining headroom.
    momo_buy += m55_buy
    momo_sell += m55_sell
    momo_orders = list(momo_orders) + m55_orders
