"""r4_v3.py — voucher MM revamp targeting R3's missed alpha.

Inherits r4_v1 baseline. Voucher changes:
  - V_VOUCHER_MIN_SPREAD 2 -> 1 (always MM, even tight books)
  - V_VOUCHER_MM_SIZE 20 -> 50 (more turnover per fill)
  - V_BS_EDGE 10 -> 3 (catch ATM mispricings, not just deep-ITM)
  - V_BS_TRADE_SIZE 30 -> 50, V_BS_POS_CAP 150 -> 250 (larger inventory per signal)
  - Add 4500, 5500 to V_BS_STRIKES (currently only 5000-5400)
  - DELTA_HEDGE_THRESHOLD 30 -> 10, continuous hedge every tick

R3 retrospective: top teams scored $345k = 2.9x perfect-foresight ceiling via active
delta-hedged voucher portfolio MM. We scored $4.6k voucher PnL of $32k total. Gate this
version on imc-mode BT voucher PnL >= $30k.
"""
...
def run_hydrogel(state, hstate):
    ...
    # v9_nodir: S17 GIGA SHORT DISABLED — pure MM only.
    # Reason: S17 short loses -$11k on 1k day 3 (HP doesn't revert in window).
...
V_INTRINSIC_QTY = 50
V_ARB_SIZE = 10
V_VOUCHER_MM_SIZE = 20           # REVERTED — MM tighten alone caused -$537k regression
V_VOUCHER_MIN_SPREAD = 2         # REVERTED — bots adverse-select on day-3 voucher crash
V_POST_SLACK_VE = 1
V_BS_SIGMA_DEFAULT = 0.18
V_BS_EDGE = 10.0                 # KEEP at 10 — EDGE=3 caused -$465k on day-3
V_BS_TRADE_SIZE = 30             # KEEP
V_BS_POS_CAP = 150               # KEEP
...
DELTA_HEDGE_THRESHOLD = 30       # KEEP
DELTA_HEDGE_SIZE = 20            # KEEP
