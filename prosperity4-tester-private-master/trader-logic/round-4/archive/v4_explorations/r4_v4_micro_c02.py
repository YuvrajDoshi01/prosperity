...
class HydrogelParams:
    ...
    # ===================== MICROPRICE TILT (v4 NEW) ====================
    # Agent 14: L2 microprice deviation predicts forward returns
    # (VFE corr=+0.139, HP corr=+0.115 at h=10, t-stat ~ 8).
    # micro_L2 = (bb * ask_vol1 + ba * bid_vol1) / (bid_vol1 + ask_vol1).
    # fv += round(TILT_COEFF * (micro - mid)) for passive MM only.
    # Skipped during S17 build / S7 cover / FLIP_HOLD phases.
    MICROPRICE_TILT_COEFF = 0.2
    MICROPRICE_TILT_CLIP = 2.0


def _l2_microprice(buys, sells):
    """L2 microprice = (bb * ask_vol1 + ba * bid_vol1) / (bid_vol1 + ask_vol1).
    Returns None if book is one-sided. sell_orders volumes are negative; abs()."""
...
def run_hydrogel(state, hstate):
    ...
    # v4: L2 microprice tilt — agent 14 reports HP h=10 corr ~ +0.115.
    fv = int(round(mid))
    micro = _l2_microprice(od.buy_orders, od.sell_orders)
    if micro is not None:
        tilt = _clip(p.MICROPRICE_TILT_COEFF * (micro - mid),
                     -p.MICROPRICE_TILT_CLIP, p.MICROPRICE_TILT_CLIP)
        fv = int(round(mid + tilt))
    bid_headroom = pos_lim - position
    ask_headroom = pos_lim + position
...
# v4: VFE Wall-Mid MM microprice tilt (agent 14: VFE h=10 corr ~ +0.139)
VFE_MICROPRICE_TILT_COEFF = 0.2
VFE_MICROPRICE_TILT_CLIP = 2.0
...
def run_vouchers(state, vstate):
    ...
    if od_ve and od_ve.buy_orders and od_ve.sell_orders:
        ...
        if wm is not None:
            ...
            # v4: L2 microprice tilt on VFE Wall-Mid (agent 14: corr +0.139, t~8).
            ve_mid = (bb + ba) / 2.0
            ve_micro = _l2_microprice(od_ve.buy_orders, od_ve.sell_orders)
            if ve_micro is not None:
                tilt = max(-VFE_MICROPRICE_TILT_CLIP,
                           min(VFE_MICROPRICE_TILT_CLIP,
                               VFE_MICROPRICE_TILT_COEFF * (ve_micro - ve_mid)))
                fv = round(wm + tilt)
            else:
                fv = round(wm)
