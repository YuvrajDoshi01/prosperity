"""r4_v4_safe.py — r4_final_v3 + DEFENSIVE position-limit clamp.

Bit-identical alpha to v3. Adds a final-pass safety net before submission
that clamps each product's order list so:
    pos + sum(buys) <= LIMIT  AND  pos - sum(sells) >= -LIMIT
Avoids IMC's all-or-nothing rejection if any layer-stack double-counts.

Validation: r4_final_v3 passed all 8 extreme-position unit tests
(test_pos_limit_v3.py) with ZERO violations, so on real BT data the clamp
is a NO-OP and PnL is byte-identical to v3.
...
def _clamp_to_position_limits(orders_by_sym, positions, debug_log=None):
    """Final-pass safety net. Truncates orders to satisfy IMC's per-product
    all-or-nothing rule. Preserves earlier orders (higher-priority alpha).

    For each product:
      total_buy must satisfy: pos + total_buy <= LIMIT  →  cap at LIMIT - pos
      total_sell must satisfy: pos - total_sell >= -LIMIT  →  cap at LIMIT + pos

    Truncation order: walks orders front-to-back, accepts each up to its remaining
    headroom, then if a partial cap applies, rewrites that order's qty downward
    rather than dropping it (preserves price level)."""
    clamped = {}
            ...
            if o.quantity > 0:
                ...
                if allow > 0:
                    if allow == o.quantity:
                        ...
                        if debug_log is not None:
                            debug_log.append(
                                f"CLAMP {sym} BUY {o.price}@{o.quantity}->{allow} pos={pos}"
                            )
                    cum_buy += allow
                elif debug_log is not None:
                    debug_log.append(
                        f"CLAMP {sym} BUY {o.price}@{o.quantity}->0 pos={pos}"
                    )
            elif o.quantity < 0:
                want = -o.quantity
                ...
                if allow > 0:
                    if allow == want:
                        ...
                        if debug_log is not None:
                            debug_log.append(
                                f"CLAMP {sym} SELL {o.price}@{want}->{allow} pos={pos}"
                            )
                    cum_sell += allow
                elif debug_log is not None:
                    debug_log.append(
                        f"CLAMP {sym} SELL {o.price}@{want}->0 pos={pos}"
                    )
            # qty == 0: drop silently
        clamped[sym] = out
    return clamped
...
class Trader:
    def run(self, state: TradingState) -> Tuple[Dict[Symbol, List[Order]], int, str]:
        ...
        # ── Defensive position-limit clamp (no-op on validated v3 paths) ──
        clamp_log: List[str] = []
