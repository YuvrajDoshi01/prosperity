"""r4_v4_voucher_obi.py — v3 + Conditional Voucher OBI layer.

Hypothesis (Agent 14 dismissed; re-tested per mandate):
  OBI = (best_bid_vol - best_ask_vol)/(best_bid_vol + best_ask_vol).
  CONDITIONAL fire only when:
    OBI > +0.7 AND spread <= 10 (compression) -> AGGRESSIVE BUY at ask
    OBI < -0.7                                  -> AGGRESSIVE SELL at bid
  Multi-strike confirm: when sign agrees on >=3 ATM strikes (5100/5200/5300/5400/5500),
  scale BUY/SELL size by 2x.

  Restricted to active strikes {4000, 5200, 5300, 5400, 5500} per mandate.
  Layered ON TOP of v3 voucher logic; obeys per-strike position cap and per-tick
  buy/sell headroom already committed by v3 layers.
"""
...
class HydrogelState:
    ...
    def to_dict(self):
        return {
            "buf500": self.mid_buf_500, "buf100": self.mid_buf_100, "row": self.row,
...
def run_hydrogel(state, hstate):
    ...
    if z is not None and abs(z) > p.Z_ENTRY:
        ...
        if z > p.Z_ENTRY:
            ...
            if delta < 0:
                ...
                if sell_qty > 0: orders.append(Order(P, best_bid, -sell_qty))
        else:
            target_pos = target_qty
            delta = target_pos - position
            if delta > 0:
                buy_qty = min(delta, pos_lim - position)
                if buy_qty > 0: orders.append(Order(P, best_ask, buy_qty))
        fv = int(round(mid))
        if z > p.Z_ENTRY:
    ...
    bid_qty = min(p.QUOTE_SIZE, max(0, bid_headroom))
    ask_qty = min(p.QUOTE_SIZE, max(0, ask_headroom))
    if bid_qty > 0: orders.append(Order(P, my_bid, bid_qty))
    if ask_qty > 0: orders.append(Order(P, my_ask, -ask_qty))
    return orders, hstate
...
def run_vfe_momentum(state, vstate, current_vfe_orders, signal_buy_used, signal_sell_used):
    ...
    extra: List[Order] = []; extra_buy = 0; extra_sell = 0
    if vstate.vfe_momo_short_entry is not None:
        ...
        cover = False
        if total_pnl >= VFE_MOMO_TP * VFE_MOMO_SIZE: cover = True
        elif total_pnl <= -VFE_MOMO_SL * VFE_MOMO_SIZE: cover = True
        if cover:
            qty_to_cover = min(VFE_MOMO_SIZE, 200 - pos); qty_to_cover = max(0, qty_to_cover)
            if qty_to_cover > 0:
                extra.append(Order(P, ba, qty_to_cover)); extra_buy = qty_to_cover
    ...
    if (not vstate.vfe_momo_fired and len(vstate.vfe_momo_buf) >= VFE_MOMO_LOOKBACK + 1):
        ...
        if velocity <= VFE_MOMO_THRESH and pos >= -50:
            ...
            if qty > 0:
                extra.append(Order(P, bb, -qty)); extra_sell = qty
...
def run_vouchers(state, vstate, obi_size_base):
    """obi_size_base: 0 disables OBI layer; otherwise base size per fire."""
    orders = {}
    ...
    DEEP_OTM_BID_PX = 0; DEEP_OTM_BID_SIZE = 100
    for K in [6000, 6500]:
        sym = VOUCHER_SYM[K]
        od_v = state.order_depths.get(sym)
        if od_v is None: continue
    ...
    # ── v4 NEW: CONDITIONAL VOUCHER OBI LAYER ────────────────────────────────
    if obi_size_base > 0:
        # Compute OBI for ATM strikes for multi-strike confirmation
        ...
        for K in OBI_ACTIVE_STRIKES:
            ...
            # BUY trigger: OBI>+0.7 AND spread<=10 (compression rule for ATM/OTM)
            if obi > OBI_THRESHOLD and sp_v <= OBI_SPREAD_COMPRESS_MAX and pos_v < OBI_POS_CAP:
                qty = obi_size_base * scale_buy
            ...
            # SELL trigger: OBI<-0.7 (no spread gate per mandate)
            if obi < -OBI_THRESHOLD and pos_v > -OBI_POS_CAP:
                qty = obi_size_base * scale_sell
...
# Module-level OBI base size (set per test variant)
_OBI_BASE_SIZE = OBI_BASE_SIZE
...
class Trader:
    def run(self, state: TradingState) -> Tuple[Dict[Symbol, List[Order]], int, str]:
        ...
        voucher_orders = run_vouchers(state, vstate, _OBI_BASE_SIZE)
