...
# v4: VFE Wall-Mid inventory-aware skew (Agent 2 finding — day-3 -$3,709 structural).
# When |pos| >= VFE_SKEW_THR, asymmetrically quote to encourage flattening:
#   long  (pos >= +THR): bid posted at fv - VFE_BID_WIDEN (widen),
#                        ask posted at fv + VFE_ASK_TIGHTEN (tighten)
#   short (pos <= -THR): bid at fv - VFE_ASK_TIGHTEN (tighten),
#                        ask at fv + VFE_BID_WIDEN  (widen)
# Symmetric. Default V_POST_SLACK_VE behavior (bid fv-1 / ask fv+1) preserved when |pos|<THR.
VFE_SKEW_THR = 50      # |pos| trigger
VFE_BID_WIDEN = 2      # offset on inventoried side (was 1)
VFE_ASK_TIGHTEN = 0    # offset on flatten side (was 1; 0 = quote AT fair)
...
def run_vouchers(state, vstate):
    ...
    if od_ve and od_ve.buy_orders and od_ve.sell_orders:
        ...
        if wm is not None:
            ...
            # v4: inventory-aware skew on the residual passive layer
            if pos >= VFE_SKEW_THR:
                # long: widen bid, tighten ask
                bid_off = VFE_BID_WIDEN
                ask_off = VFE_ASK_TIGHTEN
            elif pos <= -VFE_SKEW_THR:
                # short: tighten bid, widen ask
                bid_off = VFE_ASK_TIGHTEN
                ask_off = VFE_BID_WIDEN
            else:
                bid_off = V_POST_SLACK_VE
                ask_off = V_POST_SLACK_VE
            if tb > 0:
                ve_orders.append(Order(VEVE_SYM, min(fv - bid_off, bb + 1), tb))
            if ts > 0:
                ve_orders.append(Order(VEVE_SYM, max(fv + ask_off, ba - 1), -ts))
