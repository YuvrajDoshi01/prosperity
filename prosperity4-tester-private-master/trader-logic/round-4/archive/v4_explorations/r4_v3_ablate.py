"""r4_v3_ablate.py — Ablation harness for v3 VEV_5300 PnL decomposition.

Reads env var R4_ABLATE = comma-separated layer names to DISABLE for VEV_5300:
  bs        — Black-Scholes taking layer
  mm        — Passive voucher MM (vbb+1 / vba-1)
  otm       — OTM passive bid layer
  intrinsic — Intrinsic arb layer
  arb       — Call-spread arb layer

Or special:
  ALL_BUT_BS, ALL_BUT_MM, ALL_BUT_OTM — keep only the named layer for 5300.
"""
import itertools
import json
import math
...
class Logger:
    ...
    def _compress_state(self, state):
        return [state.timestamp, state.traderData,
...
# === HP/VFE code: copy verbatim from v3 ===
...
def compute_book_features(order_depth):
    ...
    best_bid = max(buys.keys()); best_ask = min(sells.keys())
    ...
    return {"best_bid": best_bid, "best_ask": best_ask, "spread": spread, "mid": mid,
            "book_wap_edge_L3": ((bid_wap + ask_wap) / 2.0) - mid}


class HydrogelState:
    def __init__(self):
        self.mid_buf_500 = []; self.mid_buf_100 = []; self.row = 0
        self.s17_entry_row = None; self.s17_entry_mid = None
        self.s7_covering = False; self.flip_holding = False; self.flip_entry_row = None
        self.prev_mid = None; self.prev_wap_edge = None
        self.edge_buf = []; self.ret_buf = []; self.vfe_buf = []
    def to_dict(self):
        return {"buf500": self.mid_buf_500, "buf100": self.mid_buf_100, "row": self.row,
                "s17_row": self.s17_entry_row, "s17_mid": self.s17_entry_mid,
                "s7_cov": self.s7_covering, "fh": self.flip_holding, "fer": self.flip_entry_row,
                "pmid": self.prev_mid, "pedge": self.prev_wap_edge,
                "edgeb": self.edge_buf, "retb": self.ret_buf, "vfeb": self.vfe_buf}
    @staticmethod
    def from_dict(d):
        s = HydrogelState()
        s.mid_buf_500 = d.get("buf500", []); s.mid_buf_100 = d.get("buf100", [])
        s.row = d.get("row", 0); s.s17_entry_row = d.get("s17_row"); s.s17_entry_mid = d.get("s17_mid")
        s.s7_covering = d.get("s7_cov", False); s.flip_holding = d.get("fh", False)
        s.flip_entry_row = d.get("fer"); s.prev_mid = d.get("pmid"); s.prev_wap_edge = d.get("pedge")
        s.edge_buf = d.get("edgeb", []); s.ret_buf = d.get("retb", []); s.vfe_buf = d.get("vfeb", [])
        return s


def run_hydrogel(state, hstate):
    P = Product.HYDROGEL_PACK
    orders = []; p = HydrogelParams
    if P not in state.order_depths: return orders, hstate
    od = state.order_depths[P]; position = state.position.get(P, 0); pos_lim = p.POS_LIMIT
    features = compute_book_features(od)
    if not features: return orders, hstate
    mid = features["mid"]; spread = features["spread"]
    best_bid = int(features["best_bid"]); best_ask = int(features["best_ask"])
    ...
    hstate.prev_mid = mid; hstate.prev_wap_edge = wap_edge
    if hstate.s17_entry_row is not None and position < 0:
        window_ready = len(hstate.mid_buf_500) >= p.S7_WINDOW
        if window_ready:
            sw = sorted(hstate.mid_buf_500[-p.S7_WINDOW:])
            bot = sw[int(len(sw) * p.S7_BOTTOM_Q)]; s7b = (spread == 7 and mid <= bot)
        else:
            s7b = False
        if s7b:
            hstate.s7_covering = True; hstate.s17_entry_row = None; hstate.s17_entry_mid = None
        headroom = pos_lim + position
        if headroom > 0: orders.append(Order(P, best_bid, -headroom))
        return orders, hstate
    if hstate.s7_covering:
        remaining = p.FLIP_TARGET - position
        if remaining > 0: orders.append(Order(P, best_ask, remaining))
        if position >= p.FLIP_TARGET:
            hstate.s7_covering = False; hstate.flip_holding = True; hstate.flip_entry_row = hstate.row
        return orders, hstate
    if hstate.flip_holding:
        ...
        if mid >= p.FLIP_EXIT_MID or held_for >= p.FLIP_TIMEOUT_TICKS:
            if position > 0: orders.append(Order(P, best_bid, -position))
            hstate.flip_holding = False; hstate.flip_entry_row = None
        return orders, hstate
    ...
    if vfe_mid is not None and len(hstate.vfe_buf) >= 50:
        ...
        vfe_crashing = (vfe_mid - early_avg) < -5.0
    s17_z = None
    if len(hstate.mid_buf_500) >= p.Z500_WINDOW:
        zb = hstate.mid_buf_500[-p.Z500_WINDOW:]
        zmu = sum(zb) / len(zb); zvar = sum((x - zmu) ** 2 for x in zb) / len(zb)
        zsd = zvar ** 0.5 if zvar > 0 else 0
        if zsd > 0: s17_z = (mid - zmu) / zsd
    S17_Z_MIN = 2.0
    if (spread == p.REGIME2_SPREAD and mid > p.ENTRY_MID_MIN
            and (s17_z is None or s17_z >= S17_Z_MIN)
            and hstate.s17_entry_row is None and not vfe_crashing):
        headroom = pos_lim + position; qty = min(pos_lim, headroom)
        if qty > 0:
            orders.append(Order(P, best_bid, -qty))
            hstate.s17_entry_row = hstate.row; hstate.s17_entry_mid = mid
        return orders, hstate
    z_buf = hstate.mid_buf_500[-p.Z_WINDOW:] if len(hstate.mid_buf_500) >= p.Z_WINDOW else None
    z = None
    if z_buf is not None:
        zmu = sum(z_buf) / len(z_buf); zvar = sum((x - zmu) ** 2 for x in z_buf) / len(z_buf)
        zsd = zvar ** 0.5 if zvar > 0 else 0
        if zsd > 0: z = (mid - zmu) / zsd
    if z is not None and abs(z) > p.Z_ENTRY:
        ...
        if z > p.Z_ENTRY:
            target_pos = -target_qty; delta = target_pos - position
            if delta < 0:
                sq = min(-delta, pos_lim + position)
                if sq > 0: orders.append(Order(P, best_bid, -sq))
        else:
            target_pos = target_qty; delta = target_pos - position
            if delta > 0:
                bq = min(delta, pos_lim - position)
                if bq > 0: orders.append(Order(P, best_ask, bq))
        fv = int(round(mid))
        if z > p.Z_ENTRY:
            bh = pos_lim - position; bq = min(p.QUOTE_SIZE, max(0, bh))
            if bq > 0:
                my_bid = max(best_bid + 1, fv - 1); my_bid = min(my_bid, best_ask - 1)
                orders.append(Order(P, my_bid, bq))
        else:
            ah = pos_lim + position; aq = min(p.QUOTE_SIZE, max(0, ah))
            if aq > 0:
                my_ask = min(best_ask - 1, fv + 1); my_ask = max(my_ask, best_bid + 1)
                orders.append(Order(P, my_ask, -aq))
        return orders, hstate
    if z is not None and abs(z) < p.Z_EXIT and abs(position) > p.QUOTE_SIZE:
        if position > 0:
            orders.append(Order(P, best_bid, -min(position, p.QUOTE_SIZE * 2)))
        elif position < 0:
            orders.append(Order(P, best_ask, min(-position, p.QUOTE_SIZE * 2)))
        return orders, hstate
    if len(hstate.edge_buf) >= p.EDGE_BETA_MIN_SAMPLES:
        ex = _mean(hstate.edge_buf) or 0.0; ey = _mean(hstate.ret_buf) or 0.0
    ...
    slack = 1
    fv = int(round(mid))
    bid_headroom = pos_lim - position; ask_headroom = pos_lim + position
    base_bid = min(fv - slack, best_bid + 1); base_ask = max(fv + slack, best_ask - 1)
    my_bid = int(round(base_bid + bid_offset)); my_ask = int(round(base_ask + bid_offset))
    my_bid = max(my_bid, best_bid + 1); my_ask = min(my_ask, best_ask - 1)
    my_bid = min(my_bid, best_ask - 1); my_ask = max(my_ask, best_bid + 1)
    if my_ask <= my_bid: my_ask = my_bid + 1
    bq = min(p.QUOTE_SIZE, max(0, bid_headroom)); aq = min(p.QUOTE_SIZE, max(0, ask_headroom))
    if bq > 0: orders.append(Order(P, my_bid, bq))
    if aq > 0: orders.append(Order(P, my_ask, -aq))
    return orders, hstate
...
V_TTE_DAYS_AT_START = 4.0; V_TTE_YEAR = 250.0
V_INTRINSIC_EDGE = 2; V_INTRINSIC_QTY = 50; V_ARB_SIZE = 10
V_VOUCHER_MM_SIZE = 20; V_VOUCHER_MIN_SPREAD = 2; V_POST_SLACK_VE = 1
V_BS_SIGMA_DEFAULT = 0.18; V_BS_EDGE = 10.0; V_BS_TRADE_SIZE = 30; V_BS_POS_CAP = 150
V_BS_STRIKES = [5000, 5100, 5200, 5300, 5400]
V_IV_ADAPT_WINDOW = 50; V_IV_ADAPT_MIN_HIST = 15


def v_bs_call(spot, K, T, vol):
    if T <= 0 or vol <= 0: return max(spot - K, 0.0)
...
def v_implied_vol(mkt, spot, K, T, lo=1e-4, hi=5.0):
    intr = max(spot - K, 0.0)
    if mkt <= intr + 1e-6 or T <= 0 or mkt >= spot: return None
    for _ in range(50):
        m = 0.5 * (lo + hi)
        if v_bs_call(spot, K, T, m) < mkt: lo = m
        else: hi = m
    return 0.5 * (lo + hi)
...
class VoucherState:
    def __init__(self):
        ...
        self.last_spot = None; self.spot_age = 0
        self.prev_ve_ap1 = None; self.prev_ve_bp1 = None
        self.vfe_momo_buf = []; self.vfe_momo_short_entry = None; self.vfe_momo_fired = False
    def to_dict(self):
        return {"ivh": {str(k): v[-V_IV_ADAPT_WINDOW:] for k, v in self.iv_history.items()},
                "ls": self.last_spot, "sa": self.spot_age,
                "p_ap": self.prev_ve_ap1, "p_bp": self.prev_ve_bp1,
    ...
    def from_dict(d):
        ...
        s.last_spot = d.get("ls"); s.spot_age = d.get("sa", 0)
        s.prev_ve_ap1 = d.get("p_ap"); s.prev_ve_bp1 = d.get("p_bp")
...
VFE_MOMO_BUF = 60; VFE_MOMO_LOOKBACK = 50; VFE_MOMO_THRESH = -3.0
VFE_MOMO_SIZE = 200; VFE_MOMO_TP = 10.0; VFE_MOMO_SL = 15.0


def run_vfe_momentum(state, vstate, *_):
    P = VEVE_SYM
    od = state.order_depths.get(P)
    if not od or not od.buy_orders or not od.sell_orders: return [], 0, 0
    bb = max(od.buy_orders); ba = min(od.sell_orders); mid = (bb + ba) / 2.0
    ...
    if vstate.vfe_momo_short_entry is not None:
        ...
        cover = (total_pnl >= VFE_MOMO_TP * VFE_MOMO_SIZE) or (total_pnl <= -VFE_MOMO_SL * VFE_MOMO_SIZE)
        if cover:
            qty_to_cover = max(0, min(VFE_MOMO_SIZE, 200 - pos))
            if qty_to_cover > 0:
                extra.append(Order(P, ba, qty_to_cover)); eb = qty_to_cover
            vstate.vfe_momo_short_entry = None
        return extra, eb, es
    if (not vstate.vfe_momo_fired and len(vstate.vfe_momo_buf) >= VFE_MOMO_LOOKBACK + 1):
        velocity = mid - vstate.vfe_momo_buf[-VFE_MOMO_LOOKBACK - 1]
        if velocity <= VFE_MOMO_THRESH and pos >= -50:
            target_short = VFE_MOMO_SIZE
            available = 200 + pos
            qty = min(target_short, available)
            if qty > 0:
                extra.append(Order(P, bb, -qty)); es = qty
                vstate.vfe_momo_short_entry = mid; vstate.vfe_momo_fired = True
    return extra, eb, es
...
def run_vouchers(state, vstate):
    """Voucher logic with per-layer ablation for VEV_5300 (target K).
    Layer ablation only applies to VEV_5300 — other strikes unchanged."""
    ABLATE = _ablate_set()
    TARGET_K = 5300
    TARGET_SYM = "VEV_5300"
    orders = {}
    ...
    ve_layer_e: List[Order] = []; layer_e_buy = 0; layer_e_sell = 0
    if od_ve and od_ve.buy_orders and od_ve.sell_orders:
        ...
        E_SIZE = 20; E_POS_CAP = 200
        if (vstate.prev_ve_ap1 is not None and vstate.prev_ve_bp1 is not None):
            if spread_ve == 2 and ba_ve < vstate.prev_ve_ap1:
                room_buy = E_POS_CAP - pos_ve; q = min(E_SIZE, room_buy)
                if q > 0:
                    px = int(ba_ve - 1)
                    if px > bb_ve and px < ba_ve:
                        ve_layer_e.append(Order(VEVE_SYM, px, q)); layer_e_buy = q
            elif spread_ve == 3 and bb_ve > vstate.prev_ve_bp1:
                room_sell = E_POS_CAP + pos_ve; q = min(E_SIZE, room_sell)
                if q > 0:
                    px = int(bb_ve + 1)
                    if px > bb_ve and px < ba_ve:
                        ve_layer_e.append(Order(VEVE_SYM, px, -q)); layer_e_sell = q
        vstate.prev_ve_ap1 = ba_ve; vstate.prev_ve_bp1 = bb_ve

    momo_orders, momo_buy, momo_sell = run_vfe_momentum(state, vstate)

    if od_ve and od_ve.buy_orders and od_ve.sell_orders:
        wm = v_wall_mid(od_ve)
        if wm is not None:
            bb = max(od_ve.buy_orders); ba = min(od_ve.sell_orders)
            pos = state.position.get(VEVE_SYM, 0); fv = round(wm)
            tb = 200 - pos - layer_e_buy - momo_buy
            ts = 200 + pos - layer_e_sell - momo_sell
            half = 100
            mbp = fv - 1 if pos > half else fv
            msp = fv + 1 if pos < -half else fv
            if vstate.vfe_momo_short_entry is not None: tb = 0
        ...
        elif momo_orders: orders[VEVE_SYM] = list(momo_orders)
        elif ve_layer_e: orders[VEVE_SYM] = list(ve_layer_e)
    ...
    if vstate.spot_age > 3: return orders
    ...
    for K in V_BS_STRIKES:
        ...
        if iv is not None:
            ...
            hist.append(iv)
            if len(hist) > V_IV_ADAPT_WINDOW: del hist[:len(hist) - V_IV_ADAPT_WINDOW]
    sigma = v_get_adaptive_sigma(state, vstate)

    # Intrinsic / OTM seller layer
    for K in VOUCHER_STRIKES_ALL:
        if K in VOUCHER_STRIKES_SKIP: continue
        sym = VOUCHER_SYM[K]
        # ABLATION
        if K == TARGET_K and "intrinsic" in ABLATE: continue
        od_v = state.order_depths.get(sym)
        ...
        tb = 300 - pos_v - already_buy; ts = 300 + pos_v - already_sell
        intrinsic = max(spot - K, 0.0); deep_itm = K in VOUCHER_STRIKES_DEEP_ITM
        new_orders = []
        ...
        if new_orders: orders.setdefault(sym, []).extend(new_orders)

    # BS taking layer
    for K in V_BS_STRIKES:
        sym = VOUCHER_SYM[K]
        # ABLATION
        if K == TARGET_K and "bs" in ABLATE: continue
        od_v = state.order_depths.get(sym)
        if od_v is None or not od_v.buy_orders or not od_v.sell_orders: continue
        pos_v = state.position.get(sym, 0)
        existing = orders.get(sym, [])
        already_buy = sum(o.quantity for o in existing if o.quantity > 0)
        already_sell = sum(-o.quantity for o in existing if o.quantity < 0)
        cur_pos_buy = pos_v + already_buy; cur_pos_sell = pos_v - already_sell
        fv_bs = v_bs_call(spot, K, T, sigma)
        edge = 5.0 if K == 5200 else V_BS_EDGE
        buy_thr = fv_bs - edge; sell_thr_bs = fv_bs + edge
        new_orders = []
    ...
    for K_lo, K_hi in itertools.combinations(VOUCHER_STRIKES_ALL, 2):
        ...
        # ABLATION (any pair touching 5300)
        if (K_lo == TARGET_K or K_hi == TARGET_K) and "arb" in ABLATE: continue
        sym_lo = VOUCHER_SYM[K_lo]; sym_hi = VOUCHER_SYM[K_hi]
        od_lo = state.order_depths.get(sym_lo); od_hi = state.order_depths.get(sym_hi)
        ...
        if od_lo.sell_orders and od_hi.buy_orders:
            ...
            if ask_lo - bid_hi < 0:
                ask_lo_vol = -od_lo.sell_orders[ask_lo]; bid_hi_vol = od_hi.buy_orders[bid_hi]
                ...
                if size > 0:
                    ...
                    committed_buy[sym_lo] += size; committed_sell[sym_hi] += size

    # Passive voucher MM
    for K in VOUCHER_STRIKES_TRADEABLE:
        sym = VOUCHER_SYM[K]
        # ABLATION
        if K == TARGET_K and "mm" in ABLATE: continue
        od_v = state.order_depths.get(sym)
        if od_v is None or not od_v.buy_orders or not od_v.sell_orders: continue
        ...
        tb_v = 300 - pos_v - existing_buy; ts_v = 300 + pos_v - existing_sell
        if tb_v > 0:
    ...
    # Deep ITM
    DEEP_ITM_MM_SIZE = 30; DEEP_ITM_POS_CAP = 100
    DEEP_ITM_BID_OFFSET = 1; DEEP_ITM_ASK_OFFSET = 1
    for K in VOUCHER_STRIKES_DEEP_ITM:
        ...
        if room_buy > 0:
            ...
            bid_px = min(target_bid, vbb + 1); bid_px = max(1, bid_px)
            if bid_px <= vba - 1:
    ...
    # OTM passive bid
    OTM_BID_SIZE = 5; OTM_BID_POS_CAP = 50; OTM_BID_EDGE = 2
    for K in VOUCHER_STRIKES_OTM_PASSIVE:
        sym = VOUCHER_SYM[K]
        # ABLATION
        if K == TARGET_K and "otm" in ABLATE: continue
        od_v = state.order_depths.get(sym)
        if od_v is None or not od_v.buy_orders or not od_v.sell_orders: continue
    ...
    for K in [6000, 6500]:
        ...
        if room > 0:
            orders.setdefault(sym, []).append(Order(sym, 0, min(100, room)))
