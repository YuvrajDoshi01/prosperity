...
# v4_voucher_scale: per-strike MM size override; falls back to V_VOUCHER_MM_SIZE
V_VOUCHER_MM_SIZE_PER_K = {
    5000: 20,
    5100: 20,
    5200: 50,
    5300: 50,
...
# Strikes to actually post passive 2-sided MM on (subset of TRADEABLE)
V_VOUCHER_MM_STRIKES = [5000, 5100, 5200, 5300, 5400]
V_VOUCHER_MIN_SPREAD = 2
V_POST_SLACK_VE = 1
V_BS_SIGMA_DEFAULT = 0.18
V_BS_EDGE = 10.0
# Per-strike BS edge override (smaller edge = more taking, more risk)
V_BS_EDGE_PER_K = {
    5200: 5.0,   # carried from v3
    5300: 7.0,   # winner — slight tighten
}
V_BS_TRADE_SIZE = 30
V_BS_POS_CAP = 150
...
def run_vouchers(state, vstate):
    ...
    for K in V_BS_STRIKES:
        ...
        # v4 Agent 6: per-strike edge override (defaults to V_BS_EDGE)
        edge = V_BS_EDGE_PER_K.get(K, V_BS_EDGE)
        buy_thr = fv_bs - edge
    ...
    for K in V_VOUCHER_MM_STRIKES:
        ...
        mm_size = V_VOUCHER_MM_SIZE_PER_K.get(K, V_VOUCHER_MM_SIZE)
        if tb_v > 0:
