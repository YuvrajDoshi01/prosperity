"""r4_v4_defense.py — R4 v3 + early-regime DEFENSE flag.

NEW (vs v3):
  At tick 50 (timestamp 5000), compute composite score:
    score = (VFE_init - 5280)/15 + (0.55 - vol_ratio)/0.30
    where vol_ratio = pstdev(VFE_mid_first_50) / pstdev(HP_mid_first_50).
  If score > THRESH (default 0.0): set DEFENSE flag for the rest of day.
  When DEFENSE active:
    - HP Z_MAX_POS 120 -> 60
    - HP FLIP_TARGET 200 -> 100
    - Voucher BS_EDGE += 4 (effectively V_BS_EDGE 10 -> 14)
  THRESH override via R4_DEFENSE_THRESH env var for grid testing.
...
# Threshold for DEFENSE composite score; override via R4_DEFENSE_THRESH env var
# (used during BT grid sweep). Production default 0.0 (separates day 3 from days 1/2).
try:
    DEFENSE_THRESH = float(os.environ.get("R4_DEFENSE_THRESH", "0.0"))
except Exception:
    DEFENSE_THRESH = 0.0

# DEFENSE param adjustments
DEFENSE_Z_MAX_POS    = 60      # was 120
DEFENSE_FLIP_TARGET  = 100     # was 200
DEFENSE_BS_EDGE_BUMP = 4.0     # added to V_BS_EDGE
...
def run_hydrogel(state, hstate, defense: bool = False):
    ...
    orders = []
    p      = HydrogelParams
    # DEFENSE param overrides (used as locals in lieu of mutating class attributes)
    z_max_pos = DEFENSE_Z_MAX_POS if defense else p.Z_MAX_POS
    flip_target = DEFENSE_FLIP_TARGET if defense else p.FLIP_TARGET
    ...
    if hstate.s7_covering:
        remaining = flip_target - position
        if remaining > 0:
    ...
    if z is not None and abs(z) > p.Z_ENTRY:
        ...
        target_qty = min(target_qty, z_max_pos)
...
def run_vouchers(state, vstate, defense: bool = False):
    orders = {}
    timestamp = state.timestamp
    # DEFENSE: tighten BS quotes by widening edge
    bs_edge_local = V_BS_EDGE + (DEFENSE_BS_EDGE_BUMP if defense else 0.0)
    ...
    for K in V_BS_STRIKES:
        ...
        # DEFENSE: bump 5200 too (5.0 -> 9.0) when defense active
        edge = (5.0 + (DEFENSE_BS_EDGE_BUMP if defense else 0.0)) if K == 5200 else bs_edge_local
        buy_thr = fv_bs - edge
...
class Trader:
    def run(self, state: TradingState) -> Tuple[Dict[Symbol, List[Order]], int, str]:
        ...
        # ── DEFENSE flag (early-regime gate) ──────────────────────────────────
        # Capture VFE/HP mids for the first 50 ticks; at tick 50 (timestamp 5000)
        # compute composite score; if score > THRESH, latch DEFENSE for the day.
        defense = bool(raw.get("def_flag", False))
        regime_locked = bool(raw.get("def_locked", False))
        vfe_first50 = list(raw.get("def_vfe50", []))
        hp_first50  = list(raw.get("def_hp50",  []))
        first_vfe_mid = raw.get("def_vfe0")
        defense_score = raw.get("def_score")

        if not regime_locked:
            od_ve = state.order_depths.get(Product.VELVETFRUIT_EXTRACT)
            od_hp = state.order_depths.get(Product.HYDROGEL_PACK)
            if (od_ve and od_ve.buy_orders and od_ve.sell_orders
                    and od_hp and od_hp.buy_orders and od_hp.sell_orders):
                vmid = (max(od_ve.buy_orders) + min(od_ve.sell_orders)) / 2.0
                hmid = (max(od_hp.buy_orders) + min(od_hp.sell_orders)) / 2.0
                if first_vfe_mid is None:
                    first_vfe_mid = vmid
                if len(vfe_first50) < 50:
                    vfe_first50.append(vmid)
                if len(hp_first50) < 50:
                    hp_first50.append(hmid)
            # Lock at tick 50: compute score and latch DEFENSE for rest of day
            if (len(vfe_first50) >= 50 and len(hp_first50) >= 50
                    and first_vfe_mid is not None):
                try:
                    vsd = pstdev(vfe_first50)
                    hsd = pstdev(hp_first50)
                    vol_ratio = vsd / max(hsd, 1e-3)
                    defense_score = (first_vfe_mid - 5280) / 15.0 + (0.55 - vol_ratio) / 0.30
                    defense = defense_score > DEFENSE_THRESH
                except Exception:
                    defense = False
                    defense_score = None
                regime_locked = True

        raw["def_flag"]   = defense
        raw["def_locked"] = regime_locked
        raw["def_vfe50"]  = vfe_first50
        raw["def_hp50"]   = hp_first50
        raw["def_vfe0"]   = first_vfe_mid
        raw["def_score"]  = defense_score

        hstate = HydrogelState.from_dict(raw.get("hg", {}))
        hydrogel_orders, hstate = run_hydrogel(state, hstate, defense=defense)
        orders[Product.HYDROGEL_PACK] = hydrogel_orders
        raw["hg"] = hstate.to_dict()

        vstate = VoucherState.from_dict(raw.get("v9", {}))
        voucher_orders = run_vouchers(state, vstate, defense=defense)
