"""r4_v4_circuit.py — v3 + S17 circuit breaker (Agent 21).

CIRCUIT BREAKER (NEW vs v3):
  - HydrogelState.s17_failed_count tracks FLIP_HOLD timeout exits
  - Increments only on TIMEOUT exit (mid never recovered to FLIP_EXIT_MID)
  - S17 entry condition gated on s17_failed_count < S17_MAX_FAILS
  - Bounds worst-case loss to S17_MAX_FAILS x ~$11k per failure

Persists across run() calls via to_dict/from_dict.
