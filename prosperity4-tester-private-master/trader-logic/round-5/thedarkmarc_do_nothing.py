"""thedarkmarc_do_nothing.py — pasted from Discord chat 2026-04-29.

Author: thedarkmarc (teammate). Ported R1 ASH/PEPPER template structure to R5,
trades only 6 of 50 products with handcrafted strategy types. Reported live
day-1/2/3 PnL: 69k / 31k / 49k = ~$149k 3-day.

Targeted products (others skipped):
  MEAN_REVERSION:  SNACKPACK_CHOCOLATE, ROBOT_IRONING,
                   OXYGEN_SHAKE_EVENING_BREATH, OXYGEN_SHAKE_CHOCOLATE
  MOMENTUM:        PEBBLES_XL
  HIDDEN_LIQUIDITY: ROBOT_LAUNDRY

Note: pepper_fair_value_long_biased and ash_fair_value_and_width are
dead code (Product.PEPPER / Product.ASH don't exist as R5 attrs); kept
for fidelity to the pasted file but never invoked by run().
"""
...
class Trader:
    ...
    def generic_fair_value_and_width(
        ...
        dynamic_width = max(1.0, spread / 2.0)

        if strategy_type == "MEAN_REVERSION":
            expected_mid = mid - 0.25 * last_ret
    ...
    def run(self, state: TradingState) -> Tuple[Dict[Symbol, List[Order]], int, str]:
        ...
        for product in state.order_depths.keys():
            if product in [Product.SNACKPACK_CHOCOLATE, Product.ROBOT_IRONING, Product.OXYGEN_SHAKE_EVENING_BREATH, Product.OXYGEN_SHAKE_CHOCOLATE]:
                strategy_type = "MEAN_REVERSION"
            elif product == Product.PEBBLES_XL:
                strategy_type = "MOMENTUM"
            elif product == Product.ROBOT_LAUNDRY:
            ...
            take_orders, buy_vol, sell_vol = self.take_orders(
                product, state.order_depths[product], fair_value, dynamic_width, position
            )
