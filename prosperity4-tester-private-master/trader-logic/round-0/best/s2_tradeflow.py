...
"""
s2_tradeflow: Microprice Regression + Trade Flow Signal

Base: 6913.py microprice regression (2,644 on website)
NEW: Trade flow from market_trades (r=-0.54 with future returns, currently unused)

When recent trades skew buy-heavy → price tends to revert DOWN → nudge fair down.
When recent trades skew sell-heavy → price tends to revert UP → nudge fair up.
"""
...
class TomatoesStrategy(MarketMakingStrategy):
    ...
    def compute_trade_flow(self, state: TradingState) -> float:
        """Compute signed trade flow from market_trades. Returns [-1, 1]."""
        ...
        if not trades:
            ...
            signed_vol = 0.0
            for trade in trades:
                # Lee-Ready: trade at/above mid = buy-initiated, below = sell-initiated
    ...
    def get_true_value(self, state: TradingState) -> int:
        ...
        # Base fair value from regression
        if len(self.cache) == self.cache_dim:
        ...
        # Trade flow adjustment: counter-flow (r=-0.54 means buy flow predicts down)
