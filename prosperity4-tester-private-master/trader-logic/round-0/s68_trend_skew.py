...
"""
s68_trend_skew — s58 base + trend-filtered market making

From the residual regression analysis:
- FV = EMA + predicted_residual is WORSE than raw microprice regression (RMSE +0.002-0.005)
- The raw regression already forms an implicit adaptive anchor
- So we KEEP the proven s58 FV engine

What we ADD from the trend framework:
1. Trend detection via EMA slope (not level)
   - slope = (EMA_fast - EMA_slow) / normalization
   - Positive slope = uptrend, negative = downtrend

2. Trend-skewed posting:
   - In downtrend: widen bid (don't catch falling knives), tighten ask (welcome sells)
   - In uptrend: tighten bid (welcome buys), widen ask (don't sell too cheap)
   - This replaces the fixed carry_wide_offset with a dynamic one

3. Trend-skewed position limits:
   - In downtrend with long position: reduce buy capacity (don't add to losers)
   - In uptrend with short position: reduce sell capacity

This addresses the "bots die by fighting the trend" problem without changing the FV engine.
"""
...
# === NEW: Trend detection ===
EMA_FAST_ALPHA = 0.1     # ~20 period EMA
EMA_SLOW_ALPHA = 0.02    # ~100 period EMA
TREND_NORM = 2.5          # normalize slope: 2.5-point EMA diff = full trend signal
                          # Swept 2.0-10.0: 2.5 is best total (d0 +12, d-1 +349)
# trend_signal: +1 = strong uptrend, -1 = strong downtrend, 0 = flat

# === NEW: Trend-skewed posting ===
# When trending, widen the contra-trend post and tighten the with-trend post
TREND_WIDE_OFFSET = 5     # offset for contra-trend post (normally 3)
TREND_TIGHT_OFFSET = 1    # offset for with-trend post (normally 1 from best±1)
# Only apply trend skew when trend is strong enough
TREND_SKEW_THRESHOLD = 0.3  # |trend_signal| must exceed this

# === Trend-skewed capacity (DISABLED — zero effect on all tested days) ===
TREND_CAP_POS_THRESHOLD = 999  # disabled: never triggers
TREND_CAP_SCALE = 0.5          # would multiply contra-trend capacity by this
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # ═══════════════════════════════════════════════════
        # TOMATOES — s58 FV + trend-skewed posting
        # ═══════════════════════════════════════════════════
        if "TOMATOES" in state.order_depths:
            ...
            if book.buy_orders and book.sell_orders:
                ...
                if self.ema_fast is None:
                    ...
                    self.ema_fast = EMA_FAST_ALPHA * mid + (1 - EMA_FAST_ALPHA) * self.ema_fast
                    self.ema_slow = EMA_SLOW_ALPHA * mid + (1 - EMA_SLOW_ALPHA) * self.ema_slow

                # Trend signal: positive = uptrend, negative = downtrend
                trend_signal = (self.ema_fast - self.ema_slow) / TREND_NORM
                ...
                # Trend-skewed capacity: when deeply positioned against trend, reduce adds
                if abs(pos) > TREND_CAP_POS_THRESHOLD and abs(trend_signal) > TREND_SKEW_THRESHOLD:
                    if pos > 0 and trend_signal < -TREND_SKEW_THRESHOLD:
                        # Long in downtrend — reduce buying
                        buy_capacity = int(buy_capacity * TREND_CAP_SCALE)
                    elif pos < 0 and trend_signal > TREND_SKEW_THRESHOLD:
                        # Short in uptrend — reduce selling
                        sell_capacity = int(sell_capacity * TREND_CAP_SCALE)

                buy_capacity = max(0, buy_capacity)
                sell_capacity = max(0, sell_capacity)

                # --- Phase 1: Takes at fair value (unchanged) ---
                ...
                # --- Phase 2: Trend-skewed posting ---
                # Determine bid/ask offsets based on trend
                if abs(trend_signal) > TREND_SKEW_THRESHOLD:
                    if trend_signal > 0:
                        # Uptrend: tight bid (catch dips), wide ask (don't sell cheap)
                        bid_offset = TREND_TIGHT_OFFSET
                        ask_offset = TREND_WIDE_OFFSET
                    else:
                        # Downtrend: wide bid (avoid falling knives), tight ask
                        bid_offset = TREND_WIDE_OFFSET
                        ask_offset = TREND_TIGHT_OFFSET
                else:
                    # No trend: normal symmetric posting
                    bid_offset = 1
                    ask_offset = 1

                if self.carry_signal > CARRY_THRESHOLD:
                    # Carry says expect DOWN → tight bid, wide ask
                    if buy_capacity > 0:
                    ...
                    if sell_capacity > 0:
                        if pos >= TOMATO_POST_SKEW_THRESHOLD:
                            ...
                            # Use trend-adjusted offset instead of fixed CARRY_WIDE_OFFSET
                            wide = max(CARRY_WIDE_OFFSET, ask_offset)
                            ask_price = max(fair_value_int + wide, best_ask - 1)
                        ask_price = max(ask_price, best_bid + 1)
                        tom_orders.append(Order("TOMATOES", ask_price, -sell_capacity))

                elif self.carry_signal < -CARRY_THRESHOLD:
                    # Carry says expect UP → wide bid, tight ask
                    if sell_capacity > 0:
                        ask_price = max(fair_value_int + 1, best_ask - 1)
                        ask_price = max(ask_price, best_bid + 1)
                        tom_orders.append(Order("TOMATOES", ask_price, -sell_capacity))
                    if buy_capacity > 0:
                        if pos <= -TOMATO_POST_SKEW_THRESHOLD:
                            ...
                            wide = max(CARRY_WIDE_OFFSET, bid_offset)
                            bid_price = min(fair_value_int - wide, best_bid + 1)
                        bid_price = min(bid_price, best_ask - 1)
                        tom_orders.append(Order("TOMATOES", bid_price, buy_capacity))

                else:
                    # No carry signal — use trend-skewed offsets
                    if buy_capacity > 0:
                        bid_price = min(fair_value_int - bid_offset, best_bid + 1)
                        bid_price = min(bid_price, best_ask - 1)
                        tom_orders.append(Order("TOMATOES", bid_price, buy_capacity))
                    if sell_capacity > 0:
                        ask_price = max(fair_value_int + ask_offset, best_ask - 1)
                        ask_price = max(ask_price, best_bid + 1)
                        tom_orders.append(Order("TOMATOES", ask_price, -sell_capacity))
        ...
        return result, conversions, json.dumps(
            {"mp": self.microprice_history,
             ...
             "ef": round(self.ema_fast, 2) if self.ema_fast else None,
             "es": round(self.ema_slow, 2) if self.ema_slow else None},
            separators=(",", ":")
        )
