...
"""
s61_gap_post — s36 base + gap asymmetry for POSTING DIRECTION ONLY

gap_asym = (bid1 - bid2) - (ask2 - ask1)
  -1: weak UP (60%, ~2100 ticks/10k day)
  +1: weak DOWN (60%, ~2030 ticks/10k day)
  <= -4: strong UP (90-100%, ~60 ticks/10k day)
  >= +3: strong DOWN (97-100%, ~60 ticks/10k day)

Takes remain IDENTICAL to s36. Only posting direction changes.
When gap_asym = -1: post like carry_UP (tight bid, wide ask)
When gap_asym = +1: post like carry_DOWN (wide bid, tight ask)
When |gap_asym| extreme: same but stronger

This is the safest L2 price signal exploitation:
  - No take changes (can't hurt fill quality)
  - Pure price data (no volume dependency)
  - Fires on 40% of ticks (much more frequent than carry's 7%)
"""
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # ═══════════════════════════════════════════════════
        # TOMATOES — s36 takes + gap asymmetry posting
        # ═══════════════════════════════════════════════════
        if "TOMATOES" in state.order_depths:
            ...
            if book.buy_orders and book.sell_orders:
                ...
                # --- Fair value (IDENTICAL to s36, no L2 FV shift) ---
                ...
                # --- s61: Compute gap asymmetry for posting ---
                gap_asym = 0
                ...
                # --- Phase 1: Takes (IDENTICAL to s36) ---
                ...
                # --- Phase 2: Posting ---
                # Priority: gap_asym signal > carry signal > neutral
                # gap_asym fires on ~40% of ticks; carry on ~7%
                # When both fire, they agree 91% of the time
                # Use gap_asym when available, fall back to carry

                expect_up = False
                expect_down = False

                if gap_asym <= -1:
                    # Negative gap_asym → expect UP
                    expect_up = True
                elif gap_asym >= 1:
                    # Positive gap_asym → expect DOWN
                    expect_down = True
