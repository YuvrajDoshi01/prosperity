...
"""
s52_mid_fv — s36 base but use PURE MID as FV instead of microprice regression

Idea: The microprice regression uses volumes (which differ 98.5% between CSV
and website). What if we drop microprice entirely and use mid price?

mid = (best_bid + best_ask) / 2  — purely price-based, NO volume dependency.

Keep trade flow and carry signal (both price-based).
Drop OBI shift (volume-based).
FV = mid + trade_flow_adjustment

This is the MOST volume-independent strategy possible while keeping
the carry/flow framework.
"""
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # ═══════════════════════════════════════════════════
        # TOMATOES — Pure mid FV (no microprice, no OBI)
        # ═══════════════════════════════════════════════════
        if "TOMATOES" in state.order_depths:
            ...
            if book.buy_orders and book.sell_orders:
                ...
                # FV = mid (no microprice regression, no OBI — pure price-based)
                fair_value = mid

                # Trade flow adjustment (price-based, NOT volume-dependent)
