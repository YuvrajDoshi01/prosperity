...
"""
anchored_quoting_combined_asymmetric — L1+L2 combined regression + asymmetric move classifier

Combines the two best independent findings:
1. Combined 8-lag regression (L1+L2): OOS R²=0.9725 vs L1-only 0.9631
2. Asymmetric move classifier: +296 TOMATOES PnL on day 0

The two signals are ORTHOGONAL:
- The regression uses microprice LEVELS (volume-weighted price averages)
- The asymmetric classifier uses price CHANGES (which side moved)
  When bid moves up alone → next dmid = -0.55 (structural reversal)
  When ask moves down alone → next dmid = +0.57 (structural reversal)
  These carry 5x more predictive info than symmetric moves

Original carry params (trigger=4, decay=0.7, width=3) — website-proven.
"""
...
# --- Original carry signal (website-proven) ---
CARRY_TRIGGER = 4
CARRY_DECAY = 0.7
CARRY_THRESHOLD = 0.5
CARRY_WIDE_OFFSET = 3
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # ──────────────────────────────────────────────────────────────────────
        # TOMATOES: Combined L1+L2 Regression + Asymmetric Move + Original Carry
        # ──────────────────────────────────────────────────────────────────────
