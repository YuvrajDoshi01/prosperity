...
"""
s50_post_skew — s36 base + post-only inventory skew

Idea: Shift POSTING prices by -gamma * position to mean-revert inventory.
Takes remain at true FV (no spread capture lost).
When long: post_fv shifts down → lower ask attracts sells, lower bid avoids buys.
When short: post_fv shifts up → higher bid attracts buys, higher ask avoids sells.

BT sweep showed gamma=0.01 was best (+73 vs s49 base).
Rebuilt on s36 base (NO take rounding, proven website 2,896).
"""
...
# s50: Post-only inventory skew
POST_INVENTORY_GAMMA = 0.01
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # ═══════════════════════════════════════════════════
        # TOMATOES — s36 + post-only inventory skew
        # ═══════════════════════════════════════════════════
        if "TOMATOES" in state.order_depths:
            ...
            if book.buy_orders and book.sell_orders:
                ...
                # TAKE FV: standard round (identical to s36)
                fair_value_int = round(fair_value)

                # POST FV: shifted by -gamma * position
                post_fv_int = round(fair_value - POST_INVENTORY_GAMMA * pos)
                ...
                # Phase 1: Takes at TRUE fair_value_int (same as s36)
                ...
                # Phase 2: Posting at post_fv_int (inventory-adjusted)
