...
"""
s1_tradeflow_regime: s2_tradeflow (2,851 on website) + Regime-Switching Signal

When spread tightens to 5-9 (7.2% of time), the bot has DIRECTIONAL BIAS:
  Spread 5: +4.0 (strong UP)   — verified BOTH days
  Spread 6: -1.8 (DOWN)        — verified BOTH days
  Spread 7: +2.1 (strong UP)   — verified BOTH days
  Spread 8: -1.2 (DOWN)        — verified BOTH days
  Spread 9: -2.5 (strong DOWN) — verified BOTH days

We shift fair value in the predicted direction during these windows.
"""

# Spread-conditional expected returns (verified on day -2 AND day -1)
SPREAD_DIRECTION = {5: +4.0, 6: -1.8, 7: +2.1, 8: -1.2, 9: -2.5}
REGIME_SCALE = 0.5  # Conservative: use half the expected move
...
class TomatoesStrategy(MarketMakingStrategy):
    ...
    def get_true_value(self, state: TradingState) -> int:
        ...
        # Signal 1: Trade flow (proven +207 PnL on website)
        flow_signal = self.compute_trade_flow(state)
        fv -= flow_signal * 1.5

        # Signal 2: Regime switching (spread-conditional directional bias)
        if spread in SPREAD_DIRECTION:
            fv += SPREAD_DIRECTION[spread] * REGIME_SCALE
