...
class Trader:
    ...
    def run(self, state: TradingState):
            ...
            if od.buy_orders and od.sell_orders:
                ...
                if len(bids_s) >= 2 and len(asks_s) >= 2:
                    ...
                    if (bw + aw) > 0:
                        fv += ((bw - aw) / (bw + aw)) * 0.3
                    # 2. Gap asymmetry
                    fv -= ((bids_s[0][0] - bids_s[1][0]) - (asks_s[1][0] - asks_s[0][0])) * 0.05
                    # 3. EMA deviation
                    mid = (bb + ba) * 0.5
                    spread = ba - bb
                    alpha = 0.5
                    if self.ema3 is None: self.ema3 = mid
                    else: self.ema3 = alpha * mid + (1 - alpha) * self.ema3
                    if spread > 0:
                        fv -= ((mid - self.ema3) / spread) * 0.2
        ...
        return orders, 0, json.dumps({"c":self.tc,"f":self.tf,"tw":self.tw,"ew":self.ew,"e3":self.ema3}, separators=(",",":"))
