    ...
    def run(self, state: TradingState):
        ...
        if "TOMATOES" in state.order_depths:
            ...
            if od.buy_orders and od.sell_orders:
                ...
                # Distance-weighted microprice (replaces L1-only microprice)
                ...
                if len(bids_s) >= 2 and len(asks_s) >= 2:
                    ...
                    aw = av1_l + av2_l * 0.5
                    if (bw + aw) > 0:
                        bid_avg = (bids_s[0][0] * bv1_l + bids_s[1][0] * bv2_l * 0.5) / bw
                        ask_avg = (asks_s[0][0] * av1_l + asks_s[1][0] * av2_l * 0.5) / aw
                        mp = bid_avg + (bw / (bw + aw)) * (ask_avg - bid_avg)
                    else:
                        mp = (bb + ba) * 0.5
                else:
