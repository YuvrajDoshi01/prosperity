...
"""
s18_partial_clear: s2_tradeflow + Linear Utility-style PARTIAL position clearing

Linear Utility (2nd place P2) says clearing boosts PnL by ~3%.
Our s13 (full clear) scored 2,077 = catastrophic. That's because we cleared
the ENTIRE position at fair value, wasting all capacity on zero-edge trades.

Linear Utility's approach: clear only enough to free capacity for the NEXT take.
After taking, if position is large, post a small clear order at fair value.
This reduces position risk without sacrificing all making capacity.

Rule: if |effective_position| > 30 after taking, clear 25% at fair value.
Remaining capacity goes to making at best±1 (proven optimal).
"""
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # ═══ TOMATOES (tradeflow + PARTIAL clear) ═══
        if "TOMATOES" in state.order_depths:
            ...
            if od.buy_orders and od.sell_orders:
                ...
                # STEP 2: PARTIAL CLEAR (Linear Utility style)
                # Only clear if position is large (>30), and only clear 25%
                filled_buy = (80 - pos) - tb
                filled_sell = (80 + pos) - ts
                eff_pos = pos + filled_buy - filled_sell

                if eff_pos > 30 and ts > 0:
                    clear_qty = min(max(1, eff_pos // 4), ts)  # 25% of position
                    to.append(Order("TOMATOES", tv, -clear_qty))
                    ts -= clear_qty
                elif eff_pos < -30 and tb > 0:
                    clear_qty = min(max(1, abs(eff_pos) // 4), tb)
                    to.append(Order("TOMATOES", tv, clear_qty))
                    tb -= clear_qty

                # STEP 3: MAKE (remaining capacity at best±1)
                if tb > 0 and hard:
