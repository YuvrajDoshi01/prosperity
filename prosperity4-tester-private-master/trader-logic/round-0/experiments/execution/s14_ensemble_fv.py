...
"""
s14_ensemble_fv: Ensemble fair value for TOMATOES.

Average 3 FV estimators:
  1. Microprice 4-lag regression (proven, weight 0.6)
  2. Wall mid (structural, weight 0.25)
  3. Simple mid (weight 0.15)
Trade flow applied on top of ensemble FV.
EMERALDS identical to s2_speed_flat.
"""
    ...
    def run(self, state: TradingState):
        ...
        if "TOMATOES" in state.order_depths:
            ...
            if od.buy_orders and od.sell_orders:
                ...
                # 1. Microprice regression FV
                if len(c) == 4:
                    fv_reg = 2.208667 + 0.059694*c[0] + 0.117270*c[1] + 0.244154*c[2] + 0.578440*c[3]
                else:
                    fv_reg = mp

                # 2. Wall mid (largest volume levels)
                if len(od.buy_orders) >= 2 and len(od.sell_orders) >= 2:
                    wall_bid = max(od.buy_orders.items(), key=lambda x: x[1])[0]
                    wall_ask = max(od.sell_orders.items(), key=lambda x: abs(x[1]))[0]
                    fv_wall = (wall_bid + wall_ask) / 2
                else:
                    fv_wall = (bb + ba) / 2
                ...
                fv = 0.6 * fv_reg + 0.25 * fv_wall + 0.15 * fv_mid
