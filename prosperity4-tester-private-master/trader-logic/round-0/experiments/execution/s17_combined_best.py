...
"""
s17_combined_best: Best proven changes combined.

TOMATOES: microprice regression + trade flow (proven 2,851 base)
          + position aggression at 40 (proven +175)
          + liquidation tracking
EMERALDS: fair=10000, NO position aggression for taking (from s1_resting, proven +6)
          + liquidation tracking (proven +120)
          = takes at tv=10000 regardless of position

Only difference from s2_speed_flat: EMERALDS takes at tv=10000 with no mbp/msp
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # === EMERALDS (no position aggression for taking) ===
        if "EMERALDS" in state.order_depths:
            ...
            if od.buy_orders and od.sell_orders:
                ...
                # NO position aggression: take at tv=10000 regardless of position
                for p, v in sells:
                    if tb > 0 and p <= tv:
                ...
                if tb > 0:
                    eo.append(Order("EMERALDS", min(tv, buys[0][0] + 1), tb))
                ...
                if ts > 0:
                    eo.append(Order("EMERALDS", max(tv, sells[0][0] - 1), -ts))

                orders["EMERALDS"] = eo

        # === TOMATOES (identical to s2_speed_flat, with position aggression) ===
