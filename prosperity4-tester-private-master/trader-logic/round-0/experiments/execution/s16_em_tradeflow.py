...
"""
s16_em_tradeflow: Trade flow signal added to EMERALDS.

EMERALDS change: apply trade flow (window 5, coef 1.0, norm 15) to nudge
fair value away from 10000. Smaller coefficient than TOMATOES since
EMERALDS is a stable product.
TOMATOES identical to s2_speed_flat.
"""
...
class Trader:
    def __init__(self):
        ...
        self.ef = []    # EMERALDS trade flow history (max 5)
    ...
    def run(self, state: TradingState):
        ...
        if "EMERALDS" in state.order_depths:
            ...
            if od.buy_orders and od.sell_orders:
                ...
                # Trade flow for EMERALDS
                em_trades = state.market_trades.get("EMERALDS")
                if em_trades:
                    em_mid = (max(od.buy_orders.keys()) + min(od.sell_orders.keys())) / 2
                    em_sv = sum(t.quantity if t.price >= em_mid else -t.quantity for t in em_trades)
                    self.ef.append(em_sv)
                else:
                    self.ef.append(0.0)
                if len(self.ef) > 5: self.ef = self.ef[-5:]
                em_fs = max(-1.0, min(1.0, sum(self.ef) / 15.0))
                tv = round(10000 - em_fs * 1.0)
        ...
        return orders, 0, json.dumps({"c":self.tc,"f":self.tf,"tw":self.tw,"ew":self.ew,"ef":self.ef}, separators=(",",":"))
