...
"""
s3_tradeflow_nodrift: s2_tradeflow but with intercept=0 (no drift assumption)

The 2.21 drift intercept was fit to training data. EDA shows drift direction
REVERSES between days (+6.5 on day-2 vs -49 on day-1). Removing it should
help generalization to day 0.

Everything else identical to s2_tradeflow (2,851 on website).
"""
...
class TomatoesStrategy(MarketMakingStrategy):
    ...
    def get_true_value(self, state: TradingState) -> int:
        ...
        # Regression with ZERO intercept (no drift assumption)
        if len(self.cache) == self.cache_dim:
            coef = [0.059694, 0.117270, 0.244154, 0.578440]
            fv = 0.0  # NO DRIFT INTERCEPT (was 2.208667)
            for i, val in enumerate(self.cache):
                fv += val * coef[i]
        else:
            fv = microprice

        # Trade flow (same as s2_tradeflow)
