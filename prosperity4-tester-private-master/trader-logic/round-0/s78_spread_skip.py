...
"""
s78_spread_skip — s36 base + skip posting during tight spreads

Change: When spread <= TIGHT_SPREAD_THRESHOLD, don't post (let MM bot take
the low-edge fills). Only post during wide spreads where edge is 2.5x higher.

  spread <= 9 (tight, 7.2% of ticks):  take at FV only, NO posting
  spread > 9 (wide, 92.8% of ticks):   normal s36 posting

Takes at FV always happen regardless of spread state.
"""
...
# === Spread skip — the ONLY addition ===
TIGHT_SPREAD_THRESHOLD = 9  # Skip posting when spread <= this
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # ═══════════════════════════════════════════════════
        # TOMATOES — s36 core + spread-conditional posting
        # ═══════════════════════════════════════════════════
        if "TOMATOES" in state.order_depths:
            ...
            if book.buy_orders and book.sell_orders:
                ...
                # Phase 1: Take at fair value (ALWAYS, regardless of spread)
                ...
                # Phase 2: Post ONLY during wide spreads
                if spread > TIGHT_SPREAD_THRESHOLD:
                    if self.carry_signal > CARRY_THRESHOLD:
                        ...
                        if sell_capacity > 0:
                            ...
                            ask_price = max(ask_price, best_bid + 1)
                            tom_orders.append(Order("TOMATOES", ask_price, -sell_capacity))
                # else: tight spread — no posting, just takes
