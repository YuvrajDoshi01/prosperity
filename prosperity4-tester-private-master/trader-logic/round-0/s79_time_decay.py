...
"""
s79_time_decay — s36 base + time-decay aggression curve

Change: Post tighter (more aggressive FV, closer to mid) early in the day
when inventory has time to mean-revert. Gradually widen posting as time
progresses and end-of-day MTM risk increases.

  t < RAMP_START:  normal s36 (no change)
  RAMP_START < t < TERMINAL:  linearly widen posting offset from 0 to MAX_WIDEN
  t > TERMINAL:  terminal flatten (identical to s36)

Implemented as an additive offset to fair_value_int on the non-favored side.
"""
...
# === Time-decay — the ONLY addition ===
RAMP_START = 50000           # Start widening at tick 500 (25% of 2k tutorial)
TIME_RAMP_END = 180000       # Full widen by tick 1800 (near end of 2k tutorial)
TIME_MAX_WIDEN = 2           # Max extra offset ticks by ramp end
POS_SCALE_THRESHOLD = 30     # Only widen when |pos| > this (don't penalize flat)
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # ═══════════════════════════════════════════════════
        # TOMATOES — s36 core + time-decay posting width
        # ═══════════════════════════════════════════════════
        if "TOMATOES" in state.order_depths:
            ...
            if book.buy_orders and book.sell_orders:
                ...
                # Compute time-decay widen offset
                time_widen = 0
                if state.timestamp > RAMP_START and abs(pos) > POS_SCALE_THRESHOLD:
                    progress = min(1.0, (state.timestamp - RAMP_START) / (TIME_RAMP_END - RAMP_START))
                ...
                # Phase 2: Directional posting with TIME-DECAY widening
                # time_widen pushes the INCREASING side further from mid
                if self.carry_signal > CARRY_THRESHOLD:
                    ...
                    # Neutral: widen the side that would increase position
                    if buy_capacity > 0:
                        bid_offset = time_widen if pos > POS_SCALE_THRESHOLD else 0
                        bid_price = min(fair_value_int - 1 - bid_offset, best_bid + 1)
                        bid_price = min(bid_price, best_ask - 1)
                        tom_orders.append(Order("TOMATOES", bid_price, buy_capacity))
                    if sell_capacity > 0:
                        ask_offset = time_widen if pos < -POS_SCALE_THRESHOLD else 0
                        ask_price = max(fair_value_int + 1 + ask_offset, best_ask - 1)
                        ask_price = max(ask_price, best_bid + 1)
                        tom_orders.append(Order("TOMATOES", ask_price, -sell_capacity))
