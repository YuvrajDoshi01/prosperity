...
"""
s69_taker — s36 base + selective directional book takes

Architecture:
  Core: IDENTICAL to s36_medallion (proven 2,896 website)
  Addition: When EMA deviation exceeds threshold, take L1 from the book
            in the mean-reversion direction. This builds directional position
            that earns carry PnL (pos × dmid) over 200+ ticks.

  Oracle analysis shows:
  - TOMATOES oracle ceiling = 4,762 from 54 book buys + 44 book sells
  - Oracle's avg dmid before buy = -1.63, before sell = +2.12
  - Oracle builds position gradually (~7 units/tick) and holds 200-900 ticks
  - Our s36 captures taker PnL (2,550) but zero book-take PnL

  Take logic:
  - Track slow EMA of mid price
  - When price deviates > TAKE_THRESHOLD from EMA, take L1 volume
  - Direction: mean reversion (price below EMA → buy at ask, above → sell at bid)
  - Only take TAKE_QTY per tick (gradual position building)
  - Never take if it would exceed position limit

  WARNING: EMA uses mid (volume-independent). Takes use L1 price only.
  NO volume-dependent features in the take signal.
"""
...
# === NEW: Directional take parameters ===
TAKE_EMA_ALPHA = 0.01        # Slow EMA (~400 period) to track "fair" level
TAKE_THRESHOLD = 8.0          # Min EMA deviation to trigger take (in price ticks)
TAKE_QTY = 3                  # Max qty per take (gradual build)
TAKE_COOLDOWN = 5             # Min ticks between takes (avoid whipsaw)
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        result = {}
        conversions = 0
        tick_num = state.timestamp // 100  # approximate tick number
        ...
        # ═══════════════════════════════════════════════════
        # TOMATOES — s36 core + selective directional takes
        # ═══════════════════════════════════════════════════
        if "TOMATOES" in state.order_depths:
            ...
            if book.buy_orders and book.sell_orders:
                ...
                # ═══ NEW: Selective directional take ═══
                ...
                if can_take and abs(ema_dev) > TAKE_THRESHOLD:
                    if ema_dev < -TAKE_THRESHOLD and buy_capacity > 0:
                        # Price well below EMA → buy at ask (mean reversion bet)
                        ...
                        # Price well above EMA → sell at bid (mean reversion bet)
