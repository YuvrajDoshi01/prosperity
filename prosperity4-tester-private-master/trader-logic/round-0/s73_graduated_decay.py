...
"""
s73_graduated_decay — s36 base + single addition: graduated FV decay for stale positions

Only ONE new mechanism on top of s36:
  - After DECAY_START_TICKS of holding a position, FV is progressively skewed
    toward reducing the position (0.01/tick, caps at ±3).
  - This works THROUGH the existing posting engine — no spread crossing,
    no separate flatten orders, no skipped ticks.
  - The s36 logic runs identically every tick. Decay just nudges FV.

No hard leash. No EMA amplification. No entry tracking. Just decay.
"""
...
# === Graduated decay — the ONLY addition ===
DECAY_START_TICKS = 200       # Start nudging FV after this many ticks holding
DECAY_RATE = 0.01             # FV skew per tick beyond start (1.0 skew after 100 extra ticks)
DECAY_MAX_SKEW = 3.0          # Cap at ±3 ticks of FV shift
...
class Trader:
    def __init__(self):
        ...
        # Position age tracking (only new state vs s36)
    ...
    def run(self, state: TradingState):
        ...
        # ═══════════════════════════════════════════════════
        # TOMATOES — s36 core + graduated FV decay
        # ═══════════════════════════════════════════════════
            ...
            if book.buy_orders and book.sell_orders:
                ...
                if self.pos_start_tick is not None and pos != 0:
                    ...
                    if ticks_held > DECAY_START_TICKS:
                        ...
                        if pos > 0:
                            decay_skew = -decay_skew  # lower FV → sell bias
                ...
                # Apply decay skew — the ONLY new line vs s36
