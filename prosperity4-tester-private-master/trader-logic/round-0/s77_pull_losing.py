...
"""
s77_pull_losing — s36 base + pull losing side at high position

Change: When |pos| > PULL_THRESHOLD, STOP posting on the side that would
increase the position. Only post on the reducing side.

  pos > PULL_THRESHOLD:  post sell only (no buy posting)
  pos < -PULL_THRESHOLD: post buy only (no sell posting)
  |pos| <= PULL_THRESHOLD: normal s36 two-sided posting

Takes at FV still happen on both sides (those are free money).
Only the speculative POSTING is pulled.
"""
...
# === Pull losing side — the ONLY addition ===
PULL_THRESHOLD = 60  # Stop posting on increasing side above this
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # ═══════════════════════════════════════════════════
        # TOMATOES — s36 core + pull losing side
        # ═══════════════════════════════════════════════════
        if "TOMATOES" in state.order_depths:
            ...
            if book.buy_orders and book.sell_orders:
                ...
                # Phase 1: Take at fair value (ALWAYS, both sides — free money)
                ...
                # Determine if we should pull the increasing side
                allow_buy_post = pos < PULL_THRESHOLD    # Don't post buys if too long
                allow_sell_post = pos > -PULL_THRESHOLD   # Don't post sells if too short

                # Phase 2: Directional posting with PULL logic
                if self.carry_signal > CARRY_THRESHOLD:
                    if buy_capacity > 0 and allow_buy_post:
                        bid_price = min(fair_value_int - 1, best_bid + 1)
                        bid_price = min(bid_price, best_ask - 1)
                        tom_orders.append(Order("TOMATOES", bid_price, buy_capacity))
                    if sell_capacity > 0 and allow_sell_post:
