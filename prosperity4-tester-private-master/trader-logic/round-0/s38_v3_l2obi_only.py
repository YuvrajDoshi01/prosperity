...
"""
s38_v3 — MINIMAL CHANGE: s36 + L2 OBI posting enhancement ONLY

The deep microstructure analysis found L2 OBI has r=+0.62 with next dmid,
much stronger than total OBI (r~0.35). This version makes the SMALLEST
possible change: use L2 OBI for posting FV while keeping takes identical.

CHANGE FROM s36:
  - Posting uses L2-specific OBI (bv2-av2)/(bv2+av2) instead of total OBI
  - Takes use exactly s36's FV (unchanged)
...
OBI_FV_SHIFT = 0.5  # for takes (unchanged)
L2_OBI_POST_SHIFT = 0.8  # for posting (stronger, because L2 OBI is more predictive)
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # ═══════════════════════════════════════════════════
        # TOMATOES — s36 takes + L2 OBI posting
        # ═══════════════════════════════════════════════════
        if "TOMATOES" in state.order_depths:
            ...
            if book.buy_orders and book.sell_orders:
                ...
                # Total OBI for takes (identical to s36)
                ...
                # ── POST FV: s36 base + L2 OBI enhancement ──
                post_fv = take_fv  # start from same base

                # ONLY CHANGE: Replace total OBI with L2 OBI for posting
                ...
                if bv2 + av2 > 0:
                    ...
                    # Replace the total OBI shift with L2 OBI shift
                    post_fv = post_fv - obi * OBI_FV_SHIFT + l2_obi * L2_OBI_POST_SHIFT
                ...
                # ── Phase 2: Directional posting at post_fv ──
