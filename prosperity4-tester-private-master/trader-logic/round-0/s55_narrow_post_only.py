...
"""
s55_narrow_post_only — s36 base + narrow spread signal for POSTING ONLY

Same insight as s54 but MORE CONSERVATIVE: only use the narrow spread
signal to adjust posting direction (Phase 2). Takes remain identical
to s36.

Why separate: If the narrow spread signal works differently on website
(different volumes → different spread distribution), this limits exposure
to posting shifts only (lower risk than changing take decisions).

The posting shift during narrow spreads OVERRIDES the carry signal
for that tick — the narrow spread signal has higher accuracy (70-100%)
than the carry signal alone.
"""
...
# s55: Direction from narrow spread (post only)
# spread=5,7 → expect UP (bid tight, ask wide)
# spread=6,8,9 → expect DOWN (ask tight, bid wide)
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # ═══════════════════════════════════════════════════
        # TOMATOES — s36 takes + narrow spread directional posting
        # ═══════════════════════════════════════════════════
        if "TOMATOES" in state.order_depths:
            ...
            if book.buy_orders and book.sell_orders:
                ...
                # TAKE FV = standard s36 (NO narrow spread shift for takes)
                ...
                # --- Phase 1: Takes at fair value (IDENTICAL to s36) ---
                ...
                # --- Phase 2: Posting ---
                # s55: On narrow spread ticks, OVERRIDE carry signal with
                # spread-derived direction (higher accuracy, 70-100%)
                if spread in NARROW_UP_SPREADS:
                    # Expect UP: tight bid, wide ask (same as carry > threshold)
                    if buy_capacity > 0:
                    ...
                    # Expect DOWN: wide bid, tight ask (same as carry < -threshold)
                    if sell_capacity > 0:
                        ask_price = max(fair_value_int + 1, best_ask - 1)
                        ask_price = max(ask_price, best_bid + 1)
                        tom_orders.append(Order("TOMATOES", ask_price, -sell_capacity))
                ...
                # Wide spread: use standard carry signal (same as s36)
