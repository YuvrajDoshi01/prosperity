...
"""
s54_narrow_spread — s36 base + narrow spread directional signal

DISCOVERY (log_analysis2.py, consistent across ALL 3 CSV days):
When TOMATOES spread narrows below 13, the spread value predicts
next-tick mid direction with 70-100% accuracy:

  spread=5: +3.8 mean, 96% UP   (ODD → UP)
  spread=6: -1.5 mean, 70% DOWN (EVEN → DOWN)
  spread=7: +1.2 mean, 72% UP   (ODD → UP)
  spread=8: -1.3 mean, 74% DOWN (EVEN → DOWN)
  spread=9: -2.4 mean, 100% DOWN
  spread≥13: ~0, noise

This signal is PURELY PRICE-BASED (spread = best_ask - best_bid).
No volume dependency → should transfer to website without inverted gap.

Implementation: When narrow spread detected, shift FV by expected
directional move. This affects both takes and posts for that tick.
On wide spreads (≥13), use standard s36 FV.

The narrow spread signal is related to the carry signal but MORE
precise — carry fires on bid_delta ≥ 4, but the spread signal gives
the EXACT direction from the spread VALUE itself.
"""

# --- s36 base parameters (unchanged) ---
...
# --- s54: Narrow spread directional FV shift ---
# Expected next-tick dmid by spread value (from log_analysis2.py)
# We use a conservative fraction of the mean to avoid overshooting
NARROW_SPREAD_SHIFT = {
    5: +2.0,   # mean +3.8, 96% UP → shift FV up by 2
    6: -1.0,   # mean -1.5, 70% DOWN → shift FV down by 1
    7: +0.8,   # mean +1.2, 72% UP → shift FV up by 0.8
    8: -0.8,   # mean -1.3, 74% DOWN → shift FV down by 0.8
    9: -1.5,   # mean -2.4, 100% DOWN → shift FV down by 1.5
}
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # ═══════════════════════════════════════════════════
        # TOMATOES — s36 + narrow spread directional FV shift
        # ═══════════════════════════════════════════════════
        if "TOMATOES" in state.order_depths:
            ...
            if book.buy_orders and book.sell_orders:
                ...
                # --- s54: Narrow spread directional FV shift ---
                # When spread is narrow (<13), shift FV by expected next-tick move
                # This is PURELY price-based — no volume dependency
                narrow_shift = NARROW_SPREAD_SHIFT.get(spread, 0.0)
                fair_value += narrow_shift
                ...
                # --- Phase 2: Directional posting (carry signal, same as s36) ---
