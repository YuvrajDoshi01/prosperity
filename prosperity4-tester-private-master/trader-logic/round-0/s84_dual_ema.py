...
"""
s84_dual_ema — s74 base + dual EMA crossover for carry injection

Change from s74: replaces single EMA with fast/slow pair.
  Fast EMA (alpha=0.05, ~20-tick) tracks short-term momentum
  Slow EMA (alpha=0.005, ~200-tick) tracks macro trend

  Crossover signal: fast_ema - slow_ema
  - Large positive → price trending UP above long-term → expect reversion DOWN
  - Large negative → price trending DOWN below long-term → expect reversion UP

  Replaces s74's single-EMA injection/amplification with crossover-based logic.
  All volume-independent. No spread crossing.
"""
...
# === Dual EMA — replaces s74's single EMA ===
FAST_EMA_ALPHA = 0.05         # ~20-tick lookback
SLOW_EMA_ALPHA = 0.005        # ~200-tick lookback
CROSS_AMP_THRESHOLD = 3.0     # Min |fast - slow| to trigger amplification
CROSS_AMP_FACTOR = 0.25       # Extra carry per unit of excess crossover
CROSS_AMP_CAP = 2.0           # Max amplified carry
CROSS_INJECT_THRESHOLD = 5.0  # |crossover| to inject carry when neutral
CROSS_INJECT_STRENGTH = 0.7   # Injected carry magnitude
CROSS_DAMPEN_FACTOR = 0.5     # Carry multiplier when crossover disagrees
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        if saved:
            ...
            self.fast_ema = saved.get("fe")
            self.slow_ema = saved.get("se")
        ...
        # TOMATOES — s36 core + dual EMA crossover
        if "TOMATOES" in state.order_depths:
            ...
            if book.buy_orders and book.sell_orders:
                ...
                if self.fast_ema is None:
                    ...
                    self.fast_ema += FAST_EMA_ALPHA * (mid - self.fast_ema)
                    self.slow_ema += SLOW_EMA_ALPHA * (mid - self.slow_ema)
                ...
                # === Dual EMA crossover amplification (replaces s74 single EMA) ===
                carry_effective = self.carry_signal
                crossover = self.fast_ema - self.slow_ema
                # crossover > 0 → fast above slow → uptrend → expect reversion DOWN
                # crossover < 0 → fast below slow → downtrend → expect reversion UP
                cross_direction = -1.0 if crossover > 0 else (1.0 if crossover < 0 else 0.0)

                if abs(crossover) > CROSS_AMP_THRESHOLD:
                    excess = abs(crossover) - CROSS_AMP_THRESHOLD
                    amp = excess * CROSS_AMP_FACTOR

                    # Carry and crossover AGREE → amplify
                    if (carry_effective > 0 and cross_direction > 0) or (carry_effective < 0 and cross_direction < 0):
                        carry_effective += cross_direction * amp
                        carry_effective = max(-CROSS_AMP_CAP, min(CROSS_AMP_CAP, carry_effective))

                    # Carry and crossover DISAGREE → dampen
                    elif carry_effective != 0 and cross_direction != 0:
                        carry_effective *= CROSS_DAMPEN_FACTOR

                    # Carry neutral, strong crossover → inject
                    elif abs(carry_effective) < CARRY_THRESHOLD and abs(crossover) > CROSS_INJECT_THRESHOLD:
                        carry_effective = cross_direction * CROSS_INJECT_STRENGTH
        ...
        return result, conversions, json.dumps(
            {"mp": self.microprice_history,
             ...
             "fe": round(self.fast_ema, 2) if self.fast_ema is not None else None,
             "se": round(self.slow_ema, 2) if self.slow_ema is not None else None},
            separators=(",", ":")
        )
