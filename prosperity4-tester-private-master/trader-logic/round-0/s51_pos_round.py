...
"""
s51_pos_round — s36 base + position-reducing take rounding

Idea: When we have inventory, round FV in the position-REDUCING direction:
  - When long: floor(FV) → harder to buy more (need ask <= floor(FV)),
    easier to sell (bid >= floor(FV) more likely)
  - When short: ceil(FV) → harder to sell more, easier to buy
  - When flat (|pos| < threshold): use standard round(FV)

This creates a systematic inventory-reducing bias on takes.
Rebuilt on s36 base (NO s47 directional rounding, proven website 2,896).

Different from s47's directional rounding which rounds toward FV DIRECTION
(ceil when FV > mid). This rounds toward POSITION REDUCTION.
"""
...
# s51: Position-reducing rounding
POS_ROUND_THRESHOLD = 10  # Only apply when |pos| > this
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # ═══════════════════════════════════════════════════
        # TOMATOES — s36 + position-reducing take rounding
        # ═══════════════════════════════════════════════════
        if "TOMATOES" in state.order_depths:
            ...
            if book.buy_orders and book.sell_orders:
                ...
                # s51: Position-reducing rounding
                if abs(pos) > POS_ROUND_THRESHOLD:
                    if pos > 0:
                        # Long: floor → harder to buy, easier to sell
                        fair_value_int = math.floor(fair_value)
                    else:
                        # Short: ceil → harder to sell, easier to buy
