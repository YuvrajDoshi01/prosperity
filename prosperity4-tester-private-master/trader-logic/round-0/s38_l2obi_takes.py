...
"""
s38_l2obi_takes — s36 + L2 OBI for both takes AND posting

DISCOVERY: L2 OBI (bv2-av2)/(bv2+av2) has r=+0.62 with next dmid, vs
total OBI at r~0.35. Using L2 OBI specifically for the FV shift gives
+71 PnL on day 0 backtester (2,697 vs s36's 2,626).

CHANGE FROM s36:
  - FV calculation: Replace total OBI shift (0.5) with L2 OBI shift (0.8)
  - This affects BOTH takes and posting
  - Everything else IDENTICAL to s36

WHY IT WORKS:
  The L2 (second-level) volume imbalance reveals the MM bot's NEXT quote
  update before it happens. When L2 bid volume > L2 ask volume, the bot
  is preparing to shift quotes upward. This is 2x more predictive than
  the total OBI because L1 volume is often symmetric (93.4% of ticks)
  while L2 volume carries the real information.
"""
...
# L2 OBI replaces total OBI (was OBI_FV_SHIFT = 0.5 in s36)
L2_OBI_SHIFT = 0.8
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # ═══════════════════════════════════════════════════
        # TOMATOES — s36 + L2 OBI (replaces total OBI)
        # ═══════════════════════════════════════════════════
        if "TOMATOES" in state.order_depths:
            ...
            if book.buy_orders and book.sell_orders:
                ...
                # ══════════════════════════════════════════════════════
                # THE CHANGE: L2 OBI replaces total OBI for FV shift
                # ══════════════════════════════════════════════════════
                ...
                if bv2 + av2 > 0:
                    ...
                    # Fallback to total OBI (same as s36) when no L2 data
