...
"""
s74_ema_carry — s36 base + EMA carry amplification (v2, fixed)

v1 bug: amplification pushed carry from -1.0 to -2.0 but posting only checked
  carry > 0.5 threshold with fixed CARRY_WIDE_OFFSET=3. Magnitude was ignored.
  Amplification had ZERO effect on posting in most cases.

v2 fixes:
  1. Wide-side offset now SCALES with |carry_effective| (1x→2x at carry=2.0)
  2. Strong EMA deviation can INJECT carry when carry is neutral (not just amplify)
  3. When carry and EMA disagree, carry decays FASTER (dampening)

All changes work through existing posting engine. No spread crossing.
"""
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # ═══════════════════════════════════════════════════
        # TOMATOES — s36 core + EMA carry amplification
        # ═══════════════════════════════════════════════════
        if "TOMATOES" in state.order_depths:
            ...
            if book.buy_orders and book.sell_orders:
                ...
                # --- Update EMA (only new computation vs s36) ---
                ...
                # ema_dev > 0 → mid above EMA → expect reversion DOWN
                # ema_dev < 0 → mid below EMA → expect reversion UP
                ...
                # Scale wide offset: at carry=1.0 → base offset, at carry=2.0 → 2x offset
                carry_mag = abs(carry_effective)
                wide_scale = max(1.0, carry_mag)  # 1.0 at normal, up to 2.0 at max
