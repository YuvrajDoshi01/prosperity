...
"""
s38_final — s36 + L2 OBI FV shift + narrow spread directional posting

TWO changes from s36, both grounded in deep microstructure analysis:

1. L2 OBI FV SHIFT (for takes AND posts):
   Replace s36's total OBI shift (coef=0.5) with L2-specific OBI (coef=0.8).
   L2 OBI has r=+0.62 with next dmid vs total OBI's r~0.35.
   L2 reveals the MM bot's next quote direction before it happens,
   because L1 volume is symmetric 93% of ticks while L2 carries info.

2. NARROW SPREAD DIRECTIONAL POSTING (posts only):
   During narrow spreads (7% of ticks, spread ≤ 9), override neutral posting
   with directional posting based on spread value:
     Spread 5 → UP 96-100%, Spread 7 → UP 72-82%
     Spread 6 → DOWN 70-73%, Spread 8 → DOWN 73-74%, Spread 9 → DOWN 94%
   This doesn't change takes — only where we POST our quotes. If the taker
   bot randomly arrives during a narrow spread, we're already positioned
...
# L2 OBI replaces total OBI (THE KEY CHANGE)
L2_OBI_SHIFT = 0.8
...
TERMINAL_TIMESTAMP = 190000000
TERMINAL_POSITION_THRESHOLD = 28

LIQUIDATION_WINDOW = 10

# Narrow spread direction (posting only, never takes)
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # ═══════════════════════════════════════════════════
        # TOMATOES — s36 + L2 OBI + narrow spread posting
        # ═══════════════════════════════════════════════════
        if "TOMATOES" in state.order_depths:
            ...
            if book.buy_orders and book.sell_orders:
                ...
                # ═══ CHANGE 1: L2 OBI replaces total OBI ═══
                ...
                if bv2 + av2 > 0:
                    ...
                    # Fallback to total OBI when no L2 data
                ...
                # ── Phase 1: Take at fair value (using L2 OBI FV) ──
                ...
                # ═══ CHANGE 2: Narrow spread directional posting ═══
                # Determine posting signal: carry + narrow spread override
                posting_signal = self.carry_signal

                # During narrow spreads, add directional bias from spread value
                spread_int = int(spread)
                if spread_int in SPREAD_DIRECTION:
                    narrow_dir = SPREAD_DIRECTION[spread_int]
                    # Narrow spread signal is additive, not override
                    # It activates directional posting even when carry is neutral
