...
"""
s69_taker_leash — s36 base + selective directional book takes + Time Leash

Architecture:
  Core: IDENTICAL to s36_medallion (proven 2,896 website)
  Addition: When EMA deviation exceeds threshold, take L1 from the book.
  Leash: If a position is held for > 350 ticks without reverting, flatten it.
"""
...
TAKE_QTY = 3                  
TAKE_COOLDOWN = 5             
POSITION_LEASH_TICKS = 350    # Max ticks to hold a position before puking
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # ═══════════════════════════════════════════════════
        # TOMATOES — s36 core + directional takes + leash
        # ═══════════════════════════════════════════════════
        if "TOMATOES" in state.order_depths:
            ...
            if book.buy_orders and book.sell_orders:
                ...
                # Sniper Maker logic (Passive Directional)
                    ...
                    if can_take and abs(ema_dev) > TAKE_THRESHOLD:
                        if ema_dev < -TAKE_THRESHOLD and buy_capacity > 0:
                            ...
                            if take_qty > 0:
                                # Step exactly 1 tick in front of the best bid, but don't hit the ask
                                snipe_bid = min(best_bid + 1, best_ask - 1)
                                tom_orders.append(Order("TOMATOES", snipe_bid, take_qty))
                            ...
                            if take_qty > 0:
                                # Step exactly 1 tick in front of the best ask, but don't hit the bid
                                snipe_ask = max(best_ask - 1, best_bid + 1)
                                tom_orders.append(Order("TOMATOES", snipe_ask, -take_qty))
