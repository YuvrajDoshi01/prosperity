...
"""
s57_narrow_take_1 — s36 base + minimal (+1/-1) take FV shift on narrow spreads

s54's large FV shifts (+2/-1.5) during narrow spreads HURT because they
cause taking on the WRONG side of the spread during the narrow-spread tick.

s57 tries a MINIMAL approach: when we see a narrow spread, shift take FV
by just ±1 in the predicted direction. This is enough to:
- Cross one more level when the signal says UP (buy 1 tick higher)
- Avoid one take when signal says DOWN (don't sell at current bid)

The key insight: during narrow spreads, asks are closer to mid.
If we KNOW the mid is about to move UP, buying at the current ask
(which is only ~3-5 ticks above mid instead of ~7) is a great trade.

This version ONLY shifts takes during narrow spreads. Posts are s36.
"""
...
# s57: Minimal take FV shift during narrow spreads
NARROW_UP_SPREADS = {5, 7}
NARROW_DOWN_SPREADS = {6, 8, 9}
NARROW_TAKE_SHIFT = 1  # shift FV by ±1 tick
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # ═══════════════════════════════════════════════════
        # TOMATOES — s36 + minimal take shift on narrow spreads
        # ═══════════════════════════════════════════════════
        if "TOMATOES" in state.order_depths:
            ...
            if book.buy_orders and book.sell_orders:
                ...
                # --- s57: During narrow spreads, shift take FV by ±1 ---
                if spread in NARROW_UP_SPREADS:
                    take_fv = fair_value_int + NARROW_TAKE_SHIFT  # buy more aggressively
                elif spread in NARROW_DOWN_SPREADS:
                    take_fv = fair_value_int - NARROW_TAKE_SHIFT  # sell more aggressively
                else:
                    take_fv = fair_value_int  # normal s36
                ...
                # --- Phase 1: Takes at shifted FV ---
                for price, vol in sorted(book.sell_orders.items()):
                    if buy_capacity > 0 and price <= take_fv:
                        qty = min(buy_capacity, -vol)
                        tom_orders.append(Order("TOMATOES", price, qty))
                        buy_capacity -= qty

                for price, vol in sorted(book.buy_orders.items(), reverse=True):
                    if sell_capacity > 0 and price >= take_fv:
                        qty = min(sell_capacity, vol)
                        tom_orders.append(Order("TOMATOES", price, -qty))
                        sell_capacity -= qty
                ...
                # --- Phase 2: Posting (IDENTICAL to s36) ---
