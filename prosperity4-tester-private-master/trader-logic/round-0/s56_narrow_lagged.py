...
"""
s56_narrow_lagged — s36 base + LAGGED narrow spread signal

Key insight from s54/s55 failure analysis:
- Narrow spreads last 1 tick then revert to 13/14
- The signal predicts the NEXT tick's direction, not the current tick
- So we should SAVE the signal and apply it on the NEXT tick's posting

When we see spread=5 at tick T (expect UP at T+1):
  At tick T: don't do anything special (narrow spread, let s36 handle it)
  At tick T+1: we KNOW mid should be moving UP → post directionally
    - Tight bid (expect to get filled as mid rises)
    - Wide ask (if we get hit, it's at a premium)

This is the CORRECT temporal alignment: observe signal at T, act at T+1.

Also: since the signal is about NEXT tick, we use it for POSTING only
(takes execute against the current book, which already reflects T+1).

The lagged signal REPLACES carry signal for one tick when triggered.
"""
...
# s56: Narrow spread direction signals (lagged by 1 tick)
NARROW_UP_SPREADS = {5, 7}     # ODD → next tick UP
NARROW_DOWN_SPREADS = {6, 8, 9}  # EVEN/9 → next tick DOWN
...
class Trader:
    def __init__(self):
        ...
        self.narrow_signal = 0  # +1 = expect UP, -1 = expect DOWN, 0 = no signal
    ...
    def run(self, state: TradingState):
        ...
        if saved:
            ...
            self.narrow_signal = saved.get("ns", 0)
        ...
        # ═══════════════════════════════════════════════════
        # TOMATOES — s36 + lagged narrow spread posting
        # ═══════════════════════════════════════════════════
        if "TOMATOES" in state.order_depths:
            ...
            if book.buy_orders and book.sell_orders:
                ...
                # --- s56: Read lagged narrow signal from PREVIOUS tick ---
                # This was set last tick based on that tick's spread
                use_narrow = self.narrow_signal  # +1=UP, -1=DOWN, 0=none

                # --- s56: Set narrow signal for NEXT tick based on current spread ---
                if spread in NARROW_UP_SPREADS:
                    self.narrow_signal = 1   # next tick should go UP
                elif spread in NARROW_DOWN_SPREADS:
                    self.narrow_signal = -1  # next tick should go DOWN
                else:
                    self.narrow_signal = 0   # no signal
                ...
                # --- Phase 2: Posting ---
                # s56: If we have a lagged narrow signal, use it for posting direction
                # Otherwise fall back to carry signal (same as s36)
                if use_narrow > 0:
                    # Previous tick had narrow UP spread → expect this tick to go UP
                    # Tight bid, wide ask
                    if buy_capacity > 0:
                    ...
                    # Previous tick had narrow DOWN spread → expect this tick to go DOWN
                    # Wide bid, tight ask
                    if sell_capacity > 0:
                        ask_price = max(fair_value_int + 1, best_ask - 1)
                        ask_price = max(ask_price, best_bid + 1)
                ...
                # No narrow signal: fall back to carry signal (same as s36)
