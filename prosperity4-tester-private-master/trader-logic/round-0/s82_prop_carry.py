...
"""
s82_prop_carry — s74 base + proportional carry magnitude

Change from s74: carry signal magnitude scales with bid_delta size.
  s74: bid_delta >= 4 → carry = ±1.0 always (binary)
  s82: bid_delta >= 4 → carry = min(CARRY_CAP, delta / CARRY_SCALE)
       e.g. delta=4 → carry=0.8, delta=6 → carry=1.2, delta=8 → carry=1.6

Bigger moves → stronger carry → wider non-favored posting → more edge retained.
...
# === Proportional carry — the ONLY addition vs s74 ===
CARRY_SCALE = 5.0             # delta / CARRY_SCALE = carry magnitude
CARRY_MAG_CAP = 2.0           # Max raw carry before EMA amplification
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # TOMATOES — s74 core + proportional carry
