...
"""
s75_asym_size — s36 base + asymmetric SIZE posting

Change: When carry signal is active, post MORE units on the favored side
and FEWER on the non-favored side. s36 already skews PRICE but posts
full capacity on both sides. This skews QUANTITY instead.

  carry > 0 (expect UP):  buy_qty = full capacity, sell_qty = capacity * UNFAVORED_RATIO
  carry < 0 (expect DOWN): sell_qty = full capacity, buy_qty = capacity * UNFAVORED_RATIO
  neutral: symmetric (identical to s36)
"""
...
# === Asymmetric size — the ONLY addition ===
UNFAVORED_RATIO = 0.5  # Post 50% capacity on the non-signal side
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # ═══════════════════════════════════════════════════
        # TOMATOES — s36 core + asymmetric size posting
        # ═══════════════════════════════════════════════════
        if "TOMATOES" in state.order_depths:
            ...
            if book.buy_orders and book.sell_orders:
                ...
                # Phase 2: Directional posting with ASYMMETRIC SIZE
                if self.carry_signal > CARRY_THRESHOLD:
                    # Expect UP: full buy, reduced sell
                    post_buy = buy_capacity
                    post_sell = max(1, int(sell_capacity * UNFAVORED_RATIO))
                    if post_buy > 0:
                        bid_price = min(fair_value_int - 1, best_bid + 1)
                        bid_price = min(bid_price, best_ask - 1)
                        tom_orders.append(Order("TOMATOES", bid_price, post_buy))
                    if post_sell > 0:
                        ...
                        tom_orders.append(Order("TOMATOES", ask_price, -post_sell))

                elif self.carry_signal < -CARRY_THRESHOLD:
                    # Expect DOWN: full sell, reduced buy
                    post_sell = sell_capacity
                    post_buy = max(1, int(buy_capacity * UNFAVORED_RATIO))
                    if post_sell > 0:
                        ask_price = max(fair_value_int + 1, best_ask - 1)
                        ask_price = max(ask_price, best_bid + 1)
                        tom_orders.append(Order("TOMATOES", ask_price, -post_sell))
