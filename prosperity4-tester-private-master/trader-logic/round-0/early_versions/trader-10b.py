...
class TomatoesStrategy(MarketMakingStrategy):
    ...
    def compute_vwap_mid(self, order_depth: OrderDepth) -> float:
        """VWAP across all book levels — RMSE 1.05 vs microprice 1.17."""
        # Bid side: volume-weighted average price
        bid_vwap_num = sum(price * abs(vol) for price, vol in order_depth.buy_orders.items())
        bid_vwap_den = sum(abs(vol) for vol in order_depth.buy_orders.values())
        # Ask side: volume-weighted average price
        ask_vwap_num = sum(price * abs(vol) for price, vol in order_depth.sell_orders.items())
        ask_vwap_den = sum(abs(vol) for vol in order_depth.sell_orders.values())
    ...
    def get_true_value(self, state: TradingState) -> int:
        order_depth = state.order_depths[self.symbol]
        vwap_mid = self.compute_vwap_mid(order_depth)

        if len(self.cache) == self.cache_dim:
            self.cache.pop(0)
        self.cache.append(vwap_mid)
    ...
    def _predict_next(self) -> int:
        ...
        coef = [-0.009659, 0.036465, 0.140155, 0.831947]
        intercept = 5.444233
        pred = intercept
