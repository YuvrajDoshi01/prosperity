    ...
    def run(self, state: Any):
        ...
        for product in state.order_depths:
            ...
            # --- TOMATOES ROUTE ---
                ...
                my_bid_price = math.floor(fair_value - (self.spread / 2.0))
                my_ask_price = math.ceil(fair_value + (self.spread / 2.0))

                make_buy_qty = self.pos_limit - current_pos
                make_sell_qty = self.pos_limit + current_pos

                if valid_bids and my_bid_price == max(valid_bids.keys()):
                    my_bid_price += 1
                if valid_asks and my_ask_price == min(valid_asks.keys()):
                    my_ask_price -= 1
