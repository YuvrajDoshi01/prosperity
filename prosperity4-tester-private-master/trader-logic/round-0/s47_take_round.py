...
"""
s47_take_round — s36 + conservative directional take rounding

ONLY change from s36: when |pos| < 10 and |FV - mid| > 0.3, use ceil/floor
instead of round() for fair_value_int. This biases takes toward the predicted
direction when inventory risk is low and FV signal is directional.

Backtester results (all 3 days):
  s36:  d-2=3,202  d-1=3,276  d0=2,626  total=9,104
  s47:  d-2=3,290  d-1=3,640  d0=2,852  total=9,782  (+678)

Ablation confirmed: take rounding carries ALL the edge.
Posting skew adds +31 total and interferes on d0 (-121).

Safety: pos_cap=10 prevents catastrophic inventory accumulation that killed s45.
"""
...
# --- s47 NEW: Conservative directional take rounding ---
TAKE_ROUND_POS_CAP = 10    # Only round directionally when |pos| < this
TAKE_ROUND_FV_THRESH = 0.3 # Only when |FV - mid| > this
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # ═══════════════════════════════════════════════════
        # TOMATOES — s36 + directional take rounding
        # ═══════════════════════════════════════════════════
        if "TOMATOES" in state.order_depths:
            ...
            if book.buy_orders and book.sell_orders:
                ...
                # When flat (|pos| < 10) and FV clearly disagrees with mid,
                # round FV toward the predicted direction instead of nearest int
