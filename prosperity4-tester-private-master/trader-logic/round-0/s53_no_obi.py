...
"""
s53_no_obi — s36 base but DROP the OBI shift (volume-dependent)

The OBI shift uses (total_bid_vol - total_ask_vol) / sum — this is
a normalized volume ratio. While it showed +39 PnL on the website
for s36, the s49 inverted gap (2,852 BT → 2,450 website) suggests
volume-dependent features might be hurting more than we thought.

This version keeps microprice regression (with its volume component)
but removes the explicit OBI shift. If OBI was the culprit in s49's
failure, this should score closer to or above s36.

s36 with OBI = 2,896 website
s53 without OBI = ??? (if higher, OBI hurts on some days)
"""
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # ═══════════════════════════════════════════════════
        # TOMATOES — s36 WITHOUT OBI shift
        # ═══════════════════════════════════════════════════
        if "TOMATOES" in state.order_depths:
            ...
            if book.buy_orders and book.sell_orders:
                ...
                # Microprice regression (same as s36)
                ...
                # NO OBI shift — this is the ONLY difference from s36
