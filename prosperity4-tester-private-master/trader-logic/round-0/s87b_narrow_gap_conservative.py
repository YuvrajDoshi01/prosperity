...
"""
s87b_narrow_gap_conservative — s74 base + narrow-spread gap_asym (FV shift ONLY)

Conservative variant of s87: uses gap_asym to shift FV during narrow spread
but does NOT extend the take range. Only affects posting placement.

Rationale: if the aggressive take extension in s87 causes position overload,
this variant captures the signal through better posting only.
"""
...
# === Narrow-spread gap_asym (CONSERVATIVE — FV shift only) ===
NARROW_SPREAD_THRESHOLD = 10
GAP_ASYM_FV_SHIFT = 1.5        # Smaller shift than s87 (1.5 vs 2.0)
# No take extension — only posting is affected
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # ═══════════════════════════════════════════════════
        # TOMATOES — s74 core + narrow-spread gap_asym (conservative)
        # ═══════════════════════════════════════════════════
        if "TOMATOES" in state.order_depths:
            ...
            if book.buy_orders and book.sell_orders:
                ...
                # === Narrow-spread gap_asym FV shift (conservative) ===
                if spread < NARROW_SPREAD_THRESHOLD:
                    ...
                    if len(sorted_bids) >= 2 and len(sorted_asks) >= 2:
                        ...
                        fair_value += -normalized * GAP_ASYM_FV_SHIFT
                ...
                # Phase 1: Take at fair value (standard — no extra aggression)
                ...
                # Phase 2: Directional posting (identical to s74)
