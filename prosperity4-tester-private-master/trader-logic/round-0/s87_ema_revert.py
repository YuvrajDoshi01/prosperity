...
"""
s87_ema_revert -- s74 base + mean-reversion-to-EMA signal

New signal: dev_ema20
  - Track a 20-tick EMA of the mid price (ema_alpha = 2/21 ~ 0.095)
  - deviation = mid - ema20
  - When |deviation| > 1.0, shift FV by -0.3 * sign(deviation)
    (push FV toward EMA, exploiting mean-reversion tendency r=-0.386)
  - VOLUME-INDEPENDENT: uses only mid prices, should transfer to website
...
# === NEW: Mean-reversion-to-EMA signal (s87) ===
EMA20_ALPHA = 2.0 / (20 + 1)  # ~0.09524, 20-tick EMA
EMA20_DEV_THRESHOLD = 1.0     # Min |deviation| to trigger FV shift
EMA20_FV_SHIFT = 0.3          # FV shift magnitude (in mean-reversion direction)
...
class Trader:
    def __init__(self):
        ...
        # New: 20-tick EMA for mean-reversion signal
        self.ema20 = None
    ...
    def run(self, state: TradingState):
        ...
        # ===================================================
        # TOMATOES -- s74 core + mean-reversion-to-EMA signal
        # ===================================================
        if "TOMATOES" in state.order_depths:
            ...
            if book.buy_orders and book.sell_orders:
                ...
                # --- Update 20-tick EMA for mean-reversion signal (NEW) ---
                if self.ema20 is None:
                    self.ema20 = mid
                else:
                    self.ema20 += EMA20_ALPHA * (mid - self.ema20)
                ...
                # === NEW: Mean-reversion-to-EMA FV adjustment ===
                deviation = mid - self.ema20
                if abs(deviation) > EMA20_DEV_THRESHOLD:
                    # Shift FV toward EMA (mean-reversion direction)
                    # deviation > 0 → mid above EMA → expect revert down → lower FV
                    # deviation < 0 → mid below EMA → expect revert up → raise FV
                    if deviation > 0:
                        fair_value -= EMA20_FV_SHIFT
                    else:
                        fair_value += EMA20_FV_SHIFT
        ...
        return result, conversions, json.dumps(
            {"mp": self.microprice_history,
             ...
             "e2": round(self.ema20, 2) if self.ema20 is not None else None},
            separators=(",", ":")
        )
